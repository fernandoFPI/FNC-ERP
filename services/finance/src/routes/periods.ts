...
periodsRouter.get('/', requirePermission('finance.periods.view', 'view'), async (req, res) => {
  ...
  } catch (err) { sendError(res, 500, 'INTERNAL_ERROR', 'Failed to fetch periods', err) }
})
...
periodsRouter.post('/', requirePermission('finance.periods.admin', 'admin'), async (req, res) => {
  try {
    ...
    if (start_date >= end_date) return sendError(res, 400, 'INVALID_DATES', 'start_date must be before end_date')
    const result = await query(
    ...
    await logAudit({ companyId, userId: req.auth!.userId, action: 'CREATE', tableName: 'accounting_periods', recordId: result.rows[0]!['id'] as string })
    sendOk(res, result.rows[0], 201)
  } catch (err: unknown) {
    const e = err as { code?: string }
    if (e.code === '23505') return sendError(res, 409, 'DUPLICATE_PERIOD', 'A period with this start date already exists')
    sendError(res, 500, 'INTERNAL_ERROR', 'Failed to create period', err)
  }
})

periodsRouter.post('/:id/close', requirePermission('finance.periods.admin', 'admin'), async (req, res) => {
  const companyId = req.auth!.companyId
  ...
  try {
    ...
    const period = periodResult.rows[0] as { id: string; status: string; start_date: string; end_date: string } | undefined
    if (!period) { await client.query('ROLLBACK'); return sendError(res, 404, 'NOT_FOUND', 'Period not found') }
    if (period.status === 'closed') { await client.query('ROLLBACK'); return sendError(res, 409, 'ALREADY_CLOSED', 'Period already closed') }
    ...
    if (draftCount > 0) {
      ...
      return sendError(res, 422, 'DRAFT_JOURNALS_EXIST', `Cannot close period: ${draftCount} draft journal entries must be posted or cancelled first`)
    }
    ...
    await logAudit({ companyId, userId: req.auth!.userId, action: 'UPDATE', tableName: 'accounting_periods', recordId: period.id, newValues: { status: 'closed' } })
