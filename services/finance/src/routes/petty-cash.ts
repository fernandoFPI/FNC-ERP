...
pettyCashRouter.get('/floats', requirePermission('finance.petty_cash.view', 'view'), async (req, res) => {
  try {
    const r = await query(
...
pettyCashRouter.post('/floats', requirePermission('finance.petty_cash.edit', 'edit'), async (req, res) => {
  ...
  try {
    ...
    const result = await withTransaction(
      ...
      async (client) => {
        const fRes = await client.query(
          ...
          [req.auth!.companyId, d.name, d.location ?? null, d.custodian_name ?? null, d.currency_code, d.authorized_limit, d.opening_balance, d.gl_account_id ?? null, d.notes ?? null],
        )
    ...
    if ((err as { code?: string }).code === '23505') return sendError(res, 409, 'DUPLICATE', 'Float name already exists')
    sendError(res, 500, 'INTERNAL_ERROR', 'Failed to create float', err)
  }
})

pettyCashRouter.put('/floats/:id', requirePermission('finance.petty_cash.edit', 'edit'), async (req, res) => {
  const schema = z.object({
    name: z.string().min(1), location: z.string().optional(), custodian_name: z.string().optional(),
    authorized_limit: z.coerce.number().positive(), gl_account_id: z.string().uuid().optional(),
    is_active: z.boolean().default(true), notes: z.string().optional(),
  })
  try {
    const d = schema.parse(req.body)
    const r = await query(
      ...
      [d.name, d.location ?? null, d.custodian_name ?? null, d.authorized_limit, d.gl_account_id ?? null, d.is_active, d.notes ?? null, req.params['id'], req.auth!.companyId],
    )
    if (!r.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Float not found')
    sendOk(res, r.rows[0])
  } catch (err) {
...
pettyCashRouter.get('/floats/:id', requirePermission('finance.petty_cash.view', 'view'), async (req, res) => {
  try {
    ...
    if (!fRes.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Float not found')
...
pettyCashRouter.post('/floats/:id/spend', requirePermission('finance.petty_cash.edit', 'edit'), async (req, res) => {
  const schema = z.object({
  ...
  try {
    const d = schema.parse(req.body)
    const floatRes = await query(`SELECT * FROM petty_cash_floats WHERE id=$1 AND company_id=$2`, [req.params['id'], req.auth!.companyId])
    if (!floatRes.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Float not found')
    const float_ = floatRes.rows[0]!
    if (Number(float_['current_balance']) < d.amount) {
      return sendError(res, 422, 'INSUFFICIENT_BALANCE', `Insufficient balance. Current: ${float_['current_balance']}`)
    }
    ...
    const result = await withTransaction(
      ...
      async (client) => {
        const txnRes = await client.query(
          ...
          [req.params['id'], req.auth!.companyId, d.transaction_date, d.description, d.category_id ?? null, d.category_name ?? null, d.gl_account_id ?? null, d.amount, newBalance, d.receipt_url ?? null, req.auth!.userId],
        )
        await client.query(`UPDATE petty_cash_floats SET current_balance=$1, updated_at=NOW() WHERE id=$2`, [newBalance, req.params['id']])
        ...
        if (d.gl_account_id && float_['gl_account_id']) {
          const jeRes = await client.query(
            ...
            [req.auth!.companyId, d.transaction_date, d.description, float_['currency_code'], req.auth!.userId],
          )
          ...
          await client.query(
            ...
            [jeId, req.auth!.companyId, d.gl_account_id, float_['currency_code'], d.amount, d.description, float_['gl_account_id']],
          )
          await client.query(`UPDATE petty_cash_transactions SET journal_entry_id=$1 WHERE id=$2`, [jeId, txnRes.rows[0]!['id']])
        }
...
pettyCashRouter.post('/floats/:id/replenish', requirePermission('finance.petty_cash.edit', 'edit'), async (req, res) => {
  const schema = z.object({ requested_amount: z.coerce.number().positive(), notes: z.string().optional() })
  try {
    const { requested_amount, notes } = schema.parse(req.body)
    const existing = await query(`SELECT id FROM petty_cash_floats WHERE id=$1 AND company_id=$2`, [req.params['id'], req.auth!.companyId])
    if (!existing.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Float not found')
    ...
    const r = await query(
      ...
      [req.params['id'], req.auth!.companyId, num, requested_amount, notes ?? null, req.auth!.userId],
    )
...
pettyCashRouter.post('/replenishments/:replenId/approve', requirePermission('finance.petty_cash.approve', 'approve'), async (req, res) => {
  const schema = z.object({
  ...
  try {
    ...
    if (!repRes.rows[0]) return sendError(res, 409, 'INVALID_STATUS', 'Replenishment not found or not pending')
      ...
      async (client) => {
        ...
        if (rep['float_gl_account_id'] && offset_account_id) {
          const jeRes = await client.query(
            ...
            [req.auth!.companyId, rep['replenishment_number'], rep['currency_code'], req.auth!.userId],
          )
          ...
          await client.query(
            ...
            [jeId, req.auth!.companyId, rep['float_gl_account_id'], rep['currency_code'], approved_amount, offset_account_id],
          )
        }
        ...
        await client.query(`UPDATE petty_cash_floats SET current_balance=$1, updated_at=NOW() WHERE id=$2`, [newBalance, rep['float_id']])
        await client.query(
          ...
          [rep['float_id'], req.auth!.companyId, rep['replenishment_number'], approved_amount, newBalance, jeId, req.auth!.userId],
        )
        return { approved: true, new_balance: newBalance, journal_entry_id: jeId }
      },
    )
    await logAudit({ companyId: req.auth!.companyId, userId: req.auth!.userId, action: 'UPDATE', tableName: 'petty_cash_replenishments', recordId: req.params['replenId'], newValues: { status: 'posted', approved_amount } })
    sendOk(res, result)
  } catch (err) {
...
pettyCashRouter.post('/replenishments/:replenId/reject', requirePermission('finance.petty_cash.approve', 'approve'), async (req, res) => {
  ...
  try {
    ...
    if (!r.rows[0]) return sendError(res, 409, 'INVALID_STATUS', 'Replenishment not found or not pending')
    sendOk(res, r.rows[0])
  } catch (err) {
...
pettyCashRouter.get('/replenishments', requirePermission('finance.petty_cash.view', 'view'), async (req, res) => {
  try {
    const r = await query(
