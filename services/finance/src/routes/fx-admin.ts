...
fxAdminRouter.post('/sync', requirePermission('finance.fx_rates.edit', 'edit'), async (req, res) => {
  try {
    await query(
...
fxAdminRouter.get('/sync-history', requirePermission('finance.fx_rates.view', 'view'), async (req, res) => {
  try {
...
fxAdminRouter.get('/change-log', requirePermission('finance.fx_rates.view', 'view'), async (req, res) => {
  try {
    ...
    if (from_currency) { conditions.push(`frc.from_currency = $${++p}`); values.push(from_currency) }
    if (to_currency) { conditions.push(`frc.to_currency = $${++p}`); values.push(to_currency) }
...
fxAdminRouter.get('/staleness', requirePermission('finance.fx_rates.view', 'view'), async (req, res) => {
  try {
...
fxAdminRouter.get('/alert-config', requirePermission('finance.fx_rates.view', 'view'), async (req, res) => {
  try {
    const result = await query(
      `SELECT * FROM fx_rate_alert_config WHERE company_id = $1`,
      [req.auth!.companyId],
    )
...
fxAdminRouter.put('/alert-config', requirePermission('finance.fx_rates.edit', 'edit'), async (req, res) => {
  if (!['company_admin', 'system_admin', 'module_admin'].includes(req.auth!.role)) {
    return sendError(res, 403, 'FORBIDDEN', 'Company admin or finance module admin required')
  }
  ...
  const { warn_after_hours, critical_after_hours, large_movement_pct, notify_finance_admins } = parsed.data

  if (critical_after_hours <= warn_after_hours) {
    return sendError(res, 400, 'INVALID_CONFIG', 'critical_after_hours must be greater than warn_after_hours')
  }
