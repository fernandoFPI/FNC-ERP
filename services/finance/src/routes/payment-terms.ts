...
const lineSchema = z.object({
  ...
  due_type: z.enum(['immediate', 'days', 'end_of_month', 'end_of_next_month', 'retention']).default('days'),
...
paymentTermsRouter.get('/:id', requirePermission('finance.terms.view', 'view'), async (req, res) => {
  try {
    const termRes = await query(
      `SELECT * FROM payment_terms WHERE id = $1 AND company_id = $2`,
      [req.params['id'], req.auth!.companyId],
    )
    if (!termRes.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Payment term not found')
...
paymentTermsRouter.post('/', requirePermission('finance.terms.edit', 'edit'), async (req, res) => {
  try {
      ...
      async (client) => {
        ...
        for (const line of d.lines) {
          const lr = await client.query(
            ...
            [term['id'], line.sequence, line.description ?? null, line.value_type, line.value, line.due_type, line.days, line.is_retention],
          )
    ...
    await logAudit({ companyId: req.auth!.companyId, userId: req.auth!.userId, action: 'INSERT', tableName: 'payment_terms', recordId: result['id'] as string, newValues: { name: d.name } })
    sendOk(res, result, 201)
  } catch (err) {
    if ((err as { code?: string }).code === '23505') return sendError(res, 409, 'DUPLICATE', 'Payment term name already exists')
...
paymentTermsRouter.put('/:id', requirePermission('finance.terms.edit', 'edit'), async (req, res) => {
  try {
    const d = termSchema.parse(req.body)
    const existing = await query(`SELECT id FROM payment_terms WHERE id=$1 AND company_id=$2`, [req.params['id'], req.auth!.companyId])
    if (!existing.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Payment term not found')
      ...
      async (client) => {
        ...
        for (const line of d.lines) {
          const lr = await client.query(
            `INSERT INTO payment_term_lines (term_id, sequence, description, value_type, value, due_type, days, is_retention)
             VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`,
            [req.params['id'], line.sequence, line.description ?? null, line.value_type, line.value, line.due_type, line.days, line.is_retention],
          )
          lines.push(lr.rows[0])
        }
...
paymentTermsRouter.delete('/:id', requirePermission('finance.terms.edit', 'edit'), async (req, res) => {
  try {
    const r = await query(
      `DELETE FROM payment_terms WHERE id=$1 AND company_id=$2 RETURNING id`,
      [req.params['id'], req.auth!.companyId],
    )
    if (!r.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Payment term not found')
...
paymentTermsRouter.post('/:id/compute', requirePermission('finance.terms.view', 'view'), async (req, res) => {
  ...
  try {
    ...
    if (!linesRes.rows.length) return sendError(res, 404, 'NOT_FOUND', 'Payment term not found or has no lines')

    const baseDate = new Date(invoice_date)

    const schedule = linesRes.rows.map(line => {
      const lineAmount = line['value_type'] === 'percent'
      ...
      return {
        ...
        description: line['description'] ?? (line['is_retention'] ? 'Retention' : `Installment ${line['sequence']}`),
        amount: lineAmount,
    ...
    sendOk(res, { amount, invoice_date, schedule, total: schedule.reduce((s, l) => s + l.amount, 0) })
  } catch (err) {
