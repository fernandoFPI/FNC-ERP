...
revaluationRouter.get('/', requirePermission('finance.revaluation.view', 'view'), async (req, res) => {
  try {
    const r = await query(
...
revaluationRouter.get('/:id', requirePermission('finance.revaluation.view', 'view'), async (req, res) => {
  try {
    const r = await query(`SELECT * FROM fx_revaluation_runs WHERE id=$1 AND company_id=$2`, [req.params['id'], req.auth!.companyId])
    if (!r.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Revaluation run not found')
...
revaluationRouter.post('/compute', requirePermission('finance.revaluation.view', 'view'), async (req, res) => {
...
revaluationRouter.post('/post', requirePermission('finance.revaluation.edit', 'edit'), async (req, res) => {
  const schema = z.object({
    period:               z.string().regex(/^\d{4}-\d{2}$/),
    run_date:             z.string().regex(/^\d{4}-\d{2}-\d{2}$/),
    functional_currency:  z.string().length(3).default('IQD'),
  ...
  try {
    ...
    const result = await withTransaction(
      ...
      async (client) => {
        ...
        const jeRes = await client.query(
          ...
          [req.auth!.companyId, d.run_date, `REVAL-${d.period}`, `FX Revaluation ${d.period}`, d.functional_currency, req.auth!.userId],
        )
            ...
            await client.query(
              `INSERT INTO journal_lines (journal_entry_id, company_id, account_id, currency_code, debit, credit, description)
               VALUES ($1,$2,$3,$4,$5,0,$6),
                      ($1,$2,$7,$4,0,$5,$6)`,
              [jeId, req.auth!.companyId, line.account_id, d.functional_currency, absGL, `FX Revaluation ${d.period}`, d.gain_account_id],
            )
          } else {
            ...
            await client.query(
              `INSERT INTO journal_lines (journal_entry_id, company_id, account_id, currency_code, debit, credit, description)
               VALUES ($1,$2,$3,$4,0,$5,$6),
                      ($1,$2,$7,$4,$5,0,$6)`,
              [jeId, req.auth!.companyId, line.account_id, d.functional_currency, absGL, `FX Revaluation ${d.period}`, d.loss_account_id],
            )
          }
        }
        ...
        const runRes = await client.query(
          ...
          [req.auth!.companyId, d.run_date, d.period, totalGainLoss, jeId, d.notes ?? null, req.auth!.userId],
        )
        ...
        for (const line of d.lines) {
          await client.query(
            ...
            [run['id'], req.auth!.companyId, line.account_id, line.currency_code, line.original_balance, line.fx_rate_used, line.revalued_balance, line.gain_loss],
          )
        }
    ...
    await logAudit({ companyId: req.auth!.companyId, userId: req.auth!.userId, action: 'INSERT', tableName: 'fx_revaluation_runs', recordId: result['id'] as string, newValues: { period: d.period, total_gain_loss: totalGainLoss } })
...
revaluationRouter.post('/:id/reverse', requirePermission('finance.revaluation.edit', 'edit'), async (req, res) => {
  const schema = z.object({ reversal_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/), notes: z.string().optional() })
  try {
    const { reversal_date, notes } = schema.parse(req.body)
    const runRes = await query(`SELECT * FROM fx_revaluation_runs WHERE id=$1 AND company_id=$2`, [req.params['id'], req.auth!.companyId])
    if (!runRes.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Run not found')
    const run = runRes.rows[0]!
    if (run['status'] !== 'posted') return sendError(res, 409, 'INVALID_STATUS', 'Only posted runs can be reversed')

    const linesRes = await query(`SELECT * FROM fx_revaluation_lines WHERE run_id=$1`, [req.params['id']])
    ...
    const result = await withTransaction(
      ...
      async (client) => {
        ...
        const jeRes = await client.query(
          `INSERT INTO journal_entries (company_id, entry_date, reference, description, currency_code, status, created_by)
           VALUES ($1,$2,$3,$4,$5,'posted',$6) RETURNING id`,
          [req.auth!.companyId, reversal_date, `REVAL-REV-${run['period']}`, `Reversal: FX Revaluation ${run['period']}`, 'IQD', req.auth!.userId],
        )
        const jeId = jeRes.rows[0]!['id'] as string
