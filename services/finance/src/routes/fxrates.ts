...
fxRatesRouter.get('/', requirePermission('finance.fx_rates.view', 'view'), async (req, res) => {
  try {
    ...
    let idx = 1
    if (from_currency) { sql += ` AND from_currency = $${idx++}`; params.push(from_currency) }
    if (to_currency) { sql += ` AND to_currency = $${idx++}`; params.push(to_currency) }
    if (date) { sql += ` AND rate_date <= $${idx++}`; params.push(date) }
    sql += ' ORDER BY from_currency, to_currency, rate_date DESC'

    const result = await query(sql, params)
    sendOk(res, result.rows)
  } catch (err) { sendError(res, 500, 'INTERNAL_ERROR', 'Failed to fetch FX rates', err) }
})
...
fxRatesRouter.post('/', requirePermission('finance.fx_rates.edit', 'edit'), async (req, res) => {
  try {
    ...
    if (rate_date > today) return sendError(res, 400, 'FUTURE_DATE', 'Rate date cannot be in the future')
  ...
  } catch (err) { sendError(res, 500, 'INTERNAL_ERROR', 'Failed to upsert FX rate', err) }
})
