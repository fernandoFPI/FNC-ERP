...
const UpdateSchema = CreateSchema.omit({ vendor_id: true, po_id: true, lines: true }).partial().extend({ lines: z.array(LineSchema).optional() })
...
vendorInvoicesRouter.get('/', requirePermission('finance.ap.view', 'view'), async (req, res) => {
  try {
    ...
    const total = await query(
      `SELECT COUNT(*) FROM vendor_invoices vi ${where}`,
      values
    )
  ...
  } catch (err) { sendError(res, 500, 'INTERNAL_ERROR', 'Failed to list vendor invoices', err) }
})

// ── GET /finance/vendor-invoices/ap-summary ───────────────────
vendorInvoicesRouter.get('/ap-summary', requirePermission('finance.ap.view', 'view'), async (req, res) => {
  try {
  ...
  } catch (err) { sendError(res, 500, 'INTERNAL_ERROR', 'Failed to get AP summary', err) }
})

// ── GET /finance/vendor-invoices/vendor/:vendorId/summary ─────
vendorInvoicesRouter.get('/vendor/:vendorId/summary', requirePermission('finance.ap.view', 'view'), async (req, res) => {
  try {
    const result = await query(`
      SELECT
    ...
    if (!result.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Vendor not found')
    sendOk(res, result.rows[0])
  } catch (err) { sendError(res, 500, 'INTERNAL_ERROR', 'Failed to get vendor summary', err) }
})
...
vendorInvoicesRouter.get('/:id', requirePermission('finance.ap.view', 'view'), async (req, res) => {
  try {
    ...
    if (!invoiceResult.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Vendor invoice not found')
    sendOk(res, { ...invoiceResult.rows[0], lines: linesResult.rows, payments: paymentsResult.rows })
  } catch (err) { sendError(res, 500, 'INTERNAL_ERROR', 'Failed to get vendor invoice', err) }
})
...
vendorInvoicesRouter.post('/', requirePermission('finance.ap.edit', 'edit'), async (req, res) => {
  try {
    ...
    const { vendor_id, po_id, invoice_number, invoice_date, due_date, subtotal, tax_amount, currency_code, wht_applies: whtAppliesInput, wht_rate: whtRateInput, analytic_account_id, cost_center_id, notes, lines } = parsed.data

    // If wht_applies is not explicitly set, default based on vendor's configured rate.
    const vendorResult = await query(`SELECT withholding_tax_rate FROM vendors WHERE id = $1`, [vendor_id])
    ...
    const invoice = await withTransaction(
      ...
      async (client) => {
        const inv = await client.query(`
          ...
          RETURNING *
        `, [
          req.auth!.companyId, vendor_id, po_id ?? null, invoice_number, invoice_date, due_date,
          subtotal, tax_amount, whtAmount, totalAmount, netPayable,
          currency_code, whtApplies, whtRate,
          analytic_account_id ?? null, cost_center_id ?? null, notes ?? null, req.auth!.userId,
        ])
        ...
        if (lines?.length) {
          for (let i = 0; i < lines.length; i++) {
            ...
            `, [invoiceId, line.po_line_id ?? null, line.description, line.qty, line.unit_price, lineTotal, line.account_id ?? null, i + 1])
        ...
        await logAudit({
          ...
          action: 'CREATE', tableName: 'vendor_invoices', recordId: invoiceId,
        })
...
vendorInvoicesRouter.put('/:id', requirePermission('finance.ap.edit', 'edit'), async (req, res) => {
  try {
    const current = await query(`SELECT status, vendor_id FROM vendor_invoices WHERE id = $1 AND company_id = $2`, [req.params['id'], req.auth!.companyId])
    if (!current.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Vendor invoice not found')
    if (current.rows[0]!['status'] !== 'draft') return sendError(res, 400, 'NOT_EDITABLE', 'Only draft invoices can be edited')
    ...
    const vendorResult = await query(`SELECT withholding_tax_rate FROM vendors WHERE id = $1`, [current.rows[0]!['vendor_id']])
    const vendorRate = parseFloat(String(vendorResult.rows[0]?.['withholding_tax_rate'] ?? 0))
    ...
    `, [
      d.invoice_number ?? null, d.invoice_date ?? null, d.due_date ?? null,
      subtotalNum, taxNum, whtAmount, totalAmount, netPayable,
      d.currency_code ?? null, whtApplies, whtRate,
      d.analytic_account_id ?? null, d.cost_center_id ?? null, d.notes ?? null,
      req.params['id'], req.auth!.companyId,
    ])

    sendOk(res, { message: 'Invoice updated' })
  } catch (err) { sendError(res, 500, 'INTERNAL_ERROR', 'Failed to update vendor invoice', err) }
})

// ── POST /finance/vendor-invoices/:id/submit ──────────────────
vendorInvoicesRouter.post('/:id/submit', requirePermission('finance.ap.edit', 'edit'), async (req, res) => {
  try {
    const invoice = await query(`SELECT status FROM vendor_invoices WHERE id = $1 AND company_id = $2`, [req.params['id'], req.auth!.companyId])
    if (!invoice.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Vendor invoice not found')
    if (invoice.rows[0]!['status'] !== 'draft') return sendError(res, 400, 'INVALID_STATUS', 'Only draft invoices can be submitted')

    const lineCount = await query(`SELECT COUNT(*) FROM vendor_invoice_lines WHERE vendor_invoice_id = $1`, [req.params['id']])
    if (parseInt(lineCount.rows[0]!['count'] as string) === 0) {
      return sendError(res, 400, 'NO_LINES', 'Invoice must have at least one line before submission')
    }
    ...
    await withTransaction(
      ...
      async (client) => {
        ...
        await logAudit({ userId: req.auth!.userId, companyId: req.auth!.companyId, action: 'UPDATE', tableName: 'vendor_invoices', recordId: req.params['id']! })
      }
    )

    sendOk(res, { message: 'Invoice submitted for approval' })
  } catch (err) { sendError(res, 500, 'INTERNAL_ERROR', 'Failed to submit vendor invoice', err) }
})

// ── POST /finance/vendor-invoices/:id/approve ─────────────────
vendorInvoicesRouter.post('/:id/approve', requirePermission('finance.ap.approve', 'approve'), async (req, res) => {
  try {
    if (req.auth!.role !== 'system_admin') return sendError(res, 403, 'FORBIDDEN', 'System admin required to approve vendor invoices')

    const invoice = await query(`SELECT status, submitted_by, invoice_number FROM vendor_invoices WHERE id=$1 AND company_id=$2`, [req.params['id'], req.auth!.companyId])
    if (!invoice.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Vendor invoice not found')
    if (invoice.rows[0]!['status'] !== 'submitted') return sendError(res, 400, 'INVALID_STATUS', 'Only submitted invoices can be approved')
    ...
    await withTransaction(
      ...
      async (client) => {
        ...
        const invRow = await client.query(`SELECT * FROM vendor_invoices WHERE id=$1`, [req.params['id']])
        ...
        await client.query(
          ...
          [JSON.stringify({
            invoice_id: req.params['id'], company_id: req.auth!.companyId,
            vendor_id: apInv['vendor_id'], total_amount: apInv['total_amount'],
            currency_code: apInv['currency_code'], invoice_date: apInv['invoice_date'],
          })],
        )
        ...
        if (invoice.rows[0]!['submitted_by']) {
          ...
          `, [JSON.stringify({ userId: invoice.rows[0]!['submitted_by'], invoiceId: req.params['id'], invoiceNumber: invoice.rows[0]!['invoice_number'] })])
        }
  ...
  } catch (err) { sendError(res, 500, 'INTERNAL_ERROR', 'Failed to approve vendor invoice', err) }
})

// ── POST /finance/vendor-invoices/:id/reject ──────────────────
vendorInvoicesRouter.post('/:id/reject', requirePermission('finance.ap.approve', 'approve'), async (req, res) => {
  try {
    if (req.auth!.role !== 'system_admin') return sendError(res, 403, 'FORBIDDEN', 'System admin required')

    const { reason } = req.body as { reason?: string }
    if (!reason?.trim()) return sendError(res, 400, 'REASON_REQUIRED', 'Rejection reason is required')

    const invoice = await query(`SELECT status, submitted_by FROM vendor_invoices WHERE id=$1 AND company_id=$2`, [req.params['id'], req.auth!.companyId])
    if (!invoice.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Vendor invoice not found')
    if (invoice.rows[0]!['status'] !== 'submitted') return sendError(res, 400, 'INVALID_STATUS', 'Only submitted invoices can be rejected')
    ...
    await withTransaction(
      ...
      async (client) => {
        ...
        if (invoice.rows[0]!['submitted_by']) {
          ...
          `, [JSON.stringify({ userId: invoice.rows[0]!['submitted_by'], invoiceId: req.params['id'], reason })])
        }
  ...
  } catch (err) { sendError(res, 500, 'INTERNAL_ERROR', 'Failed to reject vendor invoice', err) }
})

// ── POST /finance/vendor-invoices/:id/cancel ──────────────────
vendorInvoicesRouter.post('/:id/cancel', requirePermission('finance.ap.approve', 'approve'), async (req, res) => {
  try {
    const { reason } = req.body as { reason?: string }
    ...
    if (!['draft','submitted'].includes(invoice.rows[0]!['status'] as string)) {
      return sendError(res, 400, 'CANNOT_CANCEL', 'Only draft or submitted invoices can be cancelled')
    }

    const invToCancel = await query(`SELECT invoice_number FROM vendor_invoices WHERE id=$1`, [req.params['id']])
    await query(`
      UPDATE vendor_invoices SET
        status='cancelled',
    ...
    await logAudit({ userId: req.auth!.userId, companyId: req.auth!.companyId, action: 'DELETE', tableName: 'vendor_invoices', recordId: req.params['id']! })
    const invNum = String(invToCancel.rows[0]?.['invoice_number'] ?? '')
    ;(async () => {
      const fu = await query(`SELECT DISTINCT u.id AS user_id FROM users u JOIN user_company_roles ucr ON ucr.user_id=u.id WHERE ucr.company_id=$1 AND (ucr.role IN ('company_admin','system_admin') OR (ucr.module='finance' AND ucr.role IN ('module_admin','module_user'))) AND u.is_active=true`, [req.auth!.companyId])
      for (const u of fu.rows) await query(`INSERT INTO service_outbox (service,event_type,payload) VALUES ('notifications','VENDOR_INVOICE_CANCELLED',$1)`, [JSON.stringify({ userId: u['user_id'], companyId: req.auth!.companyId, title: `Invoice cancelled: ${invNum}`, body: `Vendor invoice ${invNum} has been cancelled${reason ? ': ' + reason : ''}`, data: { invoiceId: req.params['id'], invoiceNumber: invNum } })])
    })().catch(() => {})
    sendOk(res, { message: 'Invoice cancelled' })
  } catch (err) { sendError(res, 500, 'INTERNAL_ERROR', 'Failed to cancel vendor invoice', err) }
})

// ── POST /finance/vendor-invoices/:id/payments ────────────────
vendorInvoicesRouter.post('/:id/payments', requirePermission('finance.ap.approve', 'approve'), async (req, res) => {
  try {
    const parsed = PaymentSchema.safeParse(req.body)
    if (!parsed.success) return sendError(res, 400, 'VALIDATION_ERROR', 'Invalid input', parsed.error.flatten())
    const { payment_date, amount, currency_code, payment_method, payment_reference, notes } = parsed.data

    const invoice = await query(`SELECT * FROM vendor_invoices WHERE id=$1 AND company_id=$2`, [req.params['id'], req.auth!.companyId])
    if (!invoice.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Vendor invoice not found')

    const inv = invoice.rows[0] as Record<string, unknown>
    if (!['approved','partially_paid'].includes(inv['status'] as string)) {
      return sendError(res, 400, 'NOT_PAYABLE', 'Invoice must be approved before recording payment')
    }

    const outstandingCents = Math.round(parseFloat(String(inv['net_payable'])) * 100) - Math.round(parseFloat(String(inv['amount_paid'])) * 100)
    const outstandingDisplay = outstandingCents / 100
    if (amount <= 0) return sendError(res, 400, 'INVALID_AMOUNT', 'Payment amount must be greater than zero')
    if (Math.round(amount * 100) > outstandingCents) return sendError(res, 400, 'OVERPAYMENT', `Payment exceeds outstanding balance of ${outstandingDisplay.toFixed(2)}`)
    ...
    const payment = await withTransaction(
      ...
      async (client) => {
        ...
        `, [
          req.auth!.companyId, req.params['id'], inv['vendor_id'],
          payment_date, amount, effectiveCurrency,
          payment_method, payment_reference ?? null, notes ?? null, req.auth!.userId,
        ])

        const newAmountPaid = parseFloat(String(inv['amount_paid'])) + amount
        const isFullyPaid = Math.round(newAmountPaid * 100) >= Math.round(parseFloat(String(inv['net_payable'])) * 100)
        ...
        await logAudit({ userId: req.auth!.userId, companyId: req.auth!.companyId, action: 'CREATE', tableName: 'vendor_payments', recordId: pmt.rows[0]!['id'] as string })

        // Notify finance team that a payment was recorded
        const invNumberRow = await client.query<{ invoice_number: string }>(`SELECT invoice_number FROM vendor_invoices WHERE id=$1`, [req.params['id']])
        const invNum = String(invNumberRow.rows[0]?.invoice_number ?? '')
        const fuPmt = await client.query(`SELECT DISTINCT u.id AS user_id FROM users u JOIN user_company_roles ucr ON ucr.user_id=u.id WHERE ucr.company_id=$1 AND (ucr.role IN ('company_admin','system_admin') OR (ucr.module='finance' AND ucr.role IN ('module_admin','module_user'))) AND u.is_active=true`, [req.auth!.companyId])
        for (const u of fuPmt.rows) {
          await client.query(`INSERT INTO service_outbox (service,event_type,payload) VALUES ('notifications','VENDOR_PAYMENT_RECORDED',$1)`, [JSON.stringify({ userId: u['user_id'], companyId: req.auth!.companyId, title: `Payment recorded: ${invNum}`, body: `Payment of ${amount} ${effectiveCurrency} recorded for invoice ${invNum}${isFullyPaid ? ' — fully paid' : ''}`, data: { invoiceId: req.params['id'], invoiceNumber: invNum, amount, isFullyPaid } })])
        }

        return { ...pmt.rows[0], newStatus, isFullyPaid, outstanding: outstandingDisplay - amount }
      }
    )

    sendOk(res, payment, 201)
  } catch (err) { sendError(res, 500, 'INTERNAL_ERROR', 'Failed to record vendor payment', err) }
})
