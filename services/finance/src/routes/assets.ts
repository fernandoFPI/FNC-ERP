...
assetsRouter.get('/', requirePermission('finance.assets.view', 'view'), async (req, res) => {
  try {
    ...
    if (status) {
      params.push(status)
      where += ` AND fa.status=$${params.length}`
    }
    if (category_id) {
      params.push(category_id)
      where += ` AND fa.category_id=$${params.length}`
    }
    if (search) {
      params.push(`%${search}%`)
      where += ` AND (fa.name ILIKE $${params.length} OR fa.asset_number ILIKE $${params.length})`
    }
  ...
  try {
    ...
    const r = await query(
      ...
      [
        ...
        d.declining_rate ?? null,
        d.asset_account_id ?? null,
        d.accum_dep_account_id ?? null,
...
assetsRouter.patch('/:id', requirePermission('finance.assets.edit', 'edit'), async (req, res) => {
  try {
    ...
    if (asset.rows[0]['status'] !== 'draft') {
      sendError(
        res,
        400,
        'INVALID_STATE',
        'Only draft assets can be edited. Dispose and recreate to change an active asset.',
      )
      return
    }
    ...
    const map: Record<string, unknown> = {
      ...
      asset_account_id: d.asset_account_id,
      accum_dep_account_id: d.accum_dep_account_id,
      dep_expense_account_id: d.dep_expense_account_id,
    ...
    if (d.purchase_cost !== undefined) {
      fields.push(`book_value=$${idx++}`)
      params.push(d.purchase_cost)
    }
    if (!fields.length) {
      sendError(res, 400, 'NO_FIELDS', 'No fields to update')
      return
    }
  ...
  async (req, res) => {
    try {
      ...
      if (asset['status'] !== 'draft') {
        sendError(res, 400, 'INVALID_STATE', 'Asset is already active or disposed')
        return
      }

      const activationDate = new Date(
        (req.body.activation_date as string | undefined) ?? (asset['purchase_date'] as string),
      )
      ...
      await logAudit({
        ...
        newValues: {
          ...
          schedule_lines: schedule.length,
        },
      })
    ...
    try {
      ...
      if (Number(pendingCheck.rows[0]!['cnt']) === 0) {
        sendOk(res, {
          posted: 0,
          period,
          message: 'No pending depreciation lines for this period',
        })
        return
      }
      ...
      const posted = await withTransaction(
        ...
        async (client) => {
          ...
          for (const line of lines.rows) {
            ...
            if (depExpAccount && accumDepAccount) {
              ...
              await client.query(
                ...
                [
                  journalEntryId,
                  depExpAccount,
                  `Depreciation: ${line.asset_name as string}`,
                  depAmount,
                ],
              )
              await client.query(
                ...
                [
                  journalEntryId,
                  accumDepAccount,
                  `Accumulated Dep: ${line.asset_name as string}`,
                  depAmount,
                ],
              )
            }
            ...
            const nextStatus =
              newBook <= Number(line.salvage_value) ? 'fully_depreciated' : 'active'
            await client.query(
              `UPDATE fixed_assets SET accumulated_depreciation=$1, book_value=$2, status=$3, updated_at=NOW() WHERE id=$4`,
              [newAccum, Math.max(newBook, 0), nextStatus, line.asset_id],
            )
    ...
    try {
      ...
      const updated = await withTransaction(
        ...
        async (client) => {
          if (asset['asset_account_id'] && asset['accum_dep_account_id']) {
            ...
            await client.query(
              ...
              [
                ...
                `Clear accum dep: ${asset['name'] as string}`,
                accumDep,
              ],
            )
            await client.query(
              ...
              [
                ...
                `Dispose asset: ${asset['name'] as string}`,
                purchaseCost,
              ],
            )
            if (proceeds > 0 && d.cash_account_id) {
              await client.query(
                ...
                [
                  jeId,
                  d.cash_account_id,
                  `Disposal proceeds: ${asset['name'] as string}`,
                  proceeds,
                ],
              )
            }
              ...
              if (gainLoss > 0) {
                await client.query(
                  ...
                  [
                    ...
                    `Gain on disposal: ${asset['name'] as string}`,
                    gainLoss,
                  ],
                )
              } else {
                await client.query(
                  ...
                  [
                    jeId,
                    d.gain_loss_account_id,
                    `Loss on disposal: ${asset['name'] as string}`,
                    Math.abs(gainLoss),
                  ],
                )
              }
          ...
          await client.query(
            `UPDATE asset_depreciation_schedule SET status='skipped' WHERE asset_id=$1 AND status='pending'`,
            [asset['id']],
          )
      ...
      await logAudit({
        ...
        tableName: 'fixed_assets',
        recordId: asset['id'] as string,
        newValues: { status: 'disposed', disposal_date: d.disposal_date, gain_loss: gainLoss },
      })
      sendOk(res, updated)
    } catch (err) {
