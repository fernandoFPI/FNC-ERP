...
paymentVouchersRouter.get(
  ...
  async (req, res) => {
    try {
      ...
      if (!pv.rows[0]) {
        sendError(res, 404, 'NOT_FOUND', 'Payment voucher not found')
        return
      }
  ...
  async (req, res) => {
    ...
    try {
      ...
      await logAudit({
        ...
        recordId: pv.id,
        newValues: { voucher_number, voucher_date, total_iqd, total_usd },
        client,
      })
  ...
  async (req, res) => {
    ...
    try {
      ...
      const pvCheck = await client.query(
        `SELECT id, status FROM payment_vouchers WHERE id=$1 AND company_id=$2 FOR UPDATE`,
        [pvId, companyId],
      )
      ...
      if (pv.status !== 'draft') {
        await client.query('ROLLBACK')
        sendError(res, 422, 'NOT_DRAFT', 'Only draft vouchers can be edited')
        return
      }
      ...
      if (d.journal_ids !== undefined) {
        await client.query(`DELETE FROM payment_voucher_journals WHERE payment_voucher_id=$1`, [
          pvId,
        ])
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (pvRow.status !== 'draft') {
        sendError(res, 422, 'INVALID_STATUS', 'Only draft vouchers can be approved')
        return
      }
      const r = await query(
      ...
      if (pvRow.created_by) {
        query(
          `INSERT INTO service_outbox (service,event_type,payload) VALUES ('notifications','PAYMENT_VOUCHER_APPROVED',$1)`,
          [
            JSON.stringify({
              userId: pvRow.created_by,
              companyId,
              title: `Payment voucher approved: ${pvRow.voucher_number}`,
              body: `Payment voucher ${pvRow.voucher_number} has been approved by the auditor and is ready for payment`,
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (pvRow2.status !== 'approved') {
        sendError(res, 422, 'INVALID_STATUS', 'Voucher must be approved before marking as paid')
        return
      }
      const r = await query(
      ...
      for (const userId of [pvRow2.created_by, pvRow2.audited_by].filter(Boolean) as string[]) {
        query(
          `INSERT INTO service_outbox (service,event_type,payload) VALUES ('notifications','PAYMENT_VOUCHER_PAID',$1)`,
          [
            JSON.stringify({
              userId,
              companyId,
              title: `Payment voucher paid: ${pvRow2.voucher_number}`,
              body: `Payment voucher ${pvRow2.voucher_number} has been marked as paid by the cashier`,
