...
retentionRouter.get('/summary', requirePermission('finance.retention.view', 'view'), async (req, res) => {
  try {
    const r = await query(
      `SELECT
...
retentionRouter.get('/aging', requirePermission('finance.retention.view', 'view'), async (req, res) => {
  try {
    const r = await query(
      `SELECT
         retention_type,
...
retentionRouter.get('/', requirePermission('finance.retention.view', 'view'), async (req, res) => {
  ...
  try {
    ...
    let i = 2
    if (type) { conditions.push(`retention_type = $${i++}`); vals.push(type) }
    if (status) { conditions.push(`status = $${i++}`); vals.push(status) }
    if (project_id) { conditions.push(`project_id = $${i++}`); vals.push(project_id) }
...
retentionRouter.get('/:id', requirePermission('finance.retention.view', 'view'), async (req, res) => {
  try {
    ...
    if (!rr.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Retention record not found')
...
retentionRouter.post('/', requirePermission('finance.retention.edit', 'edit'), async (req, res) => {
  const schema = z.object({
    ...
    expected_release_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(),
    ...
    const result = await withTransaction(
      ...
      async (client) => {
        const r = await client.query(
          ...
          [
            req.auth!.companyId, recordNumber, d.retention_type,
            d.source_ref ?? null, d.project_id ?? null, d.project_name ?? null,
            d.counterparty_name, d.invoice_amount, d.retention_rate, retentionAmount,
            d.invoice_date, d.expected_release_date ?? null,
            d.retention_account_id ?? null, d.offset_account_id ?? null,
        ...
        if (d.post_journal_entry && d.retention_account_id && d.offset_account_id) {
          ...
          if (d.retention_type === 'ar') {
            ...
            await client.query(`INSERT INTO journal_lines (journal_entry_id,account_id,description,debit,credit) VALUES ($1,$2,$3,$4,0)`, [jeId, d.retention_account_id, desc, retentionAmount])
            await client.query(`INSERT INTO journal_lines (journal_entry_id,account_id,description,debit,credit) VALUES ($1,$2,$3,0,$4)`, [jeId, d.offset_account_id, desc, retentionAmount])
          } else {
            // DR Accounts Payable / CR Retention Payable
            await client.query(`INSERT INTO journal_lines (journal_entry_id,account_id,description,debit,credit) VALUES ($1,$2,$3,$4,0)`, [jeId, d.offset_account_id, desc, retentionAmount])
            await client.query(`INSERT INTO journal_lines (journal_entry_id,account_id,description,debit,credit) VALUES ($1,$2,$3,0,$4)`, [jeId, d.retention_account_id, desc, retentionAmount])
          }
        }
        return record
      },
    )
    await logAudit({ companyId: req.auth!.companyId, userId: req.auth!.userId, action: 'INSERT', tableName: 'retention_records', recordId: result['id'] as string, newValues: { record_number: recordNumber, type: d.retention_type, amount: retentionAmount } })
...
retentionRouter.post('/:id/release', requirePermission('finance.retention.edit', 'edit'), async (req, res) => {
  const schema = z.object({
  ...
  try {
    ...
    const recRes = await query(
      `SELECT * FROM retention_records WHERE id=$1 AND company_id=$2`,
      [req.params['id'], req.auth!.companyId],
    )
    const rec = recRes.rows[0]
    if (!rec) return sendError(res, 404, 'NOT_FOUND', 'Retention record not found')
    if (rec['status'] === 'released') return sendError(res, 400, 'ALREADY_RELEASED', 'Retention is already fully released')

    const outstanding = round2(Number(rec['retention_amount']) - Number(rec['released_amount']))
    if (d.amount > outstanding + 0.01) {
      return sendError(res, 400, 'EXCESS_AMOUNT', `Release amount (${d.amount}) exceeds outstanding retention (${outstanding})`)
    }

    const newReleased = round2(Number(rec['released_amount']) + d.amount)
    const newStatus   = newReleased >= Number(rec['retention_amount']) - 0.01 ? 'released' : 'partially_released'
      ...
      async (client) => {
        ...
        if (retAcctId && offsetAcctId) {
          ...
          const jeRes = await client.query(
            ...
            [req.auth!.companyId, `${rec['record_number'] as string}-REL`, desc, d.release_date, req.auth!.userId],
          )
          ...
          if (rec['retention_type'] === 'ar') {
            ...
            await client.query(`INSERT INTO journal_lines (journal_entry_id,account_id,description,debit,credit) VALUES ($1,$2,$3,$4,0)`, [journalEntryId, offsetAcctId, desc, d.amount])
            await client.query(`INSERT INTO journal_lines (journal_entry_id,account_id,description,debit,credit) VALUES ($1,$2,$3,0,$4)`, [journalEntryId, retAcctId, desc, d.amount])
          } else {
            // Release AP retention: DR Retention Payable / CR AP (offset)
            await client.query(`INSERT INTO journal_lines (journal_entry_id,account_id,description,debit,credit) VALUES ($1,$2,$3,$4,0)`, [journalEntryId, retAcctId, desc, d.amount])
            await client.query(`INSERT INTO journal_lines (journal_entry_id,account_id,description,debit,credit) VALUES ($1,$2,$3,0,$4)`, [journalEntryId, offsetAcctId, desc, d.amount])
          }
        }
        ...
        await client.query(
          ...
          [req.params['id'], req.auth!.companyId, d.release_date, d.amount, journalEntryId, d.notes ?? null, req.auth!.userId],
        )
    ...
    await logAudit({ companyId: req.auth!.companyId, userId: req.auth!.userId, action: 'UPDATE', tableName: 'retention_records', recordId: req.params['id'], newValues: { released: d.amount, status: newStatus } })
    sendOk(res, result)
  } catch (err) {
...
retentionRouter.patch('/:id', requirePermission('finance.retention.edit', 'edit'), async (req, res) => {
  const schema = z.object({
    expected_release_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(),
    notes: z.string().optional(),
  ...
  try {
    ...
    const vals: unknown[] = []
    let i = 1
    if (d.expected_release_date !== undefined) { sets.push(`expected_release_date=$${i++}`); vals.push(d.expected_release_date) }
    if (d.notes !== undefined) { sets.push(`notes=$${i++}`); vals.push(d.notes) }
    if (d.retention_account_id !== undefined) { sets.push(`retention_account_id=$${i++}`); vals.push(d.retention_account_id) }
    if (d.offset_account_id !== undefined) { sets.push(`offset_account_id=$${i++}`); vals.push(d.offset_account_id) }
    ...
    if (!r.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Retention record not found')
    sendOk(res, r.rows[0])
  } catch (err) {
