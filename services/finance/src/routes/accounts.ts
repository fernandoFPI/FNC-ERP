...
accountsRouter.get('/', requirePermission('finance.accounts.view', 'view'), async (req, res) => {
  try {
    ...
    if (type) { sql += ` AND account_type = $${idx++}`; params.push(type) }
...
accountsRouter.get('/:id', requirePermission('finance.accounts.view', 'view'), async (req, res) => {
  try {
    ...
    if (!row) return sendError(res, 404, 'NOT_FOUND', 'Account not found')
    sendOk(res, row)
  } catch (err) {
...
accountsRouter.post('/', requirePermission('finance.accounts.edit', 'edit'), async (req, res) => {
  try {
    ...
    if (parent_id) {
      const parent = await query('SELECT id FROM chart_of_accounts WHERE id = $1 AND company_id = $2', [parent_id, companyId])
      if (!parent.rows[0]) return sendError(res, 400, 'INVALID_PARENT', 'Parent account not found in this company')
    }
    ...
    await logAudit({ companyId, userId: req.auth!.userId, action: 'CREATE', tableName: 'chart_of_accounts', recordId: account['id'] as string, newValues: account })
    sendOk(res, account, 201)
  } catch (err: unknown) {
    const e = err as { code?: string }
    if (e.code === '23505') return sendError(res, 409, 'DUPLICATE_CODE', 'Account code already exists for this company')
...
accountsRouter.put('/:id', requirePermission('finance.accounts.edit', 'edit'), async (req, res) => {
  try {
    ...
    const existing = await query('SELECT * FROM chart_of_accounts WHERE id = $1 AND company_id = $2', [req.params['id'], companyId])
    const acct = existing.rows[0]
    if (!acct) return sendError(res, 404, 'NOT_FOUND', 'Account not found')
    ...
    if (account_type !== undefined) { updates.push(`account_type = $${idx++}`); params.push(account_type) }
    if (parent_id !== undefined) { updates.push(`parent_id = $${idx++}`); params.push(parent_id) }
    if (currency_code !== undefined) { updates.push(`currency_code = $${idx++}`); params.push(currency_code) }
    if (is_reconcilable !== undefined) { updates.push(`is_reconcilable = $${idx++}`); params.push(is_reconcilable) }
    ...
    await logAudit({ companyId, userId: req.auth!.userId, action: 'UPDATE', tableName: 'chart_of_accounts', recordId: req.params['id']!, newValues: parsed.data })
...
accountsRouter.delete('/:id', requirePermission('finance.accounts.admin', 'admin'), async (req, res) => {
  try {
    const companyId = req.auth!.companyId
    const lines = await query('SELECT COUNT(*) FROM journal_lines jl JOIN journal_entries je ON je.id = jl.journal_entry_id WHERE jl.account_id = $1 AND je.company_id = $2', [req.params['id'], companyId])
    if (parseInt((lines.rows[0] as { count: string }).count) > 0) {
      return sendError(res, 409, 'HAS_JOURNAL_LINES', 'Cannot delete account with journal lines')
    }
    await query('UPDATE chart_of_accounts SET is_active = false, updated_at = NOW() WHERE id = $1 AND company_id = $2', [req.params['id'], companyId])
    await logAudit({ companyId, userId: req.auth!.userId, action: 'DELETE', tableName: 'chart_of_accounts', recordId: req.params['id']! })
...
// Account ledger
accountsRouter.get('/:id/ledger', requirePermission('finance.accounts.view', 'view'), async (req, res) => {
  try {
    const companyId = req.auth!.companyId
