...
bankRouter.post('/accounts', requirePermission('finance.bank.edit', 'edit'), async (req, res) => {
  ...
  try {
    ...
    const r = await query(
      ...
      [req.auth!.companyId, d.name, d.account_number ?? null, d.bank_name ?? null, d.branch ?? null,
       d.swift_code ?? null, d.iban ?? null, d.currency_code, d.gl_account_id ?? null, d.opening_balance, d.notes ?? null],
    )
    await logAudit({ companyId: req.auth!.companyId, userId: req.auth!.userId, action: 'INSERT', tableName: 'recon_bank_accounts', recordId: r.rows[0]!['id'] as string, newValues: d })
    sendOk(res, r.rows[0], 201)
  } catch (err) {
    if ((err as { code?: string }).code === '23505') return sendError(res, 409, 'DUPLICATE', 'Bank account name already exists')
    sendError(res, 500, 'INTERNAL_ERROR', 'Failed to create bank account', err)
  }
})

bankRouter.get('/accounts/:id', requirePermission('finance.bank.view', 'view'), async (req, res) => {
  try {
    const r = await query(
    ...
    if (!r.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Bank account not found')
    sendOk(res, r.rows[0])
  } catch (err) {
...
bankRouter.put('/accounts/:id', requirePermission('finance.bank.edit', 'edit'), async (req, res) => {
  const schema = z.object({
  ...
  try {
    ...
    if (!sets.length) return sendError(res, 400, 'NO_CHANGES', 'Nothing to update')
...
bankRouter.get('/accounts/:accountId/statements', requirePermission('finance.bank.view', 'view'), async (req, res) => {
  try {
    const r = await query(
      `SELECT bs.*,
...
bankRouter.post('/accounts/:accountId/statements', requirePermission('finance.bank.edit', 'edit'), async (req, res) => {
  const schema = z.object({
  ...
  try {
    const d = schema.parse(req.body)
    const acct = await query(`SELECT id FROM recon_bank_accounts WHERE id=$1 AND company_id=$2`, [req.params['accountId'], req.auth!.companyId])
    if (!acct.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Bank account not found')
    const r = await query(
      ...
      [req.auth!.companyId, req.params['accountId'], d.period, d.statement_date, d.opening_balance, d.closing_balance, d.notes ?? null, req.auth!.userId],
    )
    sendOk(res, r.rows[0], 201)
  } catch (err) {
    if ((err as { code?: string }).code === '23505') return sendError(res, 409, 'DUPLICATE', 'Statement for this period already exists')
    sendError(res, 500, 'INTERNAL_ERROR', 'Failed to create statement', err)
  }
})

// Get statement with lines and match status
bankRouter.get('/statements/:id', requirePermission('finance.bank.view', 'view'), async (req, res) => {
  try {
    ...
    if (!stmt) return sendError(res, 404, 'NOT_FOUND', 'Statement not found')
...
bankRouter.post('/statements/:id/lines', requirePermission('finance.bank.edit', 'edit'), async (req, res) => {
  ...
  try {
    const stmtRes = await query(`SELECT id, status FROM recon_statements WHERE id=$1 AND company_id=$2`, [req.params['id'], req.auth!.companyId])
    const stmt = stmtRes.rows[0]
    if (!stmt) return sendError(res, 404, 'NOT_FOUND', 'Statement not found')
    if (stmt['status'] === 'reconciled') return sendError(res, 400, 'RECONCILED', 'Cannot add lines to a reconciled statement')

    const { lines } = schema.parse(req.body)
    const maxRes = await query(`SELECT COALESCE(MAX(line_number), 0) AS max FROM recon_lines WHERE statement_id=$1`, [req.params['id']])
      ...
      async (client) => {
        ...
        for (const line of lines) {
          ...
          const r = await client.query(
            ...
            [req.params['id'], req.auth!.companyId, lineNum, line.transaction_date, line.value_date ?? null,
             line.description, line.reference ?? null, line.debit, line.credit, line.balance_after ?? null],
          )
          rows.push(r.rows[0])
        }
        await client.query(`UPDATE recon_statements SET status='in_progress', updated_at=NOW() WHERE id=$1 AND status='draft'`, [req.params['id']])
        return rows
      },
    )
...
bankRouter.delete('/lines/:id', requirePermission('finance.bank.edit', 'edit'), async (req, res) => {
  try {
    const r = await query(`SELECT is_reconciled FROM recon_lines WHERE id=$1 AND company_id=$2`, [req.params['id'], req.auth!.companyId])
    if (!r.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Line not found')
    if (r.rows[0]['is_reconciled']) return sendError(res, 400, 'MATCHED', 'Cannot delete a matched line — unmatch it first')
...
bankRouter.get('/statements/:id/gl-entries', requirePermission('finance.bank.view', 'view'), async (req, res) => {
  try {
    const stmtRes = await query(
    ...
    if (!stmtRes.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Statement not found')

    const { period, gl_account_id } = stmtRes.rows[0] as { period: string; gl_account_id: string | null }
    if (!gl_account_id) return sendOk(res, [])
...
bankRouter.post('/statements/:id/auto-match', requirePermission('finance.bank.edit', 'edit'), async (req, res) => {
  try {
    const stmtRes = await query(
      `SELECT bs.period, ba.gl_account_id
       FROM recon_statements bs
    ...
    const { period, gl_account_id } = stmtRes.rows[0] as { period: string; gl_account_id: string | null }
    if (!gl_account_id) return sendError(res, 400, 'NO_GL_ACCOUNT', 'Bank account has no GL account linked')
      ...
      async (client) => {
        for (const bLine of bankLines.rows) {
          ...
          const glMatch = glLines.rows.find(gl => {
            ...
            const daysDiff  = Math.abs((new Date(gl['entry_date'] as string).getTime() - bDate.getTime()) / 86400000)
          ...
          if (glMatch) {
            ...
            await client.query(
              ...
              [req.auth!.companyId, bLine['id'], glMatch['id'], glMatch['journal_entry_id'], req.auth!.userId],
            )
            await client.query(`UPDATE recon_lines SET is_reconciled=true, reconciled_at=NOW() WHERE id=$1`, [bLine['id']])
            matched++
          }
        }
      },
...
bankRouter.post('/match', requirePermission('finance.bank.edit', 'edit'), async (req, res) => {
  ...
  try {
    ...
    if (Number(lineCheck.rows[0]!['n']) !== d.statement_line_ids.length) {
      return sendError(res, 400, 'INVALID_LINES', 'One or more bank lines are invalid or already matched')
    }
      ...
      async (client) => {
        ...
        for (const lineId of d.statement_line_ids) {
          for (const jlId of d.journal_line_ids) {
            const jlRes = await client.query(`SELECT journal_entry_id FROM journal_lines WHERE id=$1`, [jlId])
          ...
          await client.query(`UPDATE recon_lines SET is_reconciled=true, reconciled_at=NOW() WHERE id=$1`, [lineId])
        }
        return matches
      },
    )
...
// Unmatch a bank line
bankRouter.delete('/match/line/:statementLineId', requirePermission('finance.bank.edit', 'edit'), async (req, res) => {
  try {
    const r = await query(`SELECT is_reconciled FROM recon_lines WHERE id=$1 AND company_id=$2`, [req.params['statementLineId'], req.auth!.companyId])
    if (!r.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Line not found')
    await withTransaction(
      { companyId: req.auth!.companyId, userId: req.auth!.userId, role: req.auth!.role },
      async (client) => {
        await client.query(`DELETE FROM recon_matches WHERE statement_line_id=$1`, [req.params['statementLineId']])
        await client.query(`UPDATE recon_lines SET is_reconciled=false, reconciled_at=NULL WHERE id=$1`, [req.params['statementLineId']])
      },
    )
...
bankRouter.post('/lines/:id/create-entry', requirePermission('finance.bank.edit', 'edit'), async (req, res) => {
  const schema = z.object({
  ...
  try {
    ...
    if (!line) return sendError(res, 404, 'NOT_FOUND', 'Line not found or already matched')
    if (!line['gl_account_id']) return sendError(res, 400, 'NO_GL_ACCOUNT', 'Bank account has no GL account configured')
    ...
    const result = await withTransaction(
      ...
      async (client) => {
        const jeRes = await client.query(
          ...
          [req.auth!.companyId, `BANK-${(line['transaction_date'] as string).slice(0, 7)}`.slice(0, 50), desc, line['transaction_date'], req.auth!.userId],
        )
        const jeId = jeRes.rows[0]!['id'] as string

        if (isCredit) {
          // Money IN: DR bank, CR offset
          await client.query(`INSERT INTO journal_lines (journal_entry_id,account_id,description,debit,credit) VALUES ($1,$2,$3,$4,0)`, [jeId, bankGlId, desc, amount])
          await client.query(`INSERT INTO journal_lines (journal_entry_id,account_id,description,debit,credit) VALUES ($1,$2,$3,0,$4)`, [jeId, d.offset_account_id, desc, amount])
        } else {
          // Money OUT: DR offset, CR bank
          await client.query(`INSERT INTO journal_lines (journal_entry_id,account_id,description,debit,credit) VALUES ($1,$2,$3,$4,0)`, [jeId, d.offset_account_id, desc, amount])
          await client.query(`INSERT INTO journal_lines (journal_entry_id,account_id,description,debit,credit) VALUES ($1,$2,$3,0,$4)`, [jeId, bankGlId, desc, amount])
        }
        ...
        await client.query(`UPDATE recon_lines SET is_reconciled=true, reconciled_at=NOW() WHERE id=$1`, [req.params['id']])
...
bankRouter.post('/statements/:id/finalize', requirePermission('finance.bank.edit', 'edit'), async (req, res) => {
  try {
    const stmtRes = await query(
      `SELECT bs.*,
              COUNT(bsl.id) AS total_lines,
    ...
    if (!s) return sendError(res, 404, 'NOT_FOUND', 'Statement not found')
    if (s['status'] === 'reconciled') return sendError(res, 400, 'ALREADY_RECONCILED', 'Statement already reconciled')

    const total   = Number(s['total_lines'])
    const matched = Number(s['matched_lines'])
    if (total > 0 && matched < total) {
      return sendError(res, 400, 'UNMATCHED_LINES', `${total - matched} unmatched line(s) remaining — match or delete them first`)
    }
    ...
    await query(
      ...
      [r.rows[0]!['statement_date'], r.rows[0]!['closing_balance'], r.rows[0]!['bank_account_id']],
    )
    sendOk(res, r.rows[0])
  } catch (err) {
