...
roleTemplatesRouter.get('/', requireAuth(), requirePermission('admin.roles.view', 'view'), async (req, res) => {
  try {
    ...
    const templatesResult = await query<{ [k: string]: unknown }>(
      `SELECT id, name, description, is_system, created_at, updated_at
       FROM role_templates
       ORDER BY is_system DESC, name ASC`,
    )

    const permissionsResult = await query<{ [k: string]: string }>(
      `SELECT template_id, permission_key, access_level FROM role_template_permissions`,
    )

    const permsByTemplate: Record<string, Array<{ key: string; accessLevel: string }>> = {}
...
roleTemplatesRouter.get('/:id', requireAuth(), requirePermission('admin.roles.view', 'view'), async (req, res) => {
  try {
    const isAdmin = req.auth!.role === 'system_admin' || req.auth!.role === 'company_admin'
    if (!isAdmin) {
      return res.status(403).json({ success: false, error: { code: 'FORBIDDEN' } })
    ...
    const tmplResult = await query<{ [k: string]: unknown }>(
      `SELECT id, name, description, is_system, created_at, updated_at
    ...
    const permsResult = await query<{ [k: string]: string }>(
...
roleTemplatesRouter.post('/', requireAuth(), requirePermission('admin.roles.admin', 'admin'), async (req, res) => {
  try {
    ...
    await withSystemTransaction(async (client) => {
      const result = await client.query<{ [k: string]: unknown }>(
...
roleTemplatesRouter.put('/:id', requireAuth(), requirePermission('admin.roles.admin', 'admin'), async (req, res) => {
  try {
    if (req.auth!.role !== 'system_admin') {
      return res.status(403).json({ success: false, error: { code: 'FORBIDDEN' } })
    }
    ...
    await withSystemTransaction(async (client) => {
      ...
      if (permissions !== undefined) {
        await client.query(
          `DELETE FROM role_template_permissions WHERE template_id = $1`,
          [tmplId],
        )
        for (const perm of permissions) {
          await client.query(
...
roleTemplatesRouter.delete('/:id', requireAuth(), requirePermission('admin.roles.admin', 'admin'), async (req, res) => {
  try {
    if (req.auth!.role !== 'system_admin') {
      return res.status(403).json({ success: false, error: { code: 'FORBIDDEN' } })
    }
    ...
    const tmpl = await query<{ [k: string]: unknown }>(
