...
locationsRouter.get('/', requirePermission('inventory.locations.view', 'view'), async (req, res) => {
  try {
...
locationsRouter.post('/', requirePermission('inventory.locations.admin', 'admin'), async (req, res) => {
  try {
    ...
    await logAudit({ companyId, userId: req.auth!.userId, action: 'CREATE', tableName: 'stock_locations', recordId: result.rows[0]!['id'] as string })
    sendOk(res, result.rows[0], 201)
  } catch (err: unknown) {
    const e = err as { code?: string }
    if (e.code === '23505') return sendError(res, 409, 'DUPLICATE_CODE', 'Location code already exists')
    sendError(res, 500, 'INTERNAL_ERROR', 'Failed to create location', err)
  }
})

locationsRouter.put('/:id', requirePermission('inventory.locations.admin', 'admin'), async (req, res) => {
  try {
    const companyId = req.auth!.companyId
    ...
    if (d.type !== undefined) { updates.push(`type = $${idx++}`); params.push(d.type) }
    if (d.address !== undefined) { updates.push(`address = $${idx++}`); params.push(d.address) }
...
locationsRouter.delete('/:id', requirePermission('inventory.locations.admin', 'admin'), async (req, res) => {
  try {
    const companyId = req.auth!.companyId
    await query('UPDATE stock_locations SET is_active = false WHERE id = $1 AND company_id = $2', [req.params['id'], companyId])
    await logAudit({ companyId, userId: req.auth!.userId, action: 'DELETE', tableName: 'stock_locations', recordId: req.params['id']! })
