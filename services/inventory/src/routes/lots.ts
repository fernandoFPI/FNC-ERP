...
lotsRouter.get('/', requirePermission('inventory.lots.view', 'view'), async (req, res) => {
  try {
    ...
    if (product_id) { sql += ` AND sl.product_id = $2`; params.push(product_id) }
    sql += ' ORDER BY sl.lot_number'
    const result = await query(sql, params)
    sendOk(res, result.rows)
  } catch (err) { sendError(res, 500, 'INTERNAL_ERROR', 'Failed to fetch lots', err) }
})
...
lotsRouter.post('/', requirePermission('inventory.lots.edit', 'edit'), async (req, res) => {
  try {
    ...
    const prod = await query('SELECT id FROM products WHERE id = $1 AND company_id = $2', [product_id, companyId])
    if (!prod.rows[0]) return sendError(res, 400, 'INVALID_PRODUCT', 'Product not found in this company')
    ...
    await logAudit({ companyId, userId: req.auth!.userId, action: 'CREATE', tableName: 'stock_lots', recordId: result.rows[0]!['id'] as string })
    sendOk(res, result.rows[0], 201)
  } catch (err: unknown) {
    const e = err as { code?: string }
    if (e.code === '23505') return sendError(res, 409, 'DUPLICATE_LOT', 'Lot number already exists for this product')
