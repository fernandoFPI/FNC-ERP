...
stockTransfersRouter.get(
  '/pricing-settings/:companyId',
  requirePermission('interco.stock_transfers.view', 'view'),
  async (req, res) => {
    try {
      ...
      if (role !== 'system_admin' && companyId !== authCompanyId) {
        sendError(res, 403, 'FORBIDDEN', 'You can only view your own company settings')
        return
      }
...
stockTransfersRouter.put(
  '/pricing-settings/:companyId',
  requirePermission('interco.stock_transfers.edit', 'edit'),
  async (req, res) => {
    try {
      const companyId = req.params['companyId']!
      const authCompanyId = req.auth!.companyId
      ...
      if (role !== 'system_admin' && companyId !== authCompanyId) {
        sendError(res, 403, 'FORBIDDEN', 'You can only change your own company settings')
        return
      }
      ...
      if (method === 'cost_plus' && (!cost_plus_markup_pct || cost_plus_markup_pct <= 0)) {
        sendError(
          ...
          400,
          'VALIDATION_ERROR',
          'cost_plus_markup_pct must be > 0 when method is cost_plus',
        )
        return
      }
    ...
    try {
      ...
      if (!product_id || !from_company_id || !from_location_id || !qty) {
        sendError(
          ...
          400,
          'VALIDATION_ERROR',
          'product_id, from_company_id, from_location_id, qty are required',
        )
        return
      }
  ...
  async (req, res) => {
    try {
      ...
      if (from_company_id) {
        ...
        params.push(from_company_id)
      }
      if (to_company_id) {
        sql += ` AND ist.to_company_id = $${idx++}`
        params.push(to_company_id)
      }
      if (status) {
...
stockTransfersRouter.get(
  ...
  async (req, res) => {
    try {
      ...
      if (!transfer.rows[0]) {
        sendError(res, 404, 'NOT_FOUND', 'Transfer not found')
        return
      }
      ...
      if (
        ...
        sendError(res, 403, 'FORBIDDEN', 'You do not have access to this transfer')
        return
      }
  ...
  async (req, res) => {
    try {
      ...
      if (raw.from_company_id && raw.to_company_id && raw.from_company_id === raw.to_company_id) {
        sendError(res, 400, 'SAME_COMPANY', 'from_company_id and to_company_id must differ')
        return
      }
      ...
      for (const line of lines) {
        const [fromLoc, toLoc] = await Promise.all([
          query<{ company_id: string }>('SELECT company_id FROM stock_locations WHERE id = $1', [
        ...
        if (!fromLoc.rows[0] || fromLoc.rows[0].company_id !== from_company_id) {
          sendError(
            ...
            400,
            'INVALID_LOCATION',
            `from_location ${line.from_location_id} does not belong to from_company`,
          )
          return
        }
        if (!toLoc.rows[0] || toLoc.rows[0].company_id !== to_company_id) {
          sendError(
            ...
            400,
            'INVALID_LOCATION',
            `to_location ${line.to_location_id} does not belong to to_company`,
          )
          return
        }
      }
          ...
          if (pricing.requires_manual_input) {
            return res.status(422).json({
              ...
              error: {
                ...
                message:
                  'This company uses market-based transfer pricing. Provide market_price for each line.',
    ...
    try {
      ...
      if (!result.rows[0]) {
        sendError(
          ...
          409,
          'CANNOT_CANCEL',
          'Transfer not found or cannot be cancelled (only pending transfers can be cancelled)',
        )
        return
      }
