...
const CreateIntercoSchema = z.object({
  ...
  transaction_type: z.enum(['goods_transfer', 'service_charge', 'equipment_rental', 'management_fee']),
...
transactionsRouter.get('/', requirePermission('interco.transactions.view', 'view'), async (req, res) => {
  try {
    ...
    let idx = 2
    if (status) { sql += ` AND it.status = $${idx++}`; params.push(status) }
    sql += ` ORDER BY it.created_at DESC LIMIT $${idx++} OFFSET $${idx++}`
    params.push(parseInt(limit as string), offset)
    const result = await query(sql, params)
    sendOk(res, result.rows)
  } catch (err) { sendError(res, 500, 'INTERNAL_ERROR', 'Failed to fetch transactions', err) }
})

transactionsRouter.get('/:id', requirePermission('interco.transactions.view', 'view'), async (req, res) => {
  try {
    const companyId = req.auth!.companyId
    ...
    if (!row) return sendError(res, 404, 'NOT_FOUND', 'Transaction not found')
    sendOk(res, row)
  } catch (err) { sendError(res, 500, 'INTERNAL_ERROR', 'Failed to fetch transaction', err) }
})
...
transactionsRouter.post('/', requirePermission('interco.transactions.edit', 'edit'), async (req, res) => {
  const companyId = req.auth!.companyId
  ...
  const {
    to_company_id, transaction_type, amount, currency_code, description, reference,
    from_debit_account_id, from_credit_account_id,
    to_debit_account_id, to_credit_account_id,
    entry_date,
  } = parsed.data

  if (companyId === to_company_id) return sendError(res, 400, 'SAME_COMPANY', 'from and to company must differ')

  try {
    const tx = await withIntercoTransaction(req.auth!.userId, async (client, switchContext) => {
      // Verify to_company exists
      const toCompany = await client.query('SELECT id FROM companies WHERE id = $1', [to_company_id])
      if (!toCompany.rows[0]) throw Object.assign(new Error('Target company not found'), { code: 'INVALID_COMPANY', status: 400 })
      ...
      if (!fromPeriod.rows[0]) throw Object.assign(new Error(`No open period for date ${entry_date} in company ${companyId}`), { code: 'NO_OPEN_PERIOD', status: 422 })
      ...
      if (!toPeriod.rows[0]) throw Object.assign(new Error(`No open period for date ${entry_date} in company ${to_company_id}`), { code: 'NO_OPEN_PERIOD', status: 422 })
      ...
      const txResult = await client.query(
        ...
        [companyId, to_company_id, transaction_type, amount, currency_code, fxRate, description ?? null, txRef, fromJEId, toJEId, req.auth!.userId],
      )
      const txRow = txResult.rows[0]!

      // Audit inside the transaction
      await logAudit({
        companyId, userId: req.auth!.userId, action: 'CREATE', tableName: 'interco_transactions',
        recordId: txRow['id'] as string, newValues: { to_company_id, transaction_type, amount, currency_code }, client,
      })
    ...
    if (e.code === 'INVALID_COMPANY') return sendError(res, 400, 'INVALID_COMPANY', e.message ?? 'Invalid company')
    if (e.code === 'NO_OPEN_PERIOD') return sendError(res, 422, 'NO_OPEN_PERIOD', e.message ?? 'No open period')
    sendError(res, 500, 'INTERNAL_ERROR', 'Failed to create interco transaction', err)
  }
})

transactionsRouter.post('/:id/approve-company', requirePermission('interco.transactions.approve', 'approve'), async (req, res) => {
  const companyId = req.auth!.companyId
  ...
  try {
    ...
    const tx = txResult.rows[0] as {
      from_company_id: string; to_company_id: string; status: string
    } | undefined
    if (!tx) return sendError(res, 404, 'NOT_FOUND', 'Transaction not found')
    if (tx['status'] === 'posted') return sendError(res, 409, 'ALREADY_POSTED', 'Transaction already posted')
    if (tx['status'] === 'cancelled') return sendError(res, 409, 'CANCELLED', 'Transaction is cancelled')
  ...
  } catch (err) { sendError(res, 500, 'INTERNAL_ERROR', 'Failed to approve transaction', err) }
})

transactionsRouter.post('/:id/cancel', requirePermission('interco.transactions.approve', 'approve'), async (req, res) => {
  const companyId = req.auth!.companyId
  try {
    const tx = await withIntercoTransaction(req.auth!.userId, async (client, switchContext) => {
      await switchContext(companyId)
      ...
      const txRow = txResult.rows[0] as {
        id: string; status: string; from_journal_entry_id: string; to_journal_entry_id: string; to_company_id: string
      } | undefined
      if (!txRow) throw Object.assign(new Error('Transaction not found'), { code: 'NOT_FOUND', status: 404 })
      if (txRow['status'] === 'cancelled') throw Object.assign(new Error('Already cancelled'), { code: 'ALREADY_CANCELLED', status: 409 })

      // Cancel from-side journal entry
      await switchContext(companyId)
      await client.query(`UPDATE journal_entries SET status = 'cancelled', updated_at = NOW() WHERE id = $1`, [txRow['from_journal_entry_id']])

      // Cancel to-side journal entry
      await switchContext(txRow['to_company_id'])
      await client.query(`UPDATE journal_entries SET status = 'cancelled', updated_at = NOW() WHERE id = $1`, [txRow['to_journal_entry_id']])

      // Update interco record
      await switchContext(companyId)
      await client.query(`UPDATE interco_transactions SET status = 'cancelled', updated_at = NOW() WHERE id = $1`, [txRow['id']])

      await logAudit({ companyId, userId: req.auth!.userId, action: 'UPDATE', tableName: 'interco_transactions', recordId: txRow['id'], newValues: { status: 'cancelled' }, client })
    ...
    const e = err as { code?: string; status?: number; message?: string }
    if (e.code === 'NOT_FOUND') return sendError(res, 404, 'NOT_FOUND', e.message ?? 'Not found')
    if (e.code === 'ALREADY_CANCELLED') return sendError(res, 409, 'ALREADY_CANCELLED', e.message ?? 'Already cancelled')
