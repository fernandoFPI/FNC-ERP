...
ordersRouter.get('/approval-queue/count', requirePermission('procurement.po.view', 'view'), async (req, res) => {
  try {
    const auth = req.auth!
...
ordersRouter.get('/my-queue', requirePermission('procurement.po.view', 'view'), async (req, res) => {
  try {
    const auth = req.auth!
    const empResult = await query(
      `SELECT id, department_id FROM employees WHERE user_id = $1 AND company_id = $2 LIMIT 1`,
...
ordersRouter.get('/positions', requirePermission('procurement.po.view', 'view'), async (req, res) => {
  try {
    ...
    if (project_id)    { sql += ` AND ppa.project_id = $${params.length + 1}`;    params.push(project_id) }
    if (department_id) { sql += ` AND ppa.department_id = $${params.length + 1}`; params.push(department_id) }
...
ordersRouter.get('/', requirePermission('procurement.po.view', 'view'), async (req, res) => {
  try {
    ...
    if (vendor_id) { sql += ` AND po.vendor_id = $${params.length + 1}`; params.push(vendor_id) }
...
ordersRouter.get('/:id', requirePermission('procurement.po.view', 'view'), async (req, res) => {
  try {
    const poResult = await query(
      ...
      [req.params['id'] as string, req.auth!.companyId],
    )
    const po = poResult.rows[0]
    if (!po) return sendError(res, 404, 'NOT_FOUND', 'Purchase order not found')
...
ordersRouter.post('/', requirePermission('procurement.po.edit', 'edit'), async (req, res) => {
  ...
  const actorName = (nameRow.rows[0] as Record<string, unknown>)?.['display_name'] as string ?? 'Unknown'
  ...
  try {
    ...
    for (const line of lines ?? []) {
      ...
      const lineRes = await client.query(
        ...
        [po.id, lineNum++, line.description, line.product_id ?? null,
         line.qty_ordered, line.unit_price, line.currency_code, line.uom,
         total, line.account_id ?? null],
      )
    ...
    const fullLines = await query(`SELECT * FROM po_lines WHERE po_id = $1 ORDER BY line_number`, [po.id])
    sendOk(res, { ...full.rows[0], lines: fullLines.rows }, 201)
  } catch (err: unknown) {
    await client.query('ROLLBACK')
    const e = err as { code?: string }
    if (e.code === '23505') return sendError(res, 409, 'DUPLICATE_PO', 'PO number conflict — retry')
...
ordersRouter.post('/positions', requirePermission('procurement.po.edit', 'edit'), async (req, res) => {
  const auth = req.auth!
  ...
  if (!isAdmin(auth.role)) {
    ...
    if (!hasPOAdmin) return sendError(res, 403, 'FORBIDDEN', 'PO admin or system admin required')
  }

  if (!employee_id || !position) return sendError(res, 400, 'VALIDATION_ERROR', 'employee_id and position are required')
  if (!project_id && !department_id) return sendError(res, 400, 'VALIDATION_ERROR', 'project_id or department_id is required')
  ...
  try {
    const result = await query(
      `INSERT INTO po_position_assignments
       ...
       VALUES ($1,$2,$3,$4,$5,$6)
       RETURNING *`,
      [auth.companyId, employee_id, position, project_id ?? null, department_id ?? null, auth.userId],
    )
    sendOk(res, result.rows[0], 201)
  } catch (err: unknown) {
    const e = err as { code?: string }
    if (e.code === '23505') return sendError(res, 409, 'DUPLICATE_ASSIGNMENT', 'Assignment already exists')
...
ordersRouter.delete('/positions/:id', requirePermission('procurement.po.approve', 'approve'), async (req, res) => {
  if (req.auth!.role !== 'system_admin')
    return sendError(res, 403, 'FORBIDDEN', 'System admin required')
...
ordersRouter.post('/:id/confirm-inventory-check', requirePermission('procurement.po.edit', 'edit'), async (req, res) => {
  const auth = req.auth!
  const ctx = { userId: auth.userId, companyId: auth.companyId, poId: req.params['id'] as string }
  const { items_in_stock, notes } = req.body as { items_in_stock?: string[]; notes?: string }

  try {
    const [hasPos, admin] = await Promise.all([userHasPosition(ctx, 'store_keeper'), Promise.resolve(isAdmin(auth.role))])
    if (!hasPos && !admin) return sendError(res, 403, 'FORBIDDEN', 'Store keeper position required')
    ...
    try {
      ...
      if (emp.rows[0]) {
        await client.query(
          `UPDATE purchase_orders SET store_keeper_id = $1 WHERE id = $2`,
          [emp.rows[0].id, req.params['id'] as string],
        )
      }
      ...
      if (items_in_stock?.length) {
        await client.query(
          ...
          [items_in_stock, req.params['id'] as string],
        )
      }

      await transitionPOInClient(
        client, req.params['id'] as string, 'inventory_check', 'store_pricing',
        'confirm_inventory_check', auth, notes,
    ...
    await notifyPositionHolders(req.params['id'] as string, 'store_pricing', {
...
ordersRouter.post('/:id/submit-store-pricing', requirePermission('procurement.po.edit', 'edit'), async (req, res) => {
  const auth = req.auth!
  const ctx = { userId: auth.userId, companyId: auth.companyId, poId: req.params['id'] as string }
  const { line_prices } = req.body as {
    line_prices?: Array<{ line_id: string; store_price: number; currency_code: string; notes?: string }>
  }

  try {
    const [hasPos, admin] = await Promise.all([userHasPosition(ctx, 'store_pricing'), Promise.resolve(isAdmin(auth.role))])
    if (!hasPos && !admin) return sendError(res, 403, 'FORBIDDEN', 'Store pricing position required')
    ...
    try {
      ...
      for (const lp of line_prices ?? []) {
        await client.query(
          ...
          [lp.store_price, lp.currency_code, lp.notes ?? null, lp.line_id, req.params['id'] as string],
        )
      }
      ...
      if (emp.rows[0]) {
        await client.query(
          `UPDATE purchase_orders SET store_pricing_id = $1 WHERE id = $2`,
          [emp.rows[0].id, req.params['id'] as string],
        )
      }

      await transitionPOInClient(
        client, req.params['id'] as string, 'store_pricing', 'market_pricing',
        'submit_to_market_pricing', auth,
      )
      await client.query('COMMIT')
    } catch (err) {
      await client.query('ROLLBACK')
    ...
    await notifyPositionHolders(req.params['id'] as string, 'procurement_officer', {
...
ordersRouter.post('/:id/submit-market-pricing', requirePermission('procurement.po.edit', 'edit'), async (req, res) => {
  const auth = req.auth!
  const ctx = { userId: auth.userId, companyId: auth.companyId, poId: req.params['id'] as string }
  const { vendor_id, line_prices } = req.body as {
    vendor_id?: string
    line_prices?: Array<{ line_id: string; market_price: number; vendor_quote_ref?: string; currency_code: string }>
  }

  try {
    const [hasPos, admin] = await Promise.all([userHasPosition(ctx, 'procurement_officer'), Promise.resolve(isAdmin(auth.role))])
    if (!hasPos && !admin) return sendError(res, 403, 'FORBIDDEN', 'Procurement officer position required')
    ...
    try {
      ...
      if (vendor_id) {
        ...
        if (!v.rows[0]) { await client.query('ROLLBACK'); return sendError(res, 400, 'INVALID_VENDOR', 'Vendor not found') }
        await client.query(`UPDATE purchase_orders SET vendor_id = $1 WHERE id = $2`, [vendor_id, req.params['id'] as string])
      }
      ...
      for (const lp of line_prices ?? []) {
        await client.query(
          `UPDATE po_lines
           ...
           WHERE id = $4 AND po_id = $5`,
          [lp.market_price, lp.vendor_quote_ref ?? null, lp.currency_code, lp.line_id, req.params['id'] as string],
        )
      }
      if ((line_prices ?? []).length > 0) await recalculatePOTotals(client, req.params['id'] as string)
      ...
      await transitionPOInClient(
        client, req.params['id'] as string, 'market_pricing', 'price_verification',
        'submit_to_price_verification', auth,
      )
      await client.query('COMMIT')
    } catch (err) {
      await client.query('ROLLBACK')
    ...
    await notifyPositionHolders(req.params['id'] as string, 'procurement_2nd', {
...
ordersRouter.post('/:id/submit-price-verification', requirePermission('procurement.po.edit', 'edit'), async (req, res) => {
  const auth = req.auth!
  const ctx = { userId: auth.userId, companyId: auth.companyId, poId: req.params['id'] as string }
  const { verification_notes, line_adjustments } = req.body as {
    verification_notes?: string
    line_adjustments?: Array<{ line_id: string; verified_price: number; notes?: string }>
  }

  try {
    const [hasPos, admin] = await Promise.all([userHasPosition(ctx, 'procurement_2nd'), Promise.resolve(isAdmin(auth.role))])
    if (!hasPos && !admin) return sendError(res, 403, 'FORBIDDEN', '2nd procurement position required')
    ...
    try {
      ...
      for (const adj of line_adjustments ?? []) {
        await client.query(
          ...
          [adj.verified_price, adj.notes ?? null, adj.line_id, req.params['id'] as string],
        )
      }
      if ((line_adjustments ?? []).length > 0) await recalculatePOTotals(client, req.params['id'] as string)
      ...
      if (emp.rows[0]) {
        await client.query(
          `UPDATE purchase_orders SET procurement_2nd_id = $1 WHERE id = $2`,
          [emp.rows[0].id, req.params['id'] as string],
        )
      }

      await transitionPOInClient(
        client, req.params['id'] as string, 'price_verification', 'pending_approval',
        'submit_for_approval', auth, undefined,
    ...
    await notifyDeptHeadsAndAdmins(req.params['id'] as string, {
...
ordersRouter.post('/:id/reject-to-market-pricing', requirePermission('procurement.po.approve', 'approve'), async (req, res) => {
  const auth = req.auth!
  const ctx = { userId: auth.userId, companyId: auth.companyId, poId: req.params['id'] as string }
  ...
  try {
    const [deptHead, assignedApprover, admin] = await Promise.all([
      userIsDeptHead(ctx), userIsAssignedApprover(ctx), Promise.resolve(isAdmin(auth.role)),
    ])
    if (!deptHead && !assignedApprover && !admin) return sendError(res, 403, 'FORBIDDEN', 'Department head or admin required')

    await transitionPO(
      req.params['id'] as string, 'pending_approval', 'market_pricing',
      'reject_to_market_pricing', auth, reason,
    )
    await notifyPositionHolders(req.params['id'] as string, 'procurement_officer', {
...
ordersRouter.post('/:id/approve', requirePermission('procurement.po.approve', 'approve'), async (req, res) => {
  const auth = req.auth!
  const ctx = { userId: auth.userId, companyId: auth.companyId, poId: req.params['id'] as string }

  try {
    const poRow = await query(`SELECT status, organizer_id, po_number FROM purchase_orders WHERE id = $1`, [req.params['id'] as string])
    if (!poRow.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Purchase order not found')

    if (poRow.rows[0]['status'] !== 'pending_approval') {
      return sendError(res, 422, 'INVALID_STATUS', `Cannot approve PO in status: ${String(poRow.rows[0]['status'])}`)
    }
    ...
    if (!admin && !deptHead && !assignedApprover)
      return sendError(res, 403, 'FORBIDDEN', 'Department head or admin required to approve')

    await transitionPO(req.params['id'] as string, 'pending_approval', 'approved', 'approve', auth)
    ...
    if (approveOrgId) {
      await query(
        ...
        [JSON.stringify({ userId: approveOrgId, poId: req.params['id'] as string, poNumber: poRow.rows[0]['po_number'] })],
      )
    }
    await query(
      `INSERT INTO service_outbox (service, event_type, payload) VALUES ('worker','PO_PDF_REQUESTED',$1)`,
      [JSON.stringify({ po_id: req.params['id'] as string, company_id: auth.companyId })],
    )
...
ordersRouter.post('/:id/reject', requirePermission('procurement.po.approve', 'approve'), async (req, res) => {
  const auth = req.auth!
  const ctx = { userId: auth.userId, companyId: auth.companyId, poId: req.params['id'] as string }
  const { reason } = req.body as { reason?: string }
  if (!reason?.trim()) return sendError(res, 400, 'REASON_REQUIRED', 'Rejection reason is required')
  ...
  try {
    const poRow = await query(`SELECT status, organizer_id FROM purchase_orders WHERE id = $1`, [req.params['id'] as string])
    if (!poRow.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Purchase order not found')
    if (poRow.rows[0]['status'] !== 'pending_approval')
      return sendError(res, 422, 'INVALID_STATUS', `Cannot reject PO in status: ${String(poRow.rows[0]['status'])}`)
    ...
    if (!admin && !deptHead && !assignedApprover)
      return sendError(res, 403, 'FORBIDDEN', 'Department head or admin required to reject')

    await transitionPO(req.params['id'] as string, 'pending_approval', 'rejected', 'reject', auth, reason)
    ...
    if (rejectOrgId) {
      await query(
        ...
        [JSON.stringify({ userId: rejectOrgId, poId: req.params['id'] as string, reason })],
      )
    }
...
ordersRouter.post('/:id/reopen', requirePermission('procurement.po.edit', 'edit'), async (req, res) => {
  const auth = req.auth!
  const ctx = { userId: auth.userId, companyId: auth.companyId, poId: req.params['id'] as string }
  try {
    const poRow = await query(`SELECT status FROM purchase_orders WHERE id = $1`, [req.params['id'] as string])
    if (!poRow.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Purchase order not found')
    if (poRow.rows[0]['status'] !== 'rejected')
      return sendError(res, 422, 'INVALID_STATUS', 'PO must be in rejected status to reopen')

    const [organizer, admin] = await Promise.all([userIsOrganizer(ctx), Promise.resolve(isAdmin(auth.role))])
    if (!organizer && !admin) return sendError(res, 403, 'FORBIDDEN', 'Organizer or admin required to reopen PO')

    await transitionPO(req.params['id'] as string, 'rejected', 'draft', 'reopen', auth)
...
ordersRouter.post('/:id/cancel', requirePermission('procurement.po.edit', 'edit'), async (req, res) => {
  const auth = req.auth!
  const ctx = { userId: auth.userId, companyId: auth.companyId, poId: req.params['id'] as string }
  const { reason } = req.body as { reason?: string }
  try {
    ...
    if (!cancellable.includes(currentStatus))
      return sendError(res, 422, 'INVALID_STATUS', `Cannot cancel PO in status '${currentStatus}'`)

    const [organizer, admin] = await Promise.all([userIsOrganizer(ctx), Promise.resolve(isAdmin(auth.role))])
    if (!organizer && !admin) return sendError(res, 403, 'FORBIDDEN', 'Organizer or admin required to cancel PO')

    await transitionPO(
      req.params['id'] as string,
      currentStatus as import('@fnc-erp/workflow').POStatus,
      'cancelled', 'cancel', auth, reason,
    )
...
ordersRouter.post('/:id/send-to-audit', requirePermission('procurement.po.edit', 'edit'), async (req, res) => {
  const auth = req.auth!
  const ctx = { userId: auth.userId, companyId: auth.companyId, poId: req.params['id'] as string }
  try {
    const poRow = await query(`SELECT status, po_number FROM purchase_orders WHERE id = $1`, [req.params['id'] as string])
    if (!poRow.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Purchase order not found')
    if (poRow.rows[0]['status'] !== 'goods_received')
      return sendError(res, 422, 'INVALID_STATUS', 'PO must be in goods_received status to send to audit')

    const [organizer, admin] = await Promise.all([userIsOrganizer(ctx), Promise.resolve(isAdmin(auth.role))])
    if (!organizer && !admin) return sendError(res, 403, 'FORBIDDEN', 'Organizer or admin required')

    const client = await pool.connect()
    try {
      await client.query('BEGIN')
      await transitionPOInClient(client, req.params['id'] as string, 'goods_received', 'finance_audit', 'send_to_audit', auth)
      await client.query(`UPDATE po_lines SET audit_status='pending', audit_note=NULL WHERE po_id=$1`, [req.params['id'] as string])
      await client.query('COMMIT')
    } catch (err) {
      await client.query('ROLLBACK')
      throw err
...
ordersRouter.post('/:id/pass-audit', requirePermission('procurement.po.approve', 'approve'), async (req, res) => {
  const auth = req.auth!
  if (!isAdmin(auth.role) && auth.role !== 'finance')
    return sendError(res, 403, 'FORBIDDEN', 'Finance or admin role required to pass audit')

  try {
    const poRow = await query(`SELECT status, organizer_id, po_number FROM purchase_orders WHERE id = $1`, [req.params['id'] as string])
    if (!poRow.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Purchase order not found')
    if (poRow.rows[0]['status'] !== 'finance_audit')
      return sendError(res, 422, 'INVALID_STATUS', 'PO must be in finance_audit status')

    const flaggedRes = await query(`SELECT COUNT(*) AS c FROM po_lines WHERE po_id=$1 AND audit_status='flagged'`, [req.params['id'] as string])
    const flagged = parseInt(String(flaggedRes.rows[0]?.['c'] ?? '0'))
    if (flagged > 0)
      return sendError(res, 422, 'LINES_FLAGGED', `Cannot pass audit: ${flagged} line${flagged > 1 ? 's are' : ' is'} flagged`)

    await transitionPO(req.params['id'] as string, 'finance_audit', 'invoiced', 'pass_audit', auth)
...
ordersRouter.post('/:id/fail-audit', requirePermission('procurement.po.approve', 'approve'), async (req, res) => {
  const auth = req.auth!
  const { notes } = req.body as { notes?: string }
  if (!isAdmin(auth.role) && auth.role !== 'finance')
    return sendError(res, 403, 'FORBIDDEN', 'Finance or admin role required to fail audit')
  ...
  try {
    ...
    await transitionPO(req.params['id'] as string, 'finance_audit', 'goods_received', 'fail_audit', auth, notes)
...
ordersRouter.patch('/:id/lines/:lineId/audit-status', requirePermission('procurement.po.approve', 'approve'), async (req, res) => {
  const auth = req.auth!
  if (!isAdmin(auth.role) && auth.role !== 'finance')
    return sendError(res, 403, 'FORBIDDEN', 'Finance or admin role required to audit PO lines')
  ...
  if (!audit_status || !valid.includes(audit_status))
    return sendError(res, 400, 'VALIDATION_ERROR', `audit_status must be one of: ${valid.join(', ')}`)

  try {
    const poRow = await query(`SELECT status FROM purchase_orders WHERE id=$1`, [id])
    if (!poRow.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'PO not found')
    if (poRow.rows[0]['status'] !== 'finance_audit')
      return sendError(res, 422, 'INVALID_STATUS', 'PO must be in finance_audit status to audit lines')
    ...
    if (!result.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'PO line not found')
...
ordersRouter.post('/:id/complete', requirePermission('procurement.po.approve', 'approve'), async (req, res) => {
  const auth = req.auth!
  const { receipt_notes } = req.body as { receipt_notes?: string }

  try {
    if (!isAdmin(auth.role) && auth.role !== 'finance')
      return sendError(res, 403, 'FORBIDDEN', 'Finance or admin role required to complete PO')

    const poRow = await query(`SELECT status, organizer_id, po_number FROM purchase_orders WHERE id = $1`, [req.params['id'] as string])
    if (!poRow.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Purchase order not found')
    if (poRow.rows[0]['status'] !== 'invoiced')
      return sendError(res, 422, 'INVALID_STATUS', `PO must be in invoiced status to complete. Current: ${String(poRow.rows[0]['status'])}`)

    await transitionPO(
      req.params['id'] as string, 'invoiced', 'completed', 'complete', auth,
      receipt_notes, { receiptNotes: receipt_notes },
    )
    await query(
      `INSERT INTO service_outbox (service, event_type, payload) VALUES ('finance','PO_COMPLETION_JOURNAL_REQUESTED',$1)`,
      [JSON.stringify({ po_id: req.params['id'] as string, company_id: auth.companyId, receipt_notes })],
    )
...
ordersRouter.post('/:id/delete', requirePermission('procurement.po.edit', 'edit'), async (req, res) => {
  const auth = req.auth!
  const ctx = { userId: auth.userId, companyId: auth.companyId, poId: req.params['id'] as string }
  const { reason } = req.body as { reason?: string }
  ...
  try {
    ...
    if (!['draft', 'inventory_check'].includes(currentStatus ?? '')) {
      return sendError(res, 400, 'CANNOT_DELETE', 'PO can only be deleted from draft or inventory_check status')
    }

    const [organizer, admin] = await Promise.all([userIsOrganizer(ctx), Promise.resolve(isAdmin(auth.role))])
    if (!organizer && !admin) return sendError(res, 403, 'FORBIDDEN', 'Organizer or admin required to delete PO')

    await transitionPO(
      req.params['id'] as string,
      currentStatus as import('@fnc-erp/workflow').POStatus,
      'deleted', 'delete', auth, reason,
    )
...
ordersRouter.post('/:id/edit-requests', requirePermission('procurement.po.edit', 'edit'), async (req, res) => {
  const auth = req.auth!
  const poId = req.params['id'] as string
  const { changes, notes } = req.body as { changes: unknown; notes?: string }

  if (!changes || typeof changes !== 'object') {
    return sendError(res, 400, 'INVALID_INPUT', 'changes object is required')
  }
  ...
  try {
    ...
    if (!isAdmin(auth.role)) {
      ...
      if (!inScope) return sendError(res, 403, 'FORBIDDEN', 'You are not associated with this PO or its project')
    }
    ...
    if ((existing.rowCount ?? 0) > 0) {
      return sendError(res, 409, 'PENDING_EXISTS', 'A pending edit request already exists for this PO. It must be approved or rejected first.')
    }
...
ordersRouter.get('/:id/edit-requests', requirePermission('procurement.po.view', 'view'), async (req, res) => {
  const auth = req.auth!
  const poId = req.params['id'] as string
...
ordersRouter.post('/:id/edit-requests/:reqId/approve', requirePermission('procurement.po.approve', 'approve'), async (req, res) => {
  const auth = req.auth!
  if (!isAdmin(auth.role)) return sendError(res, 403, 'FORBIDDEN', 'Admin role required to approve edit requests')
  ...
  try {
    ...
    if (!erRow.rows[0]) {
      await client.query('ROLLBACK')
      return sendError(res, 404, 'NOT_FOUND', 'Pending edit request not found')
    }

    const changes = erRow.rows[0]['changes'] as {
      header?: Record<string, { from: unknown; to: unknown }>
      lines?: {
        edited?: Array<{ id: string; field: string; to: unknown }>
    ...
    if (changes.lines) {
      ...
      for (const edit of changes.lines.edited ?? []) {
        const lineAllowed = ['description', 'qty_ordered', 'unit_price', 'uom', 'product_id', 'account_id']
      ...
      const editActorName = (editActorNameRow.rows[0] as Record<string, unknown>)?.['display_name'] as string ?? 'Unknown'
      for (const line of changes.lines.added ?? []) {
        ...
        const addedLineRes = await client.query(
          ...
          [poId, line['description'], line['product_id'] ?? null, qty, price,
           line['currency_code'] ?? 'IQD', line['uom'] ?? 'unit', qty * price],
        )
...
ordersRouter.post('/:id/edit-requests/:reqId/reject', requirePermission('procurement.po.approve', 'approve'), async (req, res) => {
  const auth = req.auth!
  if (!isAdmin(auth.role)) return sendError(res, 403, 'FORBIDDEN', 'Admin role required to reject edit requests')
  ...
  if (!review_notes?.trim()) {
    return sendError(res, 400, 'INVALID_INPUT', 'review_notes is required when rejecting')
  }
  ...
  try {
    ...
    if (!result.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Pending edit request not found')
...
ordersRouter.get('/:id/line-comments', requirePermission('procurement.po.view', 'view'), async (req, res) => {
  const auth = (req as import('express').Request & { auth?: { companyId: string } }).auth
  if (!auth) return sendError(res, 401, 'UNAUTHORIZED', 'Not authenticated')
  const { id } = req.params
  try {
    const result = await query(
...
ordersRouter.post('/:id/lines/:lineId/comments', requirePermission('procurement.po.view', 'view'), async (req, res) => {
  const auth = (req as import('express').Request & { auth?: { companyId: string; userId: string } }).auth
  if (!auth) return sendError(res, 401, 'UNAUTHORIZED', 'Not authenticated')
  const { id, lineId } = req.params
  const { comment, flag } = req.body as { comment?: string; flag?: string }
  if (!comment?.trim()) return sendError(res, 400, 'VALIDATION_ERROR', 'Comment is required')
  const validFlags = ['info', 'warning', 'dispute']
  const safeFlag = flag && validFlags.includes(flag) ? flag : null
  try {
    const po = await query(`SELECT id FROM purchase_orders WHERE id=$1 AND company_id=$2`, [id, auth.companyId])
    if (!po.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'PO not found')
    const userRow = await query(
      `SELECT COALESCE(NULLIF(TRIM(COALESCE(first_name,'') || ' ' || COALESCE(last_name,'')), ''), email) AS display_name FROM users WHERE id=$1`,
      [auth.userId],
    )
    const displayName = (userRow.rows[0] as Record<string, unknown>)?.['display_name'] as string ?? 'Unknown'
    const result = await query(
...
ordersRouter.patch('/:id/lines/:lineId/comments/:commentId/resolve', requirePermission('procurement.po.view', 'view'), async (req, res) => {
  const auth = (req as import('express').Request & { auth?: { companyId: string; userId: string } }).auth
  if (!auth) return sendError(res, 401, 'UNAUTHORIZED', 'Not authenticated')
  ...
  try {
    ...
    if (!result.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Flag not found or already resolved')
    sendOk(res, result.rows[0])
  } catch (err) { handleError(res, err) }
})

// ── PATCH /:id/priority — set PO priority ────────────────────────────────────
ordersRouter.patch('/:id/priority', requirePermission('procurement.po.view', 'view'), async (req, res) => {
  const auth = (req as import('express').Request & { auth?: { companyId: string; userId: string } }).auth
  if (!auth) return sendError(res, 401, 'UNAUTHORIZED', 'Not authenticated')
  const { id } = req.params
  const { priority } = req.body as { priority?: string }
  if (!priority || !['low', 'high', 'emergency'].includes(priority)) {
    return sendError(res, 400, 'VALIDATION_ERROR', 'priority must be low, high, or emergency')
  }
  try {
    const result = await query(
    ...
    if (!result.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'PO not found')
    sendOk(res, result.rows[0])
  } catch (err) { handleError(res, err) }
})

// ── PATCH /:id/receiver — assign the employee who will receive items ──────────
ordersRouter.patch('/:id/receiver', requirePermission('procurement.po.edit', 'edit'), async (req, res) => {
  const auth = (req as import('express').Request & { auth?: { companyId: string; userId: string } }).auth
  if (!auth) return sendError(res, 401, 'UNAUTHORIZED', 'Not authenticated')
  const { id } = req.params
  ...
  try {
    ...
    if (employee_id) {
      const emp = await query(`SELECT id, user_id FROM employees WHERE id=$1 AND company_id=$2`, [employee_id, auth.companyId])
...
ordersRouter.patch('/:id/lines/:lineId/actual-price', requirePermission('procurement.po.edit', 'edit'), async (req, res) => {
  const auth = (req as import('express').Request & { auth?: { companyId: string; userId: string } }).auth
  if (!auth) return sendError(res, 401, 'UNAUTHORIZED', 'Not authenticated')
  const { id, lineId } = req.params
  ...
  try {
    ...
    if (!lineCheck.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'PO line not found')
    const result = await query(
      `UPDATE po_lines
