  ...
  async (req, res) => {
    try {
      ...
      const result = await query(
        'SELECT * FROM po_receipts WHERE po_id = $1 ORDER BY received_date DESC',
        [req.params['poId']],
      )
      sendOk(res, result.rows)
    } catch (err) {
      sendError(res, 500, 'INTERNAL_ERROR', 'Failed to fetch receipts', err)
    }
  },
)
  ...
  async (req, res) => {
    ...
    try {
      ...
      const po = poResult.rows[0] as
        | { id: string; status: string; currency_code: string }
        | undefined
      if (!po) {
      ...
      if (po.status !== 'approved' && po.status !== 'goods_received') {
        ...
        sendError(
          ...
          422,
          'INVALID_STATUS',
          `Cannot receive against a PO in status '${po.status}' — PO must be approved or goods_received`,
        )
        return
      }
      ...
      for (const rl of lines) {
        ...
        if (!poLine) {
          await client.query('ROLLBACK')
          sendError(res, 400, 'INVALID_LINE', `PO line ${rl.po_line_id} not found`)
          return
        }
        ...
        if (newTotalReceived > qtyOrdered) {
          ...
          await client.query('UPDATE po_lines SET qty_received = qty_received + $1 WHERE id = $2', [
            rl.qty_received,
            rl.po_line_id,
          ])
        }
      ...
      if (isFirstReceipt && po.status === 'approved') {
        ...
        const orgRes = await client.query<{ organizer_id: string; po_number: string }>(
          `SELECT organizer_id, po_number FROM purchase_orders WHERE id=$1`,
          [po.id],
        )
      ...
      await logAudit({
        ...
        recordId: receiptId,
        newValues: { receipt_number, received_date },
        client,
      })
