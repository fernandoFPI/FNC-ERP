...
const log = createServiceLogger('manufacturing')

const app = createApp()
const port = env.MANUFACTURING_SERVICE_PORT
...
async function start() {
  ...
  const server = app.listen(port, () => {
    log.info({ port }, 'Manufacturing service started')
  })
  ...
  const shutdown = async () => {
    log.info('manufacturing service shutting down')
