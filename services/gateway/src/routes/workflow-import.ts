...
workflowImportRouter.post('/', requireAuth(), upload.single('file'), async (req, res) => {
  ...
  if (!uploadedFile) {
    res.status(400).json({
      success: false,
      error: { code: 'VALIDATION_ERROR', message: 'SQL file is required' },
    })
    return
  }
  ...
  try {
    ...
    res.status(400).json({
      success: false,
      error: { code: 'PARSE_ERROR', message: 'Could not decode file as UTF-8' },
    })
    return
  }
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: { code: 'DB_ERROR', message: (err as Error).message } })
    return
  } finally {
    client.release()
  }
