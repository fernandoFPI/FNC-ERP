...
filesRouter.post('/upload-url', requireAuth(), async (req, res) => {
  ...
  if (!validation.valid) {
    res
      .status(400)
      .json({ success: false, error: { code: 'INVALID_FILE', message: validation.reason } })
    return
  }
...
filesRouter.post('/confirm', requireAuth(), async (req, res) => {
  ...
  if (!fileId) {
    res
      .status(400)
      .json({ success: false, error: { code: 'VALIDATION_ERROR', message: 'fileId is required' } })
    return
  }
  ...
  if (!file.rows[0]) {
    res
      .status(404)
      .json({ success: false, error: { code: 'FILE_NOT_FOUND', message: 'File not found' } })
    return
  }
  if (file.rows[0].status !== 'pending') {
    res.status(400).json({
      success: false,
      error: { code: 'ALREADY_CONFIRMED', message: 'File already confirmed' },
    })
    return
  }
...
filesRouter.post(
  ...
  async (req, res) => {
    ...
    if (file.rows[0].status !== 'pending') {
      res.status(400).json({
        success: false,
        error: { code: 'ALREADY_UPLOADED', message: 'File already uploaded' },
      })
      return
    }
    try {
  ...
  try {
    ...
    const withUrls = await Promise.all(
      rows.rows.map(async (row) => {
        ...
        const file = await query<{ file_key: string }>(`SELECT file_key FROM files WHERE id=$1`, [
          r.file_id,
        ])
        try {
...
filesRouter.post('/attach', requireAuth(), async (req, res) => {
  ...
  if (!fileId || !entityType || !entityId) {
    res.status(400).json({
      ...
      error: {
        code: 'VALIDATION_ERROR',
        message: 'fileId, entityType, and entityId are required',
      },
    })
    return
  }
  ...
  if (!file.rows[0]) {
    res.status(404).json({
      success: false,
      error: { code: 'FILE_NOT_FOUND', message: 'File not found or not yet uploaded' },
    })
    return
  }
  try {
...
filesRouter.delete('/:fileId', requireAuth(), async (req, res) => {
  ...
  if (parseInt(attachments.rows[0]?.count ?? '0') > 0) {
    res.status(400).json({
      ...
      error: {
        code: 'FILE_IN_USE',
        message: 'Cannot delete a file that is attached to records. Remove attachments first.',
      },
    })
    return
  }
