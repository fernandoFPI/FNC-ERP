...
function parseExcel(buffer: Buffer): ImportItem[] {
  ...
  for (const sheetName of wb.SheetNames) {
    ...
    for (let i = 4; i < rows.length; i++) {
      ...
      const arabicName = String((row as unknown[])[1] ?? '').trim()
      const englishName = String((row as unknown[])[2] ?? '').trim()
      const uom = String((row as unknown[])[3] ?? 'EA').trim() || 'EA'
      const unitCost = parseFloat(String((row as unknown[])[4] ?? '0')) || 0
      const qty = parseFloat(String((row as unknown[])[5] ?? '0')) || 0
...
inventoryImportRouter.post(
  ...
  async (req, res) => {
    ...
    if (items.length === 0) {
      res.status(400).json({
        success: false,
        error: { code: 'EMPTY_FILE', message: 'No recognised store sheets found in the uploaded file' },
      })
      return
    }
    ...
    let created = 0
    let updated = 0
    const errors: Array<{ sku: string; name: string; message: string }> = []
