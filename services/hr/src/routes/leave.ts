...
leaveRouter.get('/types', requirePermission('hr.leave.view', 'view'), async (req, res) => {
  ...
  } catch (err) { sendError(res, 500, 'INTERNAL_ERROR', 'Failed to fetch leave types', err) }
})
...
leaveRouter.get('/requests', requirePermission('hr.leave.view', 'view'), async (req, res) => {
  try {
    ...
    let idx = 2
    if (employee_id) { sql += ` AND lr.employee_id = $${idx++}`; params.push(employee_id) }
    if (status) { sql += ` AND lr.status = $${idx++}`; params.push(status) }
    sql += ` ORDER BY lr.created_at DESC LIMIT $${idx++} OFFSET $${idx++}`
    params.push(parseInt(limit as string), offset)
    const result = await query(sql, params)
    sendOk(res, result.rows)
  } catch (err) { sendError(res, 500, 'INTERNAL_ERROR', 'Failed to fetch leave requests', err) }
})
...
leaveRouter.post('/requests', requirePermission('hr.leave.edit', 'edit'), async (req, res) => {
  ...
  try {
    ...
    const emp = await query(`SELECT id FROM employees WHERE id = $1 AND company_id = $2`, [employee_id, req.auth!.companyId])
    if (!emp.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Employee not found')
    // Verify leave type belongs to company
    const lt = await query(`SELECT id, requires_approval FROM leave_types WHERE id = $1 AND company_id = $2`, [leave_type_id, req.auth!.companyId])
    if (!lt.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Leave type not found')
    ...
    const result = await query(
      ...
      [employee_id, req.auth!.companyId, leave_type_id, start_date, end_date, total_days, reason ?? null, initialStatus, req.auth!.userId],
    )
    if (requiresApproval) {
      ...
      ;(async () => {
        ...
        const empName = `${empRes.rows[0]?.first_name ?? ''} ${empRes.rows[0]?.last_name ?? ''}`.trim()
        ...
        for (const u of admins.rows) {
          await query(
            ...
            [JSON.stringify({ userId: u['user_id'], companyId, title: `Leave request: ${empName}`,
              body: `${empName} has submitted a leave request from ${start_date} to ${end_date} (${total_days} days)${reason ? ': ' + reason : ''}`,
              data: { leaveRequestId: result.rows[0]!['id'], employeeName: empName, startDate: start_date, endDate: end_date } })],
          )
        }
      })().catch(() => {})
    }
    sendOk(res, result.rows[0]!, 201)
  } catch (err) { sendError(res, 500, 'INTERNAL_ERROR', 'Failed to create leave request', err) }
})

leaveRouter.post('/requests/:id/approve', requirePermission('hr.leave.approve', 'approve'), async (req, res) => {
  try {
    const result = await query(
    ...
    if (!result.rows[0]) return sendError(res, 404, 'NOT_FOUND', 'Leave request not found or not pending')
    // Notify the requesting employee
    const lr = result.rows[0] as Record<string, unknown>
    ;(async () => {
      const empRes = await query<{ user_id: string | null }>(`SELECT user_id FROM employees WHERE id=$1`, [lr['employee_id']])
      const userId = empRes.rows[0]?.user_id
      if (userId) {
        await query(`INSERT INTO service_outbox (service,event_type,payload) VALUES ('notifications','LEAVE_REQUEST_APPROVED',$1)`, [JSON.stringify({ userId, companyId: req.auth!.companyId, title: 'Leave request approved', body: `Your leave request from ${String(lr['start_date'])} to ${String(lr['end_date'])} (${String(lr['total_days'])} days) has been approved`, data: { leaveRequestId: req.params['id'], startDate: lr['start_date'], endDate: lr['end_date'] } })])
      }
    })().catch(() => {})
    sendOk(res, result.rows[0]!)
  } catch (err) { sendError(res, 500, 'INTERNAL_ERROR', 'Failed to approve leave request', err) }
})

leaveRouter.post('/requests/:id/reject', requirePermission('hr.leave.approve', 'approve'), async (req, res) => {
  try {
    const result = await query(
    ...
    ;(async () => {
      ...
      const userId = empRes.rows[0]?.user_id
      if (userId) {
        await query(`INSERT INTO service_outbox (service,event_type,payload) VALUES ('notifications','LEAVE_REQUEST_REJECTED',$1)`, [JSON.stringify({ userId, companyId: req.auth!.companyId, title: 'Leave request not approved', body: `Your leave request from ${String(lr['start_date'])} to ${String(lr['end_date'])} has not been approved`, data: { leaveRequestId: req.params['id'], startDate: lr['start_date'], endDate: lr['end_date'] } })])
      }
    })().catch(() => {})
    sendOk(res, result.rows[0]!)
  } catch (err) { sendError(res, 500, 'INTERNAL_ERROR', 'Failed to reject leave request', err) }
})
