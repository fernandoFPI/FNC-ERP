  ...
  useEffect(() => {
    async function check() {
      if (!isAuthenticated || !accessToken) {
        setAuthed(false); setChecking(false); return
      }
      if (!isTokenExpired(accessToken)) {
        setAuthed(true); setChecking(false); return
      }
      if (!refreshToken) {
        clearAuth(); setAuthed(false); setChecking(false); return
      }
      try {
        ...
        if (newToken) { setAccessToken(newToken); setAuthed(true) }
        else throw new Error('No token in response')
      } catch {
        clearAuth(); setAuthed(false)
