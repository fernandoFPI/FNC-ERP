...
export function useInvoicePreview(params: PreviewParams | null) {
  ...
  useEffect(() => {
    ...
    timerRef.current = setTimeout(async () => {
      setLoading(true)
      try {
        const res = await api.post<InvoicePreview>(`/projects/${params.projectId}/invoice-preview`, {
        ...
        const tax = params.lines.reduce((s, l) => s + l.quantity * l.unit_price * ((l.tax_pct ?? 0) / 100), 0)
