...
export function usePermission() {
  ...
  const isSystemLevel = user?.role === 'system_admin' || user?.role === 'company_admin'

  function can(permissionKey: string, minLevel: Exclude<AccessLevel, 'none'> = 'view'): boolean {
  ...
  function canAny(keys: string[], minLevel: Exclude<AccessLevel, 'none'> = 'view'): boolean {
    return keys.some((k) => can(k, minLevel))
  }

  function canAll(keys: string[], minLevel: Exclude<AccessLevel, 'none'> = 'view'): boolean {
