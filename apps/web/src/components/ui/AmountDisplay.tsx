...
export function AmountDisplay({ amount, currency, size = 'md', showSign = false, colored = false }: AmountDisplayProps) {
  ...
  return (
    <span style={{ fontFamily: 'ui-monospace, monospace', fontSize: fontSizes[size], color, whiteSpace: 'nowrap' }}>
      {sign}{formatted} <span style={{ opacity: 0.6, fontSize: '0.85em' }}>{currency}</span>
    </span>
  )
}
