...
function rowHighlight(action: string, theme: { warningBg: string; infoBg: string }): string | undefined {
...
function DiffPanel({ old: oldRaw, next: nextRaw }: { old: string | null; next: string | null }) {
  ...
  if (allKeys.length === 0) return <span style={{ color: theme.textMuted, fontSize: 12 }}>No diff data</span>
  return (
    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
      <thead>
        <tr>
          <th style={{ textAlign: 'left' as const, color: theme.textMuted, padding: '2px 8px', fontWeight: 500 }}>Field</th>
          <th style={{ textAlign: 'left' as const, color: theme.danger, padding: '2px 8px', fontWeight: 500 }}>Old</th>
          <th style={{ textAlign: 'left' as const, color: theme.success, padding: '2px 8px', fontWeight: 500 }}>New</th>
        </tr>
      </thead>
      <tbody>
        {allKeys.map(k => (
...
export function AuditLogViewer({ filters: externalFilters, embedded = false }: AuditLogViewerProps) {
  const { theme } = useTheme()
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      {!embedded && (
        <div style={{
          ...
          <Input
            label="User ID"
            value={localUserId}
            onChange={e => { setLocalUserId(e.target.value); setPage(1) }}
          ...
          <Input
            label="Table"
            value={localTable}
            onChange={e => { setLocalTable(e.target.value); setPage(1) }}
          ...
          <Select
            label="Action"
            value={localAction}
            onChange={e => { setLocalAction(e.target.value); setPage(1) }}
          ...
          <Input
            ...
            type="date"
            value={localFrom}
            onChange={e => { setLocalFrom(e.target.value); setPage(1) }}
            style={{ width: 140 }}
          />
          <Input
            label="To"
            type="date"
            value={localTo}
            onChange={e => { setLocalTo(e.target.value); setPage(1) }}
            style={{ width: 140 }}
          />
          <Button onClick={() => {
            setLocalUserId(''); setLocalTable(''); setLocalAction('')
            setLocalFrom(''); setLocalTo(''); setPage(1)
          }}>Clear</Button>
        </div>
      )}

      {error && (
        <div style={{ color: theme.danger, fontSize: 13 }}>Failed to load audit log: {error.message}</div>
      )}
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 800 }}>
          ...
          <tbody>
            {loading && rows.length === 0 && (
              <tr>
                <td colSpan={7} style={{ padding: 24, textAlign: 'center', color: theme.textMuted }}>Loading…</td>
              </tr>
            )}
            {!loading && rows.length === 0 && (
              <tr>
                <td colSpan={7} style={{ padding: 24, textAlign: 'center', color: theme.textMuted }}>No audit entries found.</td>
              </tr>
            )}
            {rows.map(row => {
              ...
              return [
                ...
                >
                  <td style={{ padding: '9px 12px', color: theme.textMuted, fontSize: 12, whiteSpace: 'nowrap' }}>
                    <span title={row.createdAt}>{formatRelativeTime(row.createdAt)}</span>
                    <div style={{ fontSize: 10, color: theme.textMuted }}>{formatDate(row.createdAt)}</div>
                  </td>
                  <td style={{ padding: '9px 12px', color: theme.textSecondary, fontSize: 13 }}>{row.userEmail}</td>
                  <td style={{ padding: '9px 12px' }}>
                    <Badge variant={actionVariant(row.action)}>{row.action}</Badge>
                  </td>
                  <td style={{ padding: '9px 12px', color: theme.textSecondary, fontSize: 13 }}>{row.tableName}</td>
                  <td style={{ padding: '9px 12px', color: theme.textMuted, fontSize: 12, fontFamily: 'monospace' }}>
                    {row.recordId?.slice(0, 12)}
                  </td>
                  <td style={{ padding: '9px 12px', color: theme.textSecondary, fontSize: 13 }}>{row.companyName}</td>
                ...
                isExpanded && (
                  <tr key={`${row.id}-detail`} style={{ background: theme.bgSurface }}>
                    <td colSpan={7} style={{ padding: '12px 20px', borderBottom: `1px solid ${theme.tableBorder}` }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '4px 0' }}>
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          <Button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page <= 1} size="sm">Prev</Button>
          <span style={{ color: theme.textSecondary, fontSize: 13 }}>Page {page} / {totalPages}</span>
