...
export function LeaveCalendar({ month, leaves, onMonthChange }: LeaveCalendarProps) {
  ...
  const dateLeaves: Record<string, Array<LeaveEntry & { colorIdx: number }>> = {}
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
        <button onClick={() => onMonthChange(new Date(year, mon - 1, 1))}
          style={{ background: 'none', border: 'none', cursor: 'pointer', color: theme.textMuted, fontSize: '18px', padding: '4px 8px' }}>‹</button>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '2px', marginBottom: '4px' }}>
        {DAY_LABELS.map((d) => (
          <div key={d} style={{ textAlign: 'center', fontSize: '10px', fontWeight: 600, color: theme.textMuted, textTransform: 'uppercase', padding: '4px 0' }}>{d}</div>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '2px' }}>
        {cells.map((day, i) => {
          ...
          return (
            <div key={dateStr} style={{ height: '56px', border: `1px solid ${theme.border}`, borderRadius: '6px', padding: '4px', overflow: 'hidden', background: theme.bgSurface }}>
              <div style={{ fontSize: '11px', color: theme.textSecondary, marginBottom: '2px' }}>{day}</div>
        ...
        <div style={{ display: 'flex', gap: '12px', marginTop: '10px', flexWrap: 'wrap' }}>
          {Object.entries(typeColorMap).map(([type, idx]) => (
            <div key={type} style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
              <div style={{ width: 10, height: 10, borderRadius: '3px', background: LEAVE_COLORS[idx] }} />
