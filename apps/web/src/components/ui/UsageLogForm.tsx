...
export function UsageLogForm({ open, assetId, assetName, maintenanceDueHours, currentHours = 0, onSubmit, onClose }: Props) {
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px', padding: '20px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
          <Input
            ...
            onChange={(e) => setForm(f => ({ ...f, hours_used: parseFloat(e.target.value) || 0 }))}
            required
          />
          <Input
            label="Mileage (km)"
            ...
            onChange={(e) => setForm(f => ({ ...f, mileage_km: e.target.value ? parseFloat(e.target.value) : undefined }))}
          />
        </div>
        ...
        {maintenanceWarning && (
          <div style={{
            ...
            ⚠ Maintenance will be due at {maintenanceDueHours}h. Current: {currentHours}h → projected: {projectedHours}h
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
          ...
          <Button variant="primary" type="submit" loading={loading}>Save Log</Button>
        </div>
      </form>
    </Modal>
  )
