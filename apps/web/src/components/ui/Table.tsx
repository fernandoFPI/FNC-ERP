  ...
  return (
    ...
    >
      <table style={{
        ...
        <tbody>
          {loading ? (
            Array.from({ length: 3 }).map((_, i) => <SkeletonRow key={i} cols={visibleColumns.length} />)
          ) : data.length === 0 ? (
            <tr>
              <td
                colSpan={visibleColumns.length}
                style={{ padding: '32px 14px', textAlign: 'center', color: theme.textMuted, fontSize: '13px' }}
              >
                {emptyMessage}
              </td>
            </tr>
            ...
            data.map((row, idx) => {
              ...
              <tr
                ...
                onMouseEnter={(e) => { e.currentTarget.style.background = rowStyle.background ? String(rowStyle.background) : theme.tableRowHover }}
                onMouseLeave={(e) => { e.currentTarget.style.background = rowStyle.background ? String(rowStyle.background) : 'transparent' }}
              >
