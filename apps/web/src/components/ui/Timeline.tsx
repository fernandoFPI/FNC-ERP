...
export function Timeline({ events, loading = false }: TimelineProps) {
        ...
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} style={{ display: 'flex', gap: '16px', marginBottom: '20px', alignItems: 'flex-start' }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0 }}>
              ...
              {i < 3 && <div className="skeleton" style={{ width: '1px', height: '40px', marginTop: '4px' }} />}
            </div>
            <div style={{ flex: 1, paddingBottom: '8px' }}>
              <div className="skeleton" style={{ height: '13px', width: '60%', borderRadius: '4px', marginBottom: '6px' }} />
  ...
  if (events.length === 0) {
    return <p style={{ color: theme.textMuted, fontSize: '13px', textAlign: 'center', padding: '24px 0' }}>No activity yet</p>
  }
        ...
        return (
          <div key={event.id} style={{ display: 'flex', gap: '16px', alignItems: 'flex-start' }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0, width: '10px' }}>
              ...
              {i < events.length - 1 && (
                <div style={{ width: '1px', flex: 1, background: theme.border, marginTop: '4px', minHeight: '24px' }} />
              )}
            </div>
            <div style={{ flex: 1, paddingBottom: i < events.length - 1 ? '20px' : '0' }}>
              <p style={{ fontSize: '13px', fontWeight: 500, color: theme.textPrimary, margin: '0 0 2px' }}>{event.title}</p>
              {event.description && (
                <p style={{ fontSize: '12px', color: theme.textSecondary, margin: '0 0 2px' }}>{event.description}</p>
              )}
