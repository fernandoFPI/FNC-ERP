...
export function FileAttachments({
  ...
  React.useEffect(() => {
    ...
    setLoading(true)
    api
      .get<Attachment[]>(`/${entityType}/${entityId}/attachments`)
      .then((r) => {
  ...
  async function handleUpload(file: File) {
    ...
    try {
      const {
        data: { uploadUrl, fileId },
      } = await api.post<{ uploadUrl: string; fileId: string }>('/files/upload-url', {
        filename: file.name,
        mimeType: file.type,
        size: file.size,
        category,
  ...
  async function handleDownload(fileId: string) {
    try {
      const {
        data: { downloadUrl },
      } = await api.get<{ downloadUrl: string }>(`/files/${fileId}/download-url`)
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <p style={{ fontSize: '11px', color: theme.textMuted, margin: 0 }}>
                  {formatBytes(a.size_bytes)} · {a.uploaded_by_email} ·{' '}
