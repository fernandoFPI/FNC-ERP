...
function getPhaseButtons(
  ...
  // On hold — only resume
  if (status === 'on_hold') {
    return has('resume') ? [{ key: 'resume', label: 'Resume Project', variant: 'primary' }] : []
  }
  ...
  switch (phase) {
    case 'enquiry': {
      ...
      if (startKey)
        btns.push({
          ...
          variant: 'primary',
          requiresTimeline: 'rfq',
        })
      if (has('hold'))
        btns.push({ key: 'hold', label: 'Put On Hold', variant: 'warning', requiresReason: true })
      if (has('cancel'))
        btns.push({ key: 'cancel', label: 'Cancel', variant: 'danger', requiresReason: true })
      return btns
    }
    case 'scope_review': {
    ...
    case 'bidding': {
      ...
      if (has('submit'))
        btns.push({
          key: 'submit',
          label: 'Submit Bid to Client',
          variant: 'success',
          requiresTimeline: 'bid',
        })
      if (has('hold'))
        btns.push({ key: 'hold', label: 'Put On Hold', variant: 'warning', requiresReason: true })
      if (has('cancel'))
    ...
    case 'client_approval': {
      ...
      if (isRfq) {
        if (has('approve_rfq'))
          btns.push({ key: 'approve_rfq', label: 'Mark as Awarded', variant: 'success' })
        if (has('reject_rfq'))
          btns.push({
            key: 'reject_rfq',
            label: 'Reject Bid',
        ...
        if (has('approve'))
          btns.push({ key: 'approve', label: 'Mark as Awarded', variant: 'success' })
        if (has('reject_back'))
          btns.push({
            key: 'reject_back',
    ...
    case 'execution': {
      const btns: PhaseButton[] = []
      if (has('complete'))
        btns.push({ key: 'complete', label: 'Close Project', variant: 'success' })
      if (has('hold'))
        btns.push({ key: 'hold', label: 'Put On Hold', variant: 'warning', requiresReason: true })
      if (has('cancel_after_approval'))
        btns.push({
          key: 'cancel_after_approval',
...
export function ProjectStatusBar({
  ...
  const sideColor: Record<string, string> = {
    on_hold: '#f59e0b',
    cancelled: '#ef4444',
    cancelled_after_approval: '#b91c1c',
  }
        ...
        >
          ...
          <button
            ...
            style={{
              padding: '6px 14px',
              borderRadius: '6px',
              background: reason.trim() ? theme.accent : theme.border,
              color: '#fff',
              border: 'none',
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
            <div>
              ...
              <input
                ...
                value={tf.siteVisitDate}
                onChange={setTF('siteVisitDate')}
              />
            </div>
            <div>
              ...
              <input
                type="time"
                style={inputStyle(theme)}
                value={tf.siteVisitTime}
                onChange={setTF('siteVisitTime')}
              />
            </div>
            <div>
              ...
              <input
                type="date"
                style={inputStyle(theme)}
                value={tf.questionDate}
                onChange={setTF('questionDate')}
              />
            </div>
            <div>
              ...
              <input
                type="time"
                style={inputStyle(theme)}
                value={tf.questionTime}
                onChange={setTF('questionTime')}
              />
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
            <div>
              ...
              <input
                type="date"
                style={inputStyle(theme)}
                value={tf.submissionDate}
                onChange={setTF('submissionDate')}
              />
            </div>
            <div>
              ...
              <input
                type="time"
                style={inputStyle(theme)}
                value={tf.submissionTime}
                onChange={setTF('submissionTime')}
              />
            </div>
          </div>
          <div style={{ display: 'flex', gap: '8px' }}>
            <button
              ...
              style={{
                padding: '6px 16px',
                borderRadius: '6px',
                background: tf.submissionDate ? theme.accent : theme.border,
                color: '#fff',
                border: 'none',
                cursor: saving || !tf.submissionDate ? 'not-allowed' : 'pointer',
                fontSize: '13px',
                fontWeight: 500,
              }}
            >
