...
const informationalTours: Record<string, { title: string; steps: DriveStep[] }> = {
  ...
  'dashboard-overview': {
    ...
    steps: [
      { popover: { title: '📊 Dashboard — Walkthrough', description: 'The dashboard is your <strong>command centre</strong> — it shows live summaries from every module in one place. Press <strong>Next →</strong> to explore each section.' } },
      { popover: { title: 'Step 1 — KPI cards', description: 'The top row shows your most critical numbers at a glance:<br/><br/>• <strong>Revenue (MTD)</strong> — invoiced revenue this month<br/>• <strong>Outstanding Payables</strong> — AP invoices awaiting payment<br/>• <strong>POs Pending Approval</strong> — purchase orders in your queue<br/>• <strong>Active Projects</strong> — projects currently in progress<br/><br/>These numbers update automatically as transactions are posted — no need to refresh.' } },
      { popover: { title: 'Step 2 — Quick action tiles', description: 'Below the KPIs are <strong>module tiles</strong> for every section of the system. Click any tile to navigate directly.<br/><br/>Tiles with badges (e.g., "3 pending") indicate items requiring your attention. The Approval Queue badge turns red when POs are overdue.' } },
      { popover: { title: 'Step 3 — Company switcher', description: 'Click your <strong>company name</strong> in the top-left of the sidebar at any time to switch between legal entities.<br/><br/>Your role, permissions, and data all change to reflect the selected company. The active company name is always visible in the sidebar header.' } },
      { popover: { title: '✅ Dashboard Tour Complete', description: 'Use the dashboard as your starting point each day — it surfaces what needs attention without having to open individual modules.<br/><br/>Click <strong>Done ✓</strong> to exit.' } },
    ],
  },
  ...
  'accounts-payable': {
    ...
    steps: [
      { popover: { title: '🧾 Accounts Payable — Walkthrough', description: 'Accounts Payable tracks money you owe to vendors. Every vendor invoice goes through AP before generating a payment. Press <strong>Next →</strong> to continue.' } },
      { popover: { title: 'Step 1 — Record a vendor invoice', description: 'Go to <strong>Finance → Accounts Payable → New Invoice</strong>.<br/><br/>Fill in:<br/>• <strong>Vendor</strong> — must be in your vendor master<br/>• <strong>Invoice date &amp; due date</strong> — due date is calculated from payment terms<br/>• <strong>Lines</strong> — each line needs a GL account, description, and amount<br/>• <strong>PO Reference</strong> (optional) — links to an existing purchase order for 3-way matching' } },
      { popover: { title: 'Step 2 — 3-way matching', description: 'When a PO Reference is set, the system verifies the invoice against:<br/><br/>1. <strong>PO quantity</strong> — what was ordered<br/>2. <strong>Goods receipt quantity</strong> — what was actually received<br/>3. <strong>Invoice quantity</strong> — what the vendor is charging for<br/><br/>Mismatches are flagged before posting. This prevents overpayment and vendor fraud.' } },
      { popover: { title: 'Step 3 — Post the invoice', description: 'Review all lines and click <strong>Post</strong>. Posting:<br/>• Moves the invoice from Draft to Posted<br/>• Creates the journal entry in the GL (Dr: Expense account, Cr: Accounts Payable)<br/>• Makes it available for payment voucher generation<br/><br/>Posted invoices cannot be edited. To correct an error, create a credit note.' } },
      { popover: { title: 'Step 4 — Generate a payment voucher', description: 'From the posted invoice, click <strong>Generate PV</strong>.<br/><br/>The payment voucher is pre-filled with the vendor\'s bank account and the invoice amount. After the bank transfer is made:<br/>1. Upload the transfer confirmation document<br/>2. Click <strong>Mark as Paid</strong><br/><br/>The invoice status changes to Paid and the PV is locked.' } },
      { popover: { title: '✅ AP Tour Complete', description: 'AP workflow: <strong>New Invoice → Post → Generate PV → Bank Transfer → Mark Paid</strong>.<br/><br/>View all pending invoices and their due dates in the AP Dashboard. Use the <strong>Overdue</strong> filter to prioritise urgent payments.<br/><br/>Click <strong>Done ✓</strong> to exit.' } },
    ],
  },
  ...
  'accounts-receivable': {
    ...
    steps: [
      { popover: { title: '💰 Accounts Receivable — Walkthrough', description: 'Accounts Receivable tracks money owed to you by customers. Invoices are generated from project contracts or created manually, then posted and collected. Press <strong>Next →</strong> to continue.' } },
      { popover: { title: 'Step 1 — Generate invoice from contract', description: 'The fastest way to invoice is from a project contract:<br/><br/>1. Open <strong>Projects → [Your Project] → Contracts tab</strong><br/>2. Select the contract and mark milestones as reached<br/>3. Click <strong>Generate Invoice</strong><br/><br/>The AR invoice is pre-filled with the customer, line items, and amounts. Edit if needed, then post.' } },
      { popover: { title: 'Step 2 — Manual AR invoice', description: 'For ad-hoc invoices not tied to a contract:<br/><br/>Go to <strong>Finance → Accounts Receivable → New Invoice</strong>.<br/><br/>Select the customer, set the invoice date and due date, and add line items. Each line needs a revenue GL account and amount. Post when complete.' } },
      { popover: { title: 'Step 3 — Send to customer', description: 'After posting, click <strong>Send</strong> to email the invoice PDF directly to the customer\'s email address (taken from the customer record).<br/><br/>The PDF uses your company letterhead configured in Settings → Company.' } },
      { popover: { title: 'Step 4 — Record a payment', description: 'When the customer pays, open the invoice and click <strong>Record Payment</strong>.<br/><br/>Enter:<br/>• <strong>Date received</strong><br/>• <strong>Amount</strong> — partial payments are supported<br/>• <strong>Bank account</strong> — where the money was deposited<br/><br/>The invoice shows a running <em>Amount Paid</em> and <em>Balance Due</em>.' } },
      { popover: { title: 'Step 5 — Aging report', description: 'Monitor overdue invoices under <strong>Finance → AR → Aging</strong>.<br/><br/>Invoices are grouped by:<br/>• <strong>Current</strong> — not yet due<br/>• <strong>1–30 days</strong> overdue<br/>• <strong>31–60 days</strong> overdue<br/>• <strong>60+ days</strong> overdue<br/><br/>Use this view to prioritise collection calls and flag credit-risk customers.<br/><br/>Click <strong>Done ✓</strong> to exit.' } },
    ],
  },
  ...
  'expense-claims': {
    ...
    steps: [
      { popover: { title: '🧳 Expense Claims — Walkthrough', description: 'Expense Claims lets employees request reimbursement for business expenses they paid out of pocket. Finance reviews, approves, and reimburses via payment voucher. Press <strong>Next →</strong> to continue.' } },
      { popover: { title: 'Step 1 — Submit a claim', description: 'Go to <strong>Finance → Expense Claims → New Claim</strong>.<br/><br/>Add each expense as a line:<br/>• <strong>Date</strong> of the expense<br/>• <strong>Category</strong> (Travel, Meals, Office Supplies, etc.) — determines the GL account<br/>• <strong>Amount</strong> and <strong>currency</strong><br/>• <strong>Receipt attachment</strong> — required above the policy threshold<br/><br/>Click <strong>Submit</strong> when complete.' } },
      { popover: { title: 'Step 2 — Approval workflow', description: 'After submission, the claim routes through two approval stages:<br/><br/>1. <strong>Line Manager approval</strong> — the employee\'s direct manager reviews and approves<br/>2. <strong>Finance approval</strong> — Finance verifies receipt amounts and GL account coding<br/><br/>The employee receives a notification at each stage. If rejected, a reason is required and the claim returns to Draft for revision.' } },
      { popover: { title: 'Step 3 — Post and reimburse', description: 'Once Finance approves, click <strong>Post</strong>.<br/><br/>Posting creates a journal entry:<br/>• Dr: Expense account (per line category)<br/>• Cr: Employee Payable account<br/><br/>Then generate a <strong>Payment Voucher</strong> from the claim to pay the employee via bank transfer. Upload the transfer confirmation and mark it Paid.' } },
      { popover: { title: '✅ Expense Claims Tour Complete', description: 'Expense categories and GL account mappings are configured in <strong>Finance → Settings → Expense Categories</strong>.<br/><br/>The policy threshold (above which receipts are required) is set in <strong>Settings → HR → Expense Policy</strong>.<br/><br/>Click <strong>Done ✓</strong> to exit.' } },
    ],
  },
  ...
  'payment-vouchers': {
    ...
    steps: [
      { popover: { title: '📋 Payment Vouchers — Walkthrough', description: 'A Payment Voucher (PV) is a formal authorisation document created for every bank payment. It links the AP invoice to the bank transfer record. Press <strong>Next →</strong> to continue.' } },
      { popover: { title: 'Step 1 — Generate from an AP invoice', description: 'PVs are generated from <strong>posted</strong> AP invoices only. Open the invoice and click <strong>Generate PV</strong>.<br/><br/>The voucher pre-fills:<br/>• Vendor name and bank account details<br/>• Invoice number and amount<br/>• Currency and payment date<br/><br/>Review and save. The PV is now in <strong>Pending</strong> status.' } },
      { popover: { title: 'Step 2 — Execute the payment', description: 'Make the bank transfer using your banking system outside the ERP.<br/><br/>Then return to the PV and:<br/>1. Enter the actual <strong>transfer date</strong> and <strong>reference number</strong><br/>2. Upload the <strong>bank transfer confirmation</strong> (PDF or image)<br/>3. Click <strong>Mark as Paid</strong><br/><br/>The linked AP invoice status changes to Paid automatically.' } },
      { popover: { title: 'Step 3 — Print the PV', description: 'Click <strong>PDF</strong> to download a formatted payment voucher with:<br/>• Company letterhead and stamp<br/>• Vendor details and payment amount<br/>• Invoice reference number<br/>• Authorisation signatures area<br/><br/>The PDF is ready to file or send to the vendor as payment notification.' } },
      { popover: { title: '✅ Payment Vouchers Tour Complete', description: 'PVs can only be generated for <strong>posted</strong> invoices (not Draft).<br/><br/>The voucher stamp template is configured in <strong>Settings → Company → PV Template</strong>.<br/><br/>View all PVs in <strong>Finance → Payment Vouchers</strong>, filterable by vendor, date range, or payment status.<br/><br/>Click <strong>Done ✓</strong> to exit.' } },
    ],
  },
  ...
  'payment-terms': {
    title: 'Payment Terms',
    steps: [
      { popover: { title: '📅 Payment Terms — Walkthrough', description: 'Payment Terms define when invoices are due and how multi-instalment payments are structured. They are set on vendors and customers and auto-apply to all new invoices. Press <strong>Next →</strong> to continue.' } },
      { popover: { title: 'Step 1 — Create payment terms', description: 'Go to <strong>Finance → Payment Terms → New Terms</strong>.<br/><br/>Name it descriptively (e.g., "Net 30", "50/50 Split", "Immediate").<br/><br/>Add instalment lines:<br/>• <strong>% of balance</strong> due<br/>• <strong>When</strong> it is due (e.g., 0 days after invoice = immediate, 30 days after = Net 30)<br/><br/>Example for 50/50: Line 1 = 50% on invoice; Line 2 = 50% after 30 days.' } },
      { popover: { title: 'Step 2 — Apply to vendors and customers', description: 'Set the default payment terms on a vendor or customer record. When a new invoice is created for that party, the due date is calculated automatically.<br/><br/>You can always <strong>override</strong> the terms on a specific invoice if the vendor agreed to different terms for that transaction.' } },
      { popover: { title: 'Step 3 — Retention tracking', description: 'For construction contracts with a <strong>retention clause</strong> (a % held back until project completion):<br/><br/>Go to <strong>Finance → Retention</strong>.<br/><br/>Retention amounts are tracked separately from the invoice balance. When the retention release milestone is reached, generate a retention invoice to collect the withheld amount.' } },
      { popover: { title: '✅ Payment Terms Tour Complete', description: 'Payment terms directly affect:<br/>• AP aging (overdue date calculations)<br/>• AR collections (when to send reminders)<br/>• Cash flow forecasting (when money is expected in or out)<br/><br/>Review your terms setup before going live. Click <strong>Done ✓</strong> to exit.' } },
    ],
  },
  ...
  'approval-queue': {
    ...
    steps: [
      { popover: { title: '✅ Approval Queue — Walkthrough', description: 'The Approval Queue shows all Purchase Orders awaiting your sign-off. POs route here automatically after submission based on your company\'s approval thresholds. Press <strong>Next →</strong> to continue.' } },
      { popover: { title: 'Step 1 — Review a PO', description: 'Each row in the queue shows:<br/>• PO number, vendor, and total amount<br/>• Requester and submission date<br/>• Days waiting<br/><br/>Click a PO to open the full detail view. Review:<br/>• <strong>Line items</strong> — what is being purchased and at what price<br/>• <strong>Vendor</strong> — is this an approved supplier?<br/>• <strong>Delivery date</strong> — is the timeline realistic?<br/>• <strong>Attached quotes or justification</strong>' } },
      { popover: { title: 'Step 2 — Approve', description: 'Click <strong>Approve</strong> on the PO detail page.<br/><br/>If your approval level is the <em>final</em> level, the PO moves to <strong>Approved → Ready to Issue</strong>.<br/><br/>If there is a higher approval tier configured, it moves to <strong>Pending Next Level</strong> and routes to the next approver automatically.' } },
      { popover: { title: 'Step 3 — Reject', description: 'Click <strong>Reject</strong> and type a reason. The rejection reason is <strong>mandatory</strong> — it is shown to the requester in their notification.<br/><br/>The PO returns to <strong>Draft</strong> status. The requester can revise the PO and resubmit. Rejection history is preserved in the audit log.' } },
      { popover: { title: '✅ Approval Queue Tour Complete', description: 'The sidebar badge shows your pending count — keep it at zero.<br/><br/>Approval thresholds and routing rules are configured in <strong>Settings → Procurement → Approval Workflow</strong>.<br/><br/>For bulk approvals (e.g., many small POs), use the <strong>Select All → Approve Selected</strong> action if available.<br/><br/>Click <strong>Done ✓</strong> to exit.' } },
    ],
  },
  ...
  'hr-attendance': {
    title: 'Attendance',
    steps: [
      { popover: { title: '🕐 Attendance — Walkthrough', description: 'The Attendance module shows when employees punch in and out, validates geofence compliance, and calculates monthly KPIs. Press <strong>Next →</strong> to continue.' } },
      { popover: { title: 'Step 1 — Monthly calendar view', description: 'Open <strong>HR → Attendance → [Select Employee]</strong>.<br/><br/>The monthly calendar colour-codes each day:<br/>• 🟢 <strong>Green</strong> — present (punch-in recorded)<br/>• 🔴 <strong>Red</strong> — absent (no punch on a working day)<br/>• 🟡 <strong>Yellow</strong> — on approved leave<br/>• ⬜ <strong>Grey</strong> — weekend or holiday<br/><br/>Use the month/year selector at the top to navigate between periods.' } },
      { popover: { title: 'Step 2 — Day detail', description: 'Click any day on the calendar to open the punch detail:<br/><br/>• <strong>Punch-in time</strong> and <strong>Punch-out time</strong><br/>• <strong>Geofence status</strong> — Valid / Outside geofence<br/>• <strong>Distance from work location</strong> in metres<br/>• <strong>Total hours</strong> worked<br/>• <strong>Overtime hours</strong> (above the shift end time)<br/><br/>Punches outside the geofence are recorded but flagged in orange — they are included in totals but marked for review.' } },
      { popover: { title: 'Step 3 — Monthly KPI summary', description: 'The KPI row above the calendar shows the month\'s totals for the selected employee:<br/><br/>• <strong>Days Present</strong><br/>• <strong>Days Absent</strong><br/>• <strong>Total Hours</strong> worked<br/>• <strong>Overtime Hours</strong><br/>• <strong>Leave Days</strong> taken<br/><br/>These numbers flow directly into the <strong>Payroll</strong> calculation for the same period.' } },
      { popover: { title: 'Step 4 — Punch history table', description: 'Go to <strong>HR → Attendance → Punch History</strong> for a tabular view of all punch records with:<br/>• Employee name<br/>• Date and exact punch time<br/>• Punch type (In / Out)<br/>• Geofence result<br/><br/>Filter by date range, department, or employee. Export to CSV for manual review or audits.' } },
      { popover: { title: '✅ Attendance Tour Complete', description: 'Employees punch in and out exclusively via the <strong>mobile app</strong>. The app captures location automatically — no manual entry is allowed.<br/><br/>Geofence zones are configured per Work Location in <strong>HR → Settings → Work Locations</strong>.<br/><br/>Click <strong>Done ✓</strong> to exit.' } },
    ],
  },
  ...
  'hr-overtime': {
    ...
    steps: [
      { popover: { title: '⏰ Overtime — Walkthrough', description: 'The Overtime module shows overtime hours logged via the mobile app and lets managers approve or reject them before they flow into payroll. Press <strong>Next →</strong> to continue.' } },
      { popover: { title: 'Step 1 — Pending requests', description: 'Go to <strong>HR → Overtime</strong>.<br/><br/>Each card in the list shows:<br/>• <strong>Employee name</strong> and department<br/>• <strong>Date</strong> of the overtime<br/>• <strong>Regular hours</strong> worked that day<br/>• <strong>Overtime hours</strong> — the additional time above the shift end<br/><br/>Cards are sorted with the oldest first to prevent requests from expiring unreviewed.' } },
      { popover: { title: 'Step 2 — Approve or reject', description: 'Click a request to view full detail.<br/><br/>• <strong>Approve</strong> — the overtime hours are locked and included in the next payroll run. The employee is notified.<br/>• <strong>Reject</strong> — a mandatory reason field appears. The employee is notified with the reason. The hours are not included in payroll.<br/><br/>Rejected overtime can be resubmitted by the employee if the reason was an error.' } },
      { popover: { title: 'Step 3 — Bulk approve', description: 'When many requests are pending (e.g., after a busy project week), use <strong>Approve All</strong> to process every request in the list at once.<br/><br/>⚠️ Review the list carefully before bulk approving — once approved, overtime cannot be recalled without creating a payroll adjustment.' } },
      { popover: { title: '✅ Overtime Tour Complete', description: 'Overtime multipliers (1.5×, 2×) are set per-employee in the <strong>shift configuration</strong>.<br/><br/>Approved overtime automatically feeds into the <strong>Payroll</strong> calculation for the corresponding month — no manual data entry needed.<br/><br/>Click <strong>Done ✓</strong> to exit.' } },
    ],
  },
  ...
  'reporting': {
    ...
    steps: [
      { popover: { title: '📊 Financial Reports — Walkthrough', description: 'The Reports module generates the four core financial statements directly from your posted journal data — no manual exports or Excel work required. Press <strong>Next →</strong> to continue.' } },
      { popover: { title: 'Step 1 — Trial Balance', description: 'Go to <strong>Finance → Reports → Trial Balance</strong>.<br/><br/>The Trial Balance shows <strong>every GL account</strong> with:<br/>• Opening balance<br/>• Debit movements in the period<br/>• Credit movements in the period<br/>• Closing balance<br/><br/>The total debit column must equal the total credit column. Use this to verify data integrity before generating other statements. Filter by date range and company.' } },
      { popover: { title: 'Step 2 — Profit & Loss', description: 'Go to <strong>Finance → Reports → Profit &amp; Loss</strong>.<br/><br/>Shows <strong>Revenue − Expenses</strong> for the selected period:<br/>• Revenue lines (credit-balance accounts)<br/>• Direct costs and gross profit<br/>• Operating expenses and operating profit<br/>• Other income/expense<br/>• Net profit / loss<br/><br/>Click any line amount to <strong>drill down</strong> to the underlying journal entries.' } },
      { popover: { title: 'Step 3 — Balance Sheet', description: 'Go to <strong>Finance → Reports → Balance Sheet</strong>.<br/><br/>Shows your financial position as of a specific date:<br/>• <strong>Assets</strong>: Current assets (cash, receivables, inventory) + Non-current (fixed assets)<br/>• <strong>Liabilities</strong>: Current (AP, accruals) + Non-current (long-term debt)<br/>• <strong>Equity</strong>: Share capital + Retained earnings<br/><br/>Assets must always equal Liabilities + Equity. Any imbalance indicates unposted or miscoded journals.' } },
      { popover: { title: 'Step 4 — Export and period comparison', description: 'Every report has an <strong>Export</strong> button (top-right) for PDF and CSV formats.<br/><br/>For period comparison (this month vs last month, or YTD vs prior year):<br/>• Use the <strong>Comparison Period</strong> toggle in the report header<br/>• Select the comparison period from the dropdown<br/><br/>The report adds a variance column showing the absolute and percentage change.' } },
      { popover: { title: '✅ Reports Tour Complete', description: '⚠️ Reports only include <strong>posted</strong> journals. Draft journals are always excluded — post all journals before generating period-end reports.<br/><br/>Lock the accounting period in <strong>Finance → Periods</strong> after closing it to prevent backdated changes.<br/><br/>Click <strong>Done ✓</strong> to exit.' } },
    ],
  },
