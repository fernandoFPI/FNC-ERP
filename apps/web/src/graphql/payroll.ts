...
export const PAYROLL_RUNS_QUERY = gql`
  query PayrollRuns($status: String) {
    payrollRuns(status: $status) {
      ...
      total_gross total_net total_deductions created_at
    }
  }
`
...
export const PAYSLIP_QUERY = gql`
  query Payslip($id: ID!) {
    payslip(id: $id) {
      id payroll_run_id employee_id employee_name employee_number
      base_salary housing_allowance transport_allowance other_allowances
      overtime_hours overtime_pay gross_salary
      income_tax social_security other_deductions net_salary
      currency_code working_days absent_days leave_days
    }
  }
`
...
export const MY_PAYSLIPS_QUERY = gql`
  query MyPayslips {
    myPayslips {
      id payroll_run_id employee_id employee_name employee_number
      gross_salary net_salary income_tax social_security
      currency_code working_days absent_days leave_days overtime_hours
    }
  }
`
