    ...
    <div style={{ padding: '24px' }}>
      ...
      <Card padding="none">
        {loading ? (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              <thead>
                <tr style={{ background: theme.bgSurface }}>
                  {['Currency', 'Open AR', 'Open AP', 'Net Exposure', 'FX Rate', 'IQD Equivalent'].map((h, i) => (
                    <th key={i} style={{ padding: '10px 14px', textAlign: i === 0 ? 'left' : 'right', fontSize: '10px', fontWeight: 600, color: theme.textMuted, textTransform: 'uppercase', letterSpacing: '0.06em', borderBottom: `1px solid ${theme.border}`, whiteSpace: 'nowrap' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {d.rows.map((row, i) => (
                  <tr key={i} style={{ borderBottom: `1px solid ${theme.tableBorder}` }}
                    ...
                    <td style={{ padding: '12px 14px', color: theme.textPrimary, fontWeight: 600, fontSize: '14px' }}>{row.currency}</td>
                    ...
                    <td style={{ padding: '12px 14px', textAlign: 'right', color: theme.textSecondary, fontFamily: 'ui-monospace, monospace', fontSize: '12px' }}>
          ...
          <EmptyState title="No FX exposure data" message="Select a date and apply to load exposure report." />
        )}
      </Card>
    </div>
  )
