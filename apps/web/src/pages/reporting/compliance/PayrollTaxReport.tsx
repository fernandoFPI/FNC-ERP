...
export default function PayrollTaxReport() {
  ...
  const rows = (data?.payrollTaxReport.rows ?? []).filter(r =>
    !search ||
    ...
    <div style={{ padding: '24px' }}>
      ...
      {rows.length > 0 && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '12px', marginBottom: '20px' }}>
          <Card padding="sm">
            <p style={{ fontSize: '10px', color: theme.textMuted, marginBottom: '4px' }}>Total Gross Pay</p>
            <AmountDisplay amount={totalGross} currency="IQD" size="md" />
          </Card>
          <Card padding="sm">
            <p style={{ fontSize: '10px', color: theme.textMuted, marginBottom: '4px' }}>Income Tax Withheld</p>
            <AmountDisplay amount={totalIncomeTax} currency="IQD" size="md" />
          </Card>
          <Card padding="sm">
            <p style={{ fontSize: '10px', color: theme.textMuted, marginBottom: '4px' }}>Social Security</p>
      ...
      <Card padding="none">
        {loading ? (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              <thead>
                <tr style={{ background: theme.bgSurface }}>
                  {['#', 'Employee', 'Period', 'Gross Pay', 'Taxable Income', 'Income Tax', 'Social Security', 'Net Pay'].map((h, i) => (
              ...
              <tbody>
                {rows.map((row, i) => (
                  <tr key={i} style={{ borderBottom: `1px solid ${theme.tableBorder}` }}
                    ...
                    <td style={{ padding: '12px 14px', textAlign: 'right' }}><AmountDisplay amount={row.grossPay} currency="IQD" size="sm" /></td>
                    <td style={{ padding: '12px 14px', textAlign: 'right' }}><AmountDisplay amount={row.taxableIncome} currency="IQD" size="sm" /></td>
                    <td style={{ padding: '12px 14px', textAlign: 'right', color: theme.warning }}><AmountDisplay amount={row.incomeTaxWithheld} currency="IQD" size="sm" /></td>
                    <td style={{ padding: '12px 14px', textAlign: 'right', color: theme.info }}><AmountDisplay amount={row.socialSecurity} currency="IQD" size="sm" /></td>
                    <td style={{ padding: '12px 14px', textAlign: 'right', fontWeight: 500 }}><AmountDisplay amount={row.netPay} currency="IQD" size="sm" /></td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <EmptyState title="No payroll tax data" message="Adjust the date range to load payroll tax records." />
        )}
      </Card>
    </div>
  )
