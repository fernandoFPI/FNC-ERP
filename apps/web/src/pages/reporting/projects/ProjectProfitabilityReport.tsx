...
function marginColor(pct: number, theme: { success: string; warning: string; danger: string; textSecondary: string }) {
...
export default function ProjectProfitabilityReport() {
  ...
  const { data, loading, refetch } = useQuery<ProjectProfitabilityData>(PROJECT_PROFITABILITY_QUERY, {
    variables: { fromDate, toDate, status: statusFilter ? [statusFilter] : undefined },
  })

  const rows = (data?.projectProfitabilityReport ?? []).filter(r =>
    ...
    <div style={{ padding: '24px' }}>
      ...
      <Card padding="none">
        {loading ? (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              <thead>
                <tr style={{ background: theme.bgSurface }}>
                  {['Code', 'Project', 'Company', 'Type', 'Budget', 'Actual Cost', 'Revenue', 'Margin', 'Margin %', 'Status', ''].map((h, i) => (
                    <th key={i} style={{ padding: '10px 14px', textAlign: i >= 4 && i <= 8 ? 'right' : 'left', fontSize: '10px', fontWeight: 600, color: theme.textMuted, textTransform: 'uppercase', letterSpacing: '0.06em', borderBottom: `1px solid ${theme.border}`, whiteSpace: 'nowrap' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {rows.map((row) => (
                  <>
                    <tr key={row.id} style={{ borderBottom: `1px solid ${theme.tableBorder}` }}
                      ...
                      <td style={{ padding: '12px 14px', color: theme.textMuted, fontSize: '12px' }}>{row.code}</td>
                      <td style={{ padding: '12px 14px', color: theme.textPrimary, fontWeight: 500 }}>{row.name}</td>
                      <td style={{ padding: '12px 14px', color: theme.textSecondary }}>{row.companyName}</td>
                      <td style={{ padding: '12px 14px' }}>
                        <Badge variant="neutral" size="sm">{row.projectType}</Badge>
                      </td>
                      ...
                      <td style={{ padding: '12px 14px', textAlign: 'right' }}>
                        <span style={{ fontWeight: 600, color: marginColor(row.marginPct, theme), fontFamily: 'ui-monospace, monospace' }}>
                    ...
                    {expandedId === row.id && (
                      <tr key={`${row.id}-expanded`} style={{ background: theme.bgSurfaceHover }}>
                        <td colSpan={11} style={{ padding: '16px 24px' }}>
                          <p style={{ fontSize: '11px', fontWeight: 600, color: theme.textMuted, marginBottom: '10px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Cost Breakdown</p>
                          <div style={{ display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
                            {row.costBreakdown.map(cb => (
                              <div key={cb.key} style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
          ...
          <EmptyState title="No projects found" message="Adjust filters to load project profitability data." />
        )}
      </Card>
    </div>
  )
