...
function ScatterTooltip({
  ...
  active?: boolean
  payload?: { payload: ProjectScatter }[]
}) {
  ...
  return (
    ...
    >
      ...
      <p style={{ color: 'var(--text-muted)', margin: '0 0 2px' }}>
        Budget: {formatNumber(d.budget)}
      </p>
      <p style={{ color: 'var(--text-muted)', margin: '0 0 2px' }}>
        Actual: {formatNumber(d.actualCost)}
      </p>
            ...
            <Card key={entity.companyId} padding="md">
              ...
              >
                ...
                <Badge variant={entity.netThisMonth >= 0 ? 'success' : 'danger'} size="sm">
                  {entity.netThisMonth >= 0 ? '+' : ''}
                  {(entity.netThisMonth / 1000).toFixed(0)}K
                </Badge>
              </div>
              <div
                style={{
              ...
              <ScatterChart margin={{ top: 10, right: 30, left: 0, bottom: 30 }}>
                ...
                <Tooltip
                  content={(props) => (
                    <ScatterTooltip
                      active={props.active}
                      payload={props.payload as { payload: ProjectScatter }[]}
                    />
                  )}
                />
          ...
          <div style={{ display: 'flex', gap: '16px', marginTop: '8px', flexWrap: 'wrap' }}>
            {Object.entries(STATUS_COLORS).map(([status, color]) => (
              <div key={status} style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                ...
                />
                <span style={{ fontSize: 11, color: theme.textMuted, textTransform: 'capitalize' }}>
                  {status.replace('_', ' ')}
                </span>
              </div>
            ))}
