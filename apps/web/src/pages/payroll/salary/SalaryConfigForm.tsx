...
const CURRENCIES = [{ value: 'IQD', label: 'IQD' }, { value: 'USD', label: 'USD' }, { value: 'EUR', label: 'EUR' }, { value: 'GBP', label: 'GBP' }]

const emptyForm = {
  base_salary: '0', currency_code: 'IQD',
  housing_allowance: '0', transport_allowance: '0', other_allowances: '0',
  income_tax_pct: '0', social_security_pct: '0',
...
export default function SalaryConfigForm() {
  ...
  const { data: empData } = useQuery(EMPLOYEE_QUERY, { variables: { id: employeeId }, skip: !employeeId })
  const { data: salaryData } = useQuery(EMPLOYEE_SALARY_CONFIG_QUERY, { variables: { employee_id: employeeId }, skip: !employeeId })
  ...
  return (
    <div style={{ padding: '24px', margin: '0 auto', maxWidth: '900px' }}>
      <PageHeader title={`Update Salary${emp ? ` — ${emp.first_name} ${emp.last_name}` : ''}`} backPath={`/hr/employees/${employeeId}`} />

      <form onSubmit={handleSubmit}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', marginTop: '20px' }}>
          <Card style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <div style={{ fontWeight: 600, color: theme.textPrimary, marginBottom: '4px' }}>Base Salary</div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '12px' }}>
              <Input label="Base salary" type="number" min="0" value={form.base_salary} onChange={field('base_salary')} required />
              <Select label="Currency" value={form.currency_code} onChange={field('currency_code')} options={CURRENCIES} />
              <Input label="Effective from" type="date" value={form.effective_from} onChange={field('effective_from')} required />
            </div>
          </Card>

          <Card style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <div style={{ fontWeight: 600, color: theme.textPrimary, marginBottom: '4px' }}>Allowances</div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '12px' }}>
              <Input label="Housing allowance" type="number" min="0" value={form.housing_allowance} onChange={field('housing_allowance')} />
              <Input label="Transport allowance" type="number" min="0" value={form.transport_allowance} onChange={field('transport_allowance')} />
              <Input label="Other allowances" type="number" min="0" value={form.other_allowances} onChange={field('other_allowances')} />
            </div>
          </Card>

          <Card style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <div style={{ fontWeight: 600, color: theme.textPrimary, marginBottom: '4px' }}>Deductions (%)</div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '12px' }}>
              <Input label="Income tax %" type="number" min="0" max="100" step="0.1" value={form.income_tax_pct} onChange={field('income_tax_pct')} />
              <Input label="Social security %" type="number" min="0" max="100" step="0.1" value={form.social_security_pct} onChange={field('social_security_pct')} />
            </div>
          </Card>

          <Card style={{ padding: '20px', background: theme.bgSurface }}>
            <div style={{ fontWeight: 600, color: theme.textPrimary, marginBottom: '12px' }}>Estimated net pay</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              {[
                { label: 'Gross pay', value: gross, bold: false },
                { label: `Income tax (${form.income_tax_pct}%)`, value: -taxAmount, bold: false },
                { label: `Social security (${form.social_security_pct}%)`, value: -ssAmount, bold: false },
          ...
          <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
            <Button variant="ghost" type="button" onClick={() => navigate(`/hr/employees/${employeeId}`)}>Cancel</Button>
            <Button variant="primary" type="submit" loading={loading}>Update salary</Button>
          </div>
        </div>
      </form>
    </div>
