...
export default function ProductsPage() {
  ...
  const bomProductIds = useMemo<Set<string>>(() => {
    const boms: Array<{ finished_product_id: string }> = bomsData?.boms ?? []
    return new Set(boms.map((b) => b.finished_product_id))
  }, [bomsData])
  const lowStockCount = products.filter((p) => p.reorder_point && p.qty_on_hand && parseFloat(p.qty_on_hand) < parseFloat(p.reorder_point)).length
  const filtered = products.filter((p) => {
    ...
    if (showLowStock) {
      const isLow = p.reorder_point && p.qty_on_hand && parseFloat(p.qty_on_hand) < parseFloat(p.reorder_point)
  ...
  const columns: Column<Product>[] = [
    {
      key: 'sku',
      header: 'SKU',
      render: (p) => <span style={{ fontFamily: 'monospace', color: theme.accent, fontSize: '13px' }}>{p.sku}</span>,
    },
    {
      key: 'name',
      header: 'Name',
      render: (p) => (
        <div>
          ...
          {p.sub_category
            ...
            : p.category && <div style={{ color: theme.textMuted, fontSize: '11px' }}>{p.category}</div>}
        </div>
      ),
    },
    { key: 'uom', header: 'UOM', render: (p) => <Badge variant="neutral">{p.uom}</Badge> },
    { key: 'valuation_method', header: 'Valuation', render: (p) => <Badge variant="info">{p.valuation_method}</Badge> },
    {
      key: 'qty_on_hand',
      header: 'On Hand',
      render: (p) => {
        ...
        return (
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span style={{ fontFamily: 'monospace', color: isLow ? '#dc2626' : theme.textPrimary, fontWeight: isLow ? 700 : 400 }}>
              {qty.toLocaleString()}
            </span>
    ...
    { key: 'average_cost', header: 'Avg Cost', render: (p) => <span style={{ fontFamily: 'monospace', color: theme.textSecondary }}>{parseFloat(p.average_cost).toLocaleString()}</span> },
    { key: 'is_active', header: 'Status', render: (p) => <Badge variant={p.is_active ? 'success' : 'neutral'}>{p.is_active ? 'Active' : 'Inactive'}</Badge> },
  ]
  ...
  return (
    <div style={{ padding: '24px', margin: '0 auto', maxWidth: '1400px' }}>
      <PageHeader
        ...
        actions={
          <Button variant="primary" size="sm" onClick={() => navigate('/inventory/products/new')}>
            New Product
          </Button>
        }
      />
      ...
      {lowStockCount > 0 && (
        <div style={{ marginTop: '16px', marginBottom: '4px', padding: '14px 18px', borderRadius: '8px', background: '#fef2f2', border: '1px solid #fca5a5', display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <button onClick={() => setShowLowStock(true)} style={{ marginLeft: 'auto', padding: '5px 12px', borderRadius: '6px', border: '1px solid #dc2626', background: 'transparent', color: '#dc2626', fontSize: '12px', fontWeight: 600, cursor: 'pointer' }}>
            Show only
          </button>
        </div>
      )}
      ...
      <Card style={{ marginTop: '8px' }}>
        <FilterBar
          ...
          filters={[
            { key: 'category', label: 'Category', value: categoryFilter, options: CATEGORY_OPTIONS, onChange: (v) => { setCategoryFilter(v); if (v !== 'raw_material') setSubCategoryFilter('') } },
            ...(categoryFilter === 'raw_material' ? [{ key: 'sub_category', label: 'Store', value: subCategoryFilter, options: RAW_MATERIAL_SUB_CATEGORIES, onChange: setSubCategoryFilter }] : []),
          ]}
