...
import {
  ...
  CREATE_MATERIAL_ISSUE, ADD_MATERIAL_ISSUE_LINE, DELETE_MATERIAL_ISSUE_LINE,
  ISSUE_MATERIAL_ISSUE, CANCEL_MATERIAL_ISSUE,
  PROJECTS_QUERY,
...
type MILine = {
  id: string; productId: string; productName: string | null; poLineId: string | null
  qtyIssued: number; unitCost: number; totalCost: number; isInvoiced: boolean
}
type MI = {
  id: string; issueNumber: string; issueDate: string; status: string
  notes: string | null; poId: string | null; poNumber: string | null
  projectCode: string | null; projectName: string | null
  issuedByName: string | null; createdAt: string; lines: MILine[]
}
type Product = { id: string; name: string; sku: string; average_cost: number | null; uom: string | null }
type PendingLine = { key: number; productId: string; productName: string; sku: string; qty: string; unitCost: string }
...
export default function StoreOutPage() {
  ...
  const projects = (projectsData?.projects?.data ?? []) as Array<{ id: string; code: string; name: string }>
  const products = (productsData?.products ?? []) as Product[]
  type POOption  = { id: string; po_number: string; vendor_name: string; status: string }
  ...
  const poOptions      = [
    ...
    ...projectPOs.map(po => ({ value: po.id, label: po.vendor_name || po.po_number, sublabel: po.po_number })),
  ]
  ...
  const [addLine,     { loading: addingLine }] = useMutation(ADD_MATERIAL_ISSUE_LINE, {
    onCompleted: () => {
      ...
      setDraftProductId(''); setDraftQty(''); setDraftUnit('')
      void refetch()
    },
  ...
  const [issueMI, { loading: issuing }] = useMutation(ISSUE_MATERIAL_ISSUE, {
    ...
    onError: (e) => { addToast({ type: 'error', message: e.message }); setConfirmIssue(null) },
  })
  const [cancelMI, { loading: cancelling }] = useMutation(CANCEL_MATERIAL_ISSUE, {
    ...
    onError: (e) => { addToast({ type: 'error', message: e.message }); setConfirmCancel(null) },
  })

  function resetModal() {
    setShowModal(false)
    setFormDate(new Date().toISOString().slice(0, 10))
    setFormNotes(''); setFormProjectId(''); setFormPoId('')
    setPendingLines([])
    setNewProductId(''); setNewQty(''); setNewUnit('')
  }
  ...
  async function handleCreate() {
    ...
    try {
      ...
      for (const line of pendingLines) {
        ...
        }).catch((e: Error) => addToast({ type: 'error', message: `"${line.productName}": ${e.message}` }))
      }
      addToast({ type: 'success', message: `Store-out created with ${pendingLines.length} item(s)` })
      resetModal()
      void refetch()
    } catch (e: unknown) {
      addToast({ type: 'error', message: e instanceof Error ? e.message : 'Failed to create store-out' })
    }
  }
  ...
  const pendingTotal = pendingLines.reduce(
    (s, l) => s + (parseFloat(l.qty) || 0) * (parseFloat(l.unitCost) || 0), 0
  )
  ...
  function linesColumns(isDraft: boolean, issueId: string): Column<MILine>[] {
    return [
      ...
      },
      {
        ...
        width: '130px',
        render: (row) => (
          <span style={{ fontWeight: 600, fontVariantNumeric: 'tabular-nums' }}>{fmtAmt(row.totalCost)}</span>
        ),
      },
      ...(isDraft ? [{
        key: '_del',
        ...
        render: (row: MILine) => (
          <button
            onClick={(e) => { e.stopPropagation(); void deleteLine({ variables: { id: row.id, issueId } }) }}
            title="Remove"
            style={{ background: 'none', border: 'none', cursor: 'pointer', color: theme.textMuted, display: 'flex', alignItems: 'center', padding: '4px' }}
          >
  ...
  const pendingColumns: Column<PendingLine>[] = [
    ...
    },
    {
      ...
      header: 'Unit Cost',
      width: '110px',
      render: (row) => <span style={{ fontVariantNumeric: 'tabular-nums' }}>{fmtAmt(parseFloat(row.unitCost) || 0)}</span>,
    },
    {
      key: '_total',
      header: 'Total',
    ...
    },
    {
      ...
      render: (row) => (
        <button
          onClick={() => setPendingLines(prev => prev.filter(l => l.key !== row.key))}
          title="Remove"
          style={{ background: 'none', border: 'none', cursor: 'pointer', color: theme.textMuted, display: 'flex', alignItems: 'center', padding: '4px' }}
        >
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
    ...
    <div style={{ padding: '24px', maxWidth: '1200px' }}>
      ...
      <div style={{ display: 'flex', gap: '12px', marginBottom: '20px', alignItems: 'flex-end', flexWrap: 'wrap' }}>
        ...
        {(statusFilter || projectFilter) && (
          <Button variant="ghost" size="sm" onClick={() => { setStatusFilter(''); setProjectFilter('') }}>
            Clear filters
          </Button>
        )}
      </div>
      ...
      {loading ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {[1, 2, 3].map(i => (
            <div key={i} style={{ height: '56px', borderRadius: '14px', background: theme.bgSurface, border: `1px solid ${theme.border}`, animation: 'fnc-shimmer 1.5s ease-in-out infinite' }} />
          ))}
        </div>
      ) : issues.length === 0 ? (
        <EmptyState
                ...
                >
                  ...
                  <div>
                    {si.projectCode ? (
                      <>
                        <button
                          onClick={e => { e.stopPropagation(); navigate(`/projects/${si.id}`) }}
                          style={{ background: 'none', border: 'none', cursor: 'pointer', color: theme.accent, fontSize: '13px', padding: 0, fontWeight: 500 }}
                        >
                          {si.projectCode}
                        </button>
                        {si.projectName && (
                          <div style={{ fontSize: '11px', color: theme.textMuted, marginTop: '1px' }}>{si.projectName}</div>
                        )}
                      </>
                    ) : (
                  ...
                  <div style={{ fontSize: '12px', color: theme.textMuted }}>
                    {si.poNumber ? (
                      <span style={{ fontFamily: 'monospace', fontSize: '11px' }}>PO: {si.poNumber}</span>
                    ) : (
                      si.notes?.slice(0, 35) ?? '—'
                    )}
                  </div>
                  <div style={{ fontSize: '12px', color: theme.textMuted }}>{si.issueDate.slice(0, 10)}</div>
                  ...
                  <div style={{ borderTop: `1px solid ${theme.border}`, padding: '16px' }}>
                    ...
                    {isDraft && (
                      <div style={{
                        ...
                        <p style={{ fontSize: '11px', fontWeight: 600, color: theme.textMuted, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '12px' }}>
                          Add Item
                        </p>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 90px 140px auto', gap: '10px', alignItems: 'flex-end' }}>
                          <SearchableSelect
                            label="Item"
                            ...
                            onChange={val => {
                              ...
                              if (prod?.average_cost != null) setDraftUnit(String(prod.average_cost))
                            }}
                      ...
                      <div style={{ display: 'flex', gap: '8px' }}>
                        {si.projectCode && (
                          <Button variant="ghost" size="sm"
                            onClick={() => navigate(`/projects/${si.id}?tab=procurement`)}>
                            View Project
                          </Button>
                        )}
                        {isDraft && si.lines.length > 0 && (
                          <Button variant="primary" size="sm"
                            onClick={() => setConfirmIssue(si.id)}>
                            Issue Store Out
                          </Button>
                        )}
                        {(isDraft || si.status === 'issued') && (
                          <Button variant="ghost" size="sm"
                            onClick={() => setConfirmCancel(si.id)}>
                            Cancel
                          </Button>
                        )}
                      </div>
      ...
      <Modal
        ...
        footer={
          <>
            <Button variant="ghost" size="md" onClick={resetModal}>Cancel</Button>
            <Button
              variant="primary"
              size="md"
            ...
            >
              Create{pendingLines.length > 0 ? ` (${pendingLines.length} item${pendingLines.length > 1 ? 's' : ''})` : ''}
            </Button>
          </>
        }
      >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '14px', marginBottom: '14px' }}>
          ...
          <SearchableSelect
            ...
            required
            value={formProjectId}
            onChange={val => { setFormProjectId(val); setFormPoId('') }}
            options={projectOptions}
        ...
        <div style={{ marginTop: '20px' }}>
          <div style={{
            ...
            <p style={{ fontSize: '12px', fontWeight: 600, color: theme.textSecondary, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
              Items to Issue
            </p>
          ...
          {/* Pending lines table */}
          {pendingLines.length > 0 && (
            <div style={{ marginBottom: '14px', border: `1px solid ${theme.border}`, borderRadius: '10px', overflow: 'hidden' }}>
              <Table<PendingLine>
          ...
          <div style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 130px auto', gap: '10px', alignItems: 'flex-end' }}>
              <SearchableSelect
                label="Item"
                value={newProductId}
                onChange={val => {
          ...
          {pendingLines.length === 0 && (
            <p style={{ fontSize: '11px', color: theme.textMuted, marginTop: '8px', textAlign: 'right' }}>
      ...
      <ConfirmDialog
        ...
        onConfirm={() => { if (confirmIssue) void issueMI({ variables: { id: confirmIssue } }) }}
      ...
      <ConfirmDialog
        ...
        onConfirm={() => { if (confirmCancel) void cancelMI({ variables: { id: confirmCancel } }) }}
