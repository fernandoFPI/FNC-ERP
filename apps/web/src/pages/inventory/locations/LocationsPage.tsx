  ...
  const columns: Column<StockLocation>[] = [
    {
      ...
      render: (l) => (
        <div>
          ...
          {l.parent_name && (
            <div style={{ fontSize: '11px', color: theme.textMuted }}>{l.parent_name}</div>
          )}
        </div>
      ),
    },
    ...
    },
    {
      ...
      render: (l) => (
        <Badge
          variant={
            l.type === 'warehouse' ? 'success' : l.type === 'transit' ? 'warning' : 'neutral'
          }
        >
          {l.type.replace('_', ' ')}
        </Badge>
    ...
    },
    {
      ...
      header: 'Address',
      render: (l) => (
        <span style={{ color: theme.textMuted, fontSize: '13px' }}>{l.address ?? '—'}</span>
      ),
    },
    {
      key: 'is_active',
