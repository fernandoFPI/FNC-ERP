...
function statusVariant(status: string): 'success' | 'warning' | 'danger' | 'info' | 'neutral' | 'accent' {
  const m: Record<string, 'success' | 'warning' | 'danger' | 'info' | 'neutral' | 'accent'> = {
    completed: 'success', draft: 'neutral', cancelled: 'danger', in_transit: 'info', pending: 'warning',
  }
...
export default function IntercoStockTransfersPage() {
  ...
  const items = (data?.intercoStockTransfers.items ?? []).filter(r =>
    !search ||
  ...
  return (
    <div style={{ padding: '24px' }}>
      <PageHeader
        ...
        actions={
          <div style={{ display: 'flex', gap: '8px' }}>
            ...
            <Button variant="primary" size="sm" onClick={() => navigate('/interco/stock-transfers/new')}>New Transfer</Button>
          </div>
        }
      />
      ...
      <Card padding="sm" style={{ marginBottom: '16px' }}>
        <FilterBar
          search={{ value: search, onChange: setSearch, placeholder: 'Search transfer number or company…' }}
          filters={[
            {
              key: 'status',
              label: 'Status',
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              <thead>
                <tr style={{ background: theme.bgSurface }}>
                  {['Transfer #', 'From', 'To', 'Pricing Method', 'Total Value', 'Status', 'Date'].map((h, i) => (
              ...
              <tbody>
                {items.map(row => (
                  <tr key={row.id} style={{ borderBottom: `1px solid ${theme.tableBorder}`, cursor: 'pointer' }}
                    onClick={() => navigate(`/interco/stock-transfers/${row.id}`)}
                    onMouseEnter={(e) => { e.currentTarget.style.background = theme.tableRowHover }}
                    onMouseLeave={(e) => { e.currentTarget.style.background = 'transparent' }}>
                    <td style={{ padding: '12px 14px', color: theme.accent, fontWeight: 500 }}>{row.transferNumber}</td>
                    ...
                    <td style={{ padding: '12px 14px' }}>
                      <Badge variant="neutral" size="sm">{row.pricingMethod}</Badge>
                    </td>
