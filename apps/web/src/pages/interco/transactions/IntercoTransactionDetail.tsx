...
function JournalEntryPreview({ tx, theme }: { tx: IntercoTxDetail; theme: Record<string, string> }) {
  const tdBase: React.CSSProperties = { padding: '8px 12px', fontSize: 13, borderBottom: `1px solid ${theme['tableBorder'] ?? theme['border']}` }

  const entries = [
    { company: tx.fromCompanyName, side: 'Debit', account: tx.fromAccountName || tx.fromAccountId || '—', amount: tx.amount, currency: tx.currencyCode },
    { company: tx.fromCompanyName, side: 'Credit', account: tx.fromAccountName || tx.fromAccountId || '—', amount: tx.amount, currency: tx.currencyCode },
    { company: tx.toCompanyName, side: 'Debit', account: tx.toAccountName || tx.toAccountId || '—', amount: tx.amount, currency: tx.currencyCode },
    { company: tx.toCompanyName, side: 'Credit', account: tx.toAccountName || tx.toAccountId || '—', amount: tx.amount, currency: tx.currencyCode },
  ]
  ...
  return (
    <div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          <thead>
            <tr style={{ background: theme['bgSurfaceHover'] }}>
              <th style={{ ...tdBase, textAlign: 'left', fontWeight: 600, color: theme['textMuted'], fontSize: 11, textTransform: 'uppercase' }}>Company</th>
              <th style={{ ...tdBase, textAlign: 'left', fontWeight: 600, color: theme['textMuted'], fontSize: 11, textTransform: 'uppercase' }}>Account</th>
              <th style={{ ...tdBase, textAlign: 'center', fontWeight: 600, color: theme['textMuted'], fontSize: 11, textTransform: 'uppercase' }}>Side</th>
              <th style={{ ...tdBase, textAlign: 'right', fontWeight: 600, color: theme['textMuted'], fontSize: 11, textTransform: 'uppercase' }}>Amount</th>
            </tr>
          </thead>
          <tbody>
            {entries.map((e, i) => (
              <tr key={i}>
                ...
                <td style={{ ...tdBase, color: theme['textPrimary'], fontFamily: 'monospace', fontSize: 12 }}>{e.account}</td>
                <td style={{ ...tdBase, textAlign: 'center' }}>
                  <Badge variant={e.side === 'Debit' ? 'info' : 'warning'} size="sm">{e.side}</Badge>
                </td>
      ...
      <p style={{ fontSize: 11, color: theme['textMuted'], marginTop: 10, fontStyle: 'italic' }}>
        Note: Exact accounts are determined at creation time. This preview shows the transaction accounts on record.
      </p>
    </div>
  )
}
...
function AccountPicker({
  ...
  const options = (data?.accounts ?? []).map(a => ({ value: a.id, label: `${a.code} — ${a.name}` }))
  ...
  return (
    <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
      ...
      />
      <Button variant="secondary" size="sm" disabled={!accountId} loading={setting} onClick={() => onSet(accountId)}>
        Set
      </Button>
    </div>
  )
...
export default function IntercoTransactionDetail() {
  ...
  const [cancelTx, { loading: cancelling }] = useMutation(CANCEL_INTERCO_TRANSACTION, {
    variables: { id },
    onCompleted: () => { addToast({ type: 'success', message: 'Transaction cancelled' }); setConfirmCancel(false) },
    onError: (e) => { addToast({ type: 'error', message: e.message }); setConfirmCancel(false) },
    refetchQueries: [{ query: INTERCO_TRANSACTION_QUERY, variables: { id } }],
  })
  ...
  return (
    <div style={{ padding: '24px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            {isDraft && !canPost && (
              <span style={{ fontSize: '12px', color: theme.textMuted }}>
                {!eitherApproved ? 'Awaiting approval from at least one company' : 'Awaiting posting account selection from both companies'}
              </span>
            )}
            {canPost && (
              <Button variant="primary" size="sm" onClick={() => setShowJournalPreview(true)} loading={posting}>
                Post
              </Button>
            )}
            {canCancel && (
              <Button variant="danger" size="sm" onClick={() => setConfirmCancel(true)} loading={cancelling}>
                Cancel
              </Button>
            )}
          </div>
      ...
      {/* Dual Approval Panel */}
      {showApprovalPanel && (
        <Card padding="md" style={{ marginBottom: '16px', border: `1px solid ${theme.warningBorder ?? theme.border}`, background: theme.warningBg ?? theme.bgSurface }}>
          <p style={{ fontSize: '12px', fontWeight: 600, color: theme.textPrimary, marginBottom: '12px' }}>Company Approvals Required</p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '12px' }}>
            ...
            <div style={{ padding: '10px 14px', borderRadius: '8px', background: theme.bgSurface, border: `1px solid ${theme.border}` }}>
              <p style={{ fontSize: '11px', color: theme.textMuted, marginBottom: '4px' }}>{tx.fromCompanyName}</p>
              {tx.fromCompanyApprovedBy ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <Badge variant="success" size="sm">Approved</Badge>
                  {tx.fromCompanyApprovedAt && (
                    <span style={{ fontSize: '11px', color: theme.textMuted }}>{new Date(tx.fromCompanyApprovedAt).toLocaleString()}</span>
                  )}
                </div>
              ) : canApproveFrom ? (
                <Button variant="ghost" size="sm" onClick={() => { approveTx() }} loading={approving}>
            ...
            {/* To company approval */}
            <div style={{ padding: '10px 14px', borderRadius: '8px', background: theme.bgSurface, border: `1px solid ${theme.border}` }}>
              <p style={{ fontSize: '11px', color: theme.textMuted, marginBottom: '4px' }}>{tx.toCompanyName}</p>
              {tx.toCompanyApprovedBy ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <Badge variant="success" size="sm">Approved</Badge>
                  {tx.toCompanyApprovedAt && (
                    <span style={{ fontSize: '11px', color: theme.textMuted }}>{new Date(tx.toCompanyApprovedAt).toLocaleString()}</span>
                  )}
                </div>
              ) : canApproveTo ? (
                <Button variant="ghost" size="sm" onClick={() => { approveTx() }} loading={approving}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
        {/* From entity */}
        <Card padding="md">
          <p style={{ fontSize: '11px', fontWeight: 600, color: theme.textMuted, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '12px' }}>From Entity</p>
          <dl style={{ display: 'grid', gridTemplateColumns: '120px 1fr', gap: '8px 12px', fontSize: '13px' }}>
            <dt style={{ color: theme.textMuted }}>Company</dt>
            <dd style={{ color: theme.textPrimary, fontWeight: 500, margin: 0 }}>{tx.fromCompanyName}</dd>
            <dt style={{ color: theme.textMuted }}>Account</dt>
            <dd style={{ color: theme.textSecondary, margin: 0 }}>
              {tx.fromAccountName || tx.fromAccountId || (
                canSetFromAccount ? (
                  <AccountPicker id={tx.id} accountType="revenue" setting={settingAccount}
        ...
        {/* To entity */}
        <Card padding="md">
          <p style={{ fontSize: '11px', fontWeight: 600, color: theme.textMuted, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '12px' }}>To Entity</p>
          <dl style={{ display: 'grid', gridTemplateColumns: '120px 1fr', gap: '8px 12px', fontSize: '13px' }}>
            <dt style={{ color: theme.textMuted }}>Company</dt>
            <dd style={{ color: theme.textPrimary, fontWeight: 500, margin: 0 }}>{tx.toCompanyName}</dd>
            <dt style={{ color: theme.textMuted }}>Account</dt>
            <dd style={{ color: theme.textSecondary, margin: 0 }}>
              {tx.toAccountName || tx.toAccountId || (
                canSetToAccount ? (
                  <AccountPicker id={tx.id} accountType="expense" setting={settingAccount}
                    onSet={(accountId) => setAccount({ variables: { id: tx.id, accountId } })} />
                ) : '—'
              )}
            </dd>
        ...
        {/* Transaction Details */}
        <Card padding="md">
          <p style={{ fontSize: '11px', fontWeight: 600, color: theme.textMuted, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '12px' }}>Transaction Details</p>
          <dl style={{ display: 'grid', gridTemplateColumns: '120px 1fr', gap: '8px 12px', fontSize: '13px' }}>
            <dt style={{ color: theme.textMuted }}>Amount</dt>
            <dd style={{ margin: 0 }}><AmountDisplay amount={tx.amount} currency={tx.currencyCode} size="md" /></dd>
            <dt style={{ color: theme.textMuted }}>Type</dt>
            <dd style={{ margin: 0 }}><Badge variant="neutral">{tx.transactionType}</Badge></dd>
            ...
            <dd style={{ color: theme.textMuted, margin: 0, fontSize: '12px' }}>{new Date(tx.createdAt).toLocaleString()}</dd>
            {tx.postedAt && (
              <>
                <dt style={{ color: theme.textMuted }}>Posted</dt>
                <dd style={{ color: theme.textMuted, margin: 0, fontSize: '12px' }}>{new Date(tx.postedAt).toLocaleString()}</dd>
                <dt style={{ color: theme.textMuted }}>Posted By</dt>
                <dd style={{ color: theme.textMuted, margin: 0, fontSize: '12px' }}>{tx.postedBy}</dd>
              </>
            )}
          </dl>
        </Card>
      ...
      <Modal
        ...
        onClose={() => setShowJournalPreview(false)}
        title="Review Journal Entries"
        size="md"
        footer={
          <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
            <Button variant="ghost" size="sm" onClick={() => setShowJournalPreview(false)}>Cancel</Button>
            <Button variant="primary" size="sm" onClick={() => { postTx() }} loading={posting}>
              Confirm &amp; Post
            </Button>
          </div>
        }
