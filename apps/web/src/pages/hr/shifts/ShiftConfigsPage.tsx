...
export default function ShiftConfigsPage() {
  ...
  function openEdit(s: ShiftConfig) {
    setForm({
      ...
      overtime_threshold_hours: parseFloat(s.overtime_threshold_hours ?? '8'),
    })
    setEditId(s.id)
    setModalOpen(true)
  }
  ...
  async function handleSubmit() {
    ...
    try {
      if (editId) {
        await updateShift({ variables: { id: editId, input } })
  ...
  const columns: Column<ShiftConfig>[] = [
    {
      key: 'name',
      header: 'Name',
      render: (s) => <span style={{ fontWeight: 500, color: theme.textPrimary }}>{s.name}</span>,
    },
    {
      key: 'start_time',
      header: 'Start',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          >
            <Input
              ...
              onChange={(e) => {
                updateTime('start_time', e.target.value)
              }}
            />
            <Input
              label="End time"
              ...
              value={form.end_time}
              onChange={(e) => {
                updateTime('end_time', e.target.value)
              }}
            />
            <Input
              label="Break (min)"
              ...
              onChange={(e) => {
                ...
                setForm((f) => {
                  const reg = calcRegularHours(f.start_time, f.end_time, v)
                  return { ...f, break_minutes: v, overtime_threshold_hours: reg }
                })
              }}
            />
          </div>
          ...
          >
            ...
            h shift with {form.break_minutes} min break ={' '}
            <strong>
              {calcRegularHours(form.start_time, form.end_time, form.break_minutes).toFixed(1)}{' '}
              regular hours
            </strong>
          </div>
          ...
          <Input
            ...
            type="number"
            step="0.5"
            value={String(form.overtime_threshold_hours)}
            onChange={(e) => {
              setForm((f) => ({ ...f, overtime_threshold_hours: parseFloat(e.target.value) || 0 }))
            }}
          />
          <div style={{ fontSize: '11px', color: theme.textMuted }}>
            Iraq Labour Law minimum OT multiplier: 1.5×
          </div>
        </div>
      </Modal>
    </div>
