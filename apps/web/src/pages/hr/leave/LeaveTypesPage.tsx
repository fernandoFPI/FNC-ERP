...
export default function LeaveTypesPage() {
  ...
  const columns: Column<LeaveType>[] = [
    ...
    },
    {
      ...
      header: 'Paid',
      render: (l) => (
        <Badge variant={l.is_paid ? 'success' : 'neutral'}>{l.is_paid ? 'Paid' : 'Unpaid'}</Badge>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (l) => (
        <Badge variant={l.requires_approval ? 'info' : 'neutral'}>
          {l.requires_approval ? 'Yes' : 'No'}
        </Badge>
      ),
    },
    {
  ...
  return (
    <div style={{ padding: '24px', margin: '0 auto', maxWidth: '1100px' }}>
      <PageHeader
        title="Leave Types"
        subtitle={`${leaveTypes.length} leave types`}
        actions={
          <PermissionGate permission="hr.leave.approve" minLevel="approve">
            <Button
              variant="primary"
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          <Input
            ...
            onChange={(e) => {
              setForm((f) => ({ ...f, max_days_per_year: e.target.value }))
            }}
          ...
          <Checkbox
            ...
            onChange={(checked) => {
              setForm((f) => ({ ...f, requires_approval: checked }))
            }}
          />
        </div>
      </Modal>
      ...
      >
        <p style={{ margin: 0, color: theme.textPrimary }}>
          ...
          undone. If this type has existing leave requests, the deletion will be blocked.
        </p>
      </Modal>
    </div>
  )
