...
import { BANK_DETAILS_SUMMARY_QUERY, REVEAL_BANK_DETAILS, UPDATE_BANK_DETAILS } from '../../../graphql/hr'
...
export function BankDetailsForm({ employeeId }: Props) {
  ...
  return (
    <div style={{ marginTop: '16px', border: `1px solid ${theme.border}`, borderRadius: '8px', overflow: 'hidden' }}>
      <button
        onClick={() => setExpanded(e => !e)}
        style={{
          width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between',
          padding: '12px 16px', background: theme.bgSurface, border: 'none', cursor: 'pointer', textAlign: 'left',
        }}
      >
        <span style={{ fontWeight: 600, fontSize: '13px', color: theme.textPrimary }}>Bank Details</span>
        ...
        <div style={{ padding: '16px', background: theme.bgCanvas }}>
          {editing ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <Input label="Bank Name" value={editForm.bank_name ?? ''}
                onChange={e => setEditForm(f => ({ ...f, bank_name: e.target.value }))} />
              <Input label="Account Number" value={editForm.account_number ?? ''}
                onChange={e => setEditForm(f => ({ ...f, account_number: e.target.value }))}
                placeholder={revealed?.account_number ? '(leave blank to keep existing)' : 'Enter account number'} />
              <Input label="IBAN" value={editForm.iban ?? ''}
              ...
              <div>
                <SearchableSelect
                  ...
                  onChange={(v) => setEditForm(f => ({ ...f, currency_code: v }))}
            ...
            <div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '8px', marginBottom: '12px' }}>
                {[
                  { label: 'Bank',           value: summary?.bank_name ?? revealed?.bank_name ?? '—', sensitive: false },
                  { label: 'Account Number', value: revealed?.account_number ?? (hasData ? '••••••••' : '—'), sensitive: !revealed },
                  { label: 'IBAN',           value: revealed?.iban ?? (hasData ? '••••••••' : '—'), sensitive: !revealed },
                  { label: 'Currency',       value: summary?.currency_code ?? revealed?.currency_code ?? '—', sensitive: false },
                ].map(({ label, value, sensitive }) => (
                  <div key={label} style={{ padding: '10px 12px', background: theme.bgSurface, border: `1px solid ${theme.border}`, borderRadius: '6px' }}>
                    <div style={{ fontSize: '10px', color: theme.textMuted, marginBottom: '3px' }}>{label}</div>
                    <div style={{ fontSize: '13px', color: theme.textPrimary, fontFamily: sensitive ? 'monospace' : undefined, letterSpacing: sensitive ? '0.1em' : undefined }}>
                      {value}
                    </div>
                  </div>
                ))}
              ...
              <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                {revealed ? (
                  <span style={{ fontSize: '12px', color: theme.warning, fontWeight: 500 }}>Hiding in {countdown}s</span>
                ) : (
                  hasData && (
                    <PermissionGate permission="hr.employees.view_salary" minLevel="view">
                      <Button variant="secondary" size="sm" loading={revealing} onClick={handleReveal}>Reveal</Button>
                    </PermissionGate>
                  )
                )}
