...
import { CREATE_EMPLOYEE, UPDATE_EMPLOYEE, EMPLOYEE_QUERY, DEPARTMENTS_QUERY, WORK_LOCATIONS_QUERY } from '../../../graphql/hr'
...
const EMPLOYMENT_TYPE_OPTIONS = [
  ...
  { value: 'daily', label: 'Daily' }, { value: 'expat', label: 'Expat' },
...
const emptyForm = {
  ...
  employee_number: '', hire_date: '', employment_type: 'full_time',
...
function SectionHeader({ title }: { title: string }) {
  ...
  return <div style={{ fontSize: '13px', fontWeight: 600, color: theme.textMuted, textTransform: 'uppercase', letterSpacing: '0.06em', paddingBottom: '8px', borderBottom: `1px solid ${theme.border}` }}>{title}</div>
}
  ...
  useEffect(() => {
    if (isEdit && empData?.employee) {
      ...
      setForm({
        ...
        national_id: e.national_id ?? '', passport_number: e.passport_number ?? '',
        nationality: e.nationality ?? '', date_of_birth: e.date_of_birth?.slice(0, 10) ?? '',
        gender: e.gender ?? '', employee_number: e.employee_number ?? '',
        hire_date: e.hire_date?.slice(0, 10) ?? '', employment_type: e.employment_type ?? 'full_time',
        job_title: e.job_title ?? '', department_id: e.department_id ?? '',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', marginTop: '20px' }}>
          <Card style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '14px' }}>
              <Input label="First name" value={form.first_name} onChange={field('first_name')} required error={errors.first_name} />
              <Input label="Last name" value={form.last_name} onChange={field('last_name')} required error={errors.last_name} />
              <Input label="National ID" value={form.national_id} onChange={field('national_id')} error={errors.national_id} />
              <Input label="Passport number" value={form.passport_number} onChange={field('passport_number')} />
          ...
          <Card style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '14px' }}>
              <Input label="Employee number" value={form.employee_number} onChange={field('employee_number')} placeholder="EMP-FNC-001" />
              <Input label="Hire date" type="date" value={form.hire_date} onChange={field('hire_date')} required error={errors.hire_date} />
              <Select label="Employment type" value={form.employment_type} onChange={field('employment_type')} options={EMPLOYMENT_TYPE_OPTIONS} />
              <Input label="Job title" value={form.job_title} onChange={field('job_title')} />
              <Select label="Department" value={form.department_id} onChange={field('department_id')} placeholder="Select…"
                options={departments.map((d: { id: string; name: string }) => ({ value: d.id, label: d.name }))} />
              <Select label="Work location" value={form.work_location_id} onChange={field('work_location_id')} placeholder="Select…"
