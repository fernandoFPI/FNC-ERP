  ...
  const columns: Column<Employee>[] = [
    {
      ...
      render: (e) => (
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          />
          <div>
            <div style={{ fontWeight: 500, color: theme.textPrimary, fontSize: '13px' }}>
              {e.first_name} {e.last_name}
            </div>
            {e.job_title && (
              <div style={{ fontSize: '11px', color: theme.textMuted }}>{e.job_title}</div>
            )}
          </div>
        </div>
      ),
    },
    {
      ...
      header: 'Number',
      render: (e) => (
        <span style={{ fontFamily: 'monospace', fontSize: '12px', color: theme.accent }}>
          {e.employee_number ?? '—'}
        </span>
      ),
    },
    ...
    },
    {
      key: 'employment_type',
      header: 'Type',
      render: (e) => <Badge variant="neutral">{(e.employment_type ?? '').replace('_', ' ')}</Badge>,
    },
    {
      key: 'status',
      header: 'Status',
          ...
          >
            <button
              ...
              style={{
                ...
                borderRadius: '6px',
                color: showInactive ? theme.accent : theme.textMuted,
                cursor: 'pointer',
                fontSize: '12px',
                padding: '4px 10px',
              }}
