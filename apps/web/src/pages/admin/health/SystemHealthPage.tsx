...
function statusVariant(s: string): 'success' | 'warning' | 'danger' | 'info' | 'neutral' | 'accent' {
  const m: Record<string, 'success' | 'warning' | 'danger' | 'info' | 'neutral' | 'accent'> = {
    healthy: 'success', degraded: 'warning', unhealthy: 'danger', starting: 'info',
  }
...
export default function SystemHealthPage() {
  ...
  useEffect(() => { setSecondsAgo(0) }, [data])
  ...
  return (
    <div style={{ padding: '24px' }}>
      <PageHeader
        ...
        actions={
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <span style={{ display: 'flex', alignItems: 'center', gap: '5px', fontSize: '11px', color: theme.textMuted }}>
              <span style={{
                width: '6px', height: '6px', borderRadius: '50%', background: theme.success,
          ...
          {services.map(svc => (
            <Card key={svc.service} padding="md" rimHighlight>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
                <div>
                  <p style={{ fontSize: '14px', fontWeight: 600, color: theme.textPrimary, textTransform: 'capitalize' }}>{svc.service}</p>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '12px', marginBottom: '12px' }}>
                <div>
                  <p style={{ fontSize: '10px', color: theme.textMuted, marginBottom: '2px' }}>Latency</p>
                  <p style={{ fontSize: '16px', fontWeight: 500, color: svc.latencyMs > 200 ? theme.warning : theme.success }}>
                    {svc.latencyMs}ms
                  </p>
                </div>
                <div>
                  <p style={{ fontSize: '10px', color: theme.textMuted, marginBottom: '2px' }}>Uptime</p>
                  <p style={{ fontSize: '16px', fontWeight: 500, color: theme.textPrimary }}>{formatUptime(svc.uptime)}</p>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                {Object.entries(svc.checks).map(([key, val]) => (
                  <div key={key} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '12px', color: theme.textSecondary, textTransform: 'capitalize' }}>{key}</span>
                    <Badge variant={checkVariant(val)} size="sm">{val}</Badge>
                  </div>
                ))}
              </div>
            </Card>
      ...
      {!loading && services.length === 0 && (
        <Card padding="md">
          <p style={{ color: theme.textMuted, textAlign: 'center', fontSize: '13px' }}>No health data available. Ensure the gateway health endpoint is running.</p>
        </Card>
      )}
    </div>
  )
