...
export default function RoleTemplatesPage() {
  ...
  const { data, loading, refetch } = useQuery<{ roleTemplates: RoleTemplate[] }>(GET_ROLE_TEMPLATES, {
    fetchPolicy: 'cache-and-network',
  })
  const templates = data?.roleTemplates ?? []

  const [createTemplate, { loading: creating }] = useMutation(CREATE_ROLE_TEMPLATE, {
    onCompleted: () => { addToast({ type: 'success', message: 'Template created' }); closeDrawer(); void refetch() },
    onError: (e) => addToast({ type: 'error', message: e.message }),
  })

  const [updateTemplate, { loading: updating }] = useMutation(UPDATE_ROLE_TEMPLATE, {
    onCompleted: () => { addToast({ type: 'success', message: 'Template updated' }); closeDrawer(); void refetch() },
    onError: (e) => addToast({ type: 'error', message: e.message }),
  })

  const [deleteTemplate, { loading: deleting }] = useMutation(DELETE_ROLE_TEMPLATE, {
    onCompleted: () => { addToast({ type: 'success', message: 'Template deleted' }); setDeleteId(null); void refetch() },
    onError: (e) => { addToast({ type: 'error', message: e.message }); setDeleteId(null) },
  })
  ...
  function handleSave() {
    ...
    if (editingId) {
      void updateTemplate({ variables: { id: editingId, input: { ...form, description: form.description || undefined, permissions: permissionsPayload } } })
    } else {
      void createTemplate({ variables: { input: { ...form, description: form.description || undefined, permissions: permissionsPayload } } })
    }
  }

  const permCount = (t: RoleTemplate) => t.permissions.filter((p) => p.accessLevel !== 'none').length
  ...
  return (
    <div style={{ padding: '24px', margin: '0 auto', maxWidth: '1300px' }}>
      ...
      <Card style={{ marginTop: '20px' }} padding="none">
        {loading ? (
          <div style={{ padding: '32px', textAlign: 'center', color: theme.textMuted }}>Loading…</div>
        ) : templates.length === 0 ? (
          <EmptyState title="No templates" message="Create a role template to quickly apply permission sets to users." />
        ) : (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              ...
              <tbody>
                {templates.map((t) => (
                  ...
                  >
                    <td style={{ padding: '12px 16px', fontWeight: 500, color: theme.textPrimary }}>{t.name}</td>
                    <td style={{ padding: '12px 16px', color: theme.textSecondary }}>{t.description ?? '—'}</td>
                    <td style={{ padding: '12px 16px' }}>
                      <span style={{ fontFamily: 'monospace', color: theme.textSecondary }}>{permCount(t)}</span>
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', paddingBottom: '80px' }}>
          ...
          <div>
            <p style={{ fontSize: '12px', fontWeight: 600, color: theme.textMuted, marginBottom: '10px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
              Permissions
            </p>
      ...
      <ConfirmDialog
        ...
        onConfirm={() => { if (deleteId) void deleteTemplate({ variables: { id: deleteId } }) }}
