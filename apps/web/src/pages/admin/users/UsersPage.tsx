...
import { USERS_QUERY, DEACTIVATE_USER, CREATE_USER, INVITE_USER, COMPANIES_QUERY } from '../../../graphql/admin'
...
export default function UsersPage() {
  ...
  const [confirmDeactivateUser, setConfirmDeactivateUser] = useState<{ id: string; email: string } | null>(null)
  ...
  const { data, loading, refetch } = useQuery<UsersData>(USERS_QUERY, {
    variables: { search: search || undefined, isActive: isActive === '' ? undefined : isActive === 'true', page, limit: 50 },
  })

  const { data: companiesData } = useQuery<{ companies: { id: string; name: string }[] }>(COMPANIES_QUERY)
  const companies = companiesData?.companies ?? []

  const [deactivateUser] = useMutation(DEACTIVATE_USER, {
    onCompleted: () => { addToast({ type: 'success', message: 'User deactivated' }); void refetch() },
  ...
  return (
    <div style={{ padding: '24px' }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              <thead>
                <tr style={{ background: theme.bgSurface }}>
                  {['Email', 'Status', 'MFA', 'Companies', 'Sessions', 'Last Login', ''].map((h) => (
              ...
              <tbody>
                {users.map((user) => (
                  ...
                  >
                    <td style={{ padding: '12px 14px', color: theme.textPrimary, fontWeight: 500 }}>{user.email}</td>
                    <td style={{ padding: '12px 14px' }}>
                      <Badge variant={user.isActive ? 'success' : 'neutral'} size="sm">{user.isActive ? 'Active' : 'Inactive'}</Badge>
                    </td>
                    <td style={{ padding: '12px 14px' }}>
                      <Badge variant={user.mfaEnabled ? 'success' : 'warning'} size="sm">{user.mfaEnabled ? 'On' : 'Off'}</Badge>
                    </td>
                    <td style={{ padding: '12px 14px', color: theme.textMuted, fontSize: '12px' }}>{user.companies.join(', ') || '—'}</td>
                    <td style={{ padding: '12px 14px', color: user.activeSessions > 0 ? theme.accent : theme.textMuted }}>{user.activeSessions}</td>
                    <td style={{ padding: '12px 14px', color: theme.textMuted, fontSize: '12px' }}>
                      {user.lastLogin ? new Date(user.lastLogin).toLocaleDateString() : 'Never'}
                    </td>
                    <td style={{ padding: '12px 14px' }} onClick={(e) => e.stopPropagation()}>
                      {user.isActive && (
                        <Button variant="danger" size="sm" onClick={() => setConfirmDeactivateUser({ id: user.id, email: user.email })}>
                          Deactivate
                        </Button>
                      )}
                    </td>
      ...
      {/* Invite user modal */}
      {showInviteModal && (
        <Modal open={showInviteModal} title="Invite User" onClose={() => setShowInviteModal(false)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', minWidth: '380px' }}>
            ...
            <div>
              <SearchableSelect
                ...
                onChange={(v) => setInviteForm({ ...inviteForm, companyId: v })}
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
              ...
              <Button
                ...
                onClick={() => void inviteUser({ variables: { input: { email: inviteForm.email, company_id: inviteForm.companyId || undefined, role: inviteForm.role || undefined } } })}
              >
                Send invitation
              </Button>
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', minWidth: '380px' }}>
            ...
            <div>
              <SearchableSelect
                ...
                onChange={(v) => setCreateForm({ ...createForm, companyId: v })}
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
              ...
              <Button
                ...
                onClick={() => void createUser({ variables: { input: { email: createForm.email, password: createForm.password, company_id: createForm.companyId || undefined, role: createForm.role || undefined } } })}
              >
                Create user
              </Button>
            </div>
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          if (confirmDeactivateUser) void deactivateUser({ variables: { userId: confirmDeactivateUser.id } })
