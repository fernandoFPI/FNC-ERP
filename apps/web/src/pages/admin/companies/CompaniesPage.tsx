...
export default function CompaniesPage() {
  ...
  const [form, setForm] = useState({ name: '', legalName: '', currencyCode: 'USD', countryCode: '' })
  ...
  return (
    <div style={{ padding: '24px', margin: '0 auto', maxWidth: '1300px' }}>
      ...
      {loading && <div style={{ padding: '48px', textAlign: 'center', color: theme.textMuted }}>Loading…</div>}
        ...
        {companies.map((c) => (
          ...
          <Card style={{ padding: '20px', transition: 'border-color 0.15s' }}>
            ...
            {c.legalName && c.legalName !== c.name && (
              <div style={{ fontSize: '12px', color: theme.textMuted, marginBottom: '6px' }}>{c.legalName}</div>
            )}
            <div style={{ display: 'flex', gap: '16px', marginTop: '10px' }}>
              <div style={{ fontSize: '11px', color: theme.textMuted }}>
                <span style={{ color: theme.textPrimary, fontWeight: 500 }}>{c.currencyCode}</span> · {c.countryCode ?? '—'}
              </div>
      ...
      {showCreateModal && (
        <Modal open={showCreateModal} title="New Company" onClose={() => setShowCreateModal(false)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', minWidth: '380px' }}>
            <div>
              <label style={{ fontSize: '12px', color: theme.textMuted, display: 'block', marginBottom: '4px' }}>Company name *</label>
              <input
                value={form.name}
            ...
            <div>
              <label style={{ fontSize: '12px', color: theme.textMuted, display: 'block', marginBottom: '4px' }}>Legal name</label>
              <input
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '10px' }}>
              <div>
                <label style={{ fontSize: '12px', color: theme.textMuted, display: 'block', marginBottom: '4px' }}>Currency *</label>
                <input
              ...
              <div>
                <label style={{ fontSize: '12px', color: theme.textMuted, display: 'block', marginBottom: '4px' }}>Country code</label>
                <input
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
              ...
              <Button
                ...
                onClick={() => void createCompany({ variables: { input: { name: form.name, legalName: form.legalName || undefined, currencyCode: form.currencyCode, countryCode: form.countryCode || undefined } } })}
              >
                Create
              </Button>
            </div>
