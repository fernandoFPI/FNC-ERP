...
import { USERS_QUERY, OUTBOX_MONITOR_QUERY, OUTBOX_EVENTS_QUERY, SYSTEM_HEALTH_QUERY, AUDIT_LOG_QUERY, OUTBOX_EVENT_CONFIGS_QUERY, OUTBOX_DLQ_QUERY } from '../../../graphql/admin'
...
const adminUser = { id: 'u1', email: 'admin@fnc.com', mfaEnabled: false, system_admin: true, role: 'system_admin' }

const usersMock = {
  request: { query: USERS_QUERY, variables: { search: undefined, isActive: undefined, companyId: undefined, page: 1, limit: 20 } },
  result: { data: { users: { items: [], total: 0, page: 1, limit: 20 } } },
}

const outboxMock = {
  request: { query: OUTBOX_MONITOR_QUERY, variables: {} },
  result: { data: { outboxMonitor: { health: 'healthy', counts: { pending: 5, failed: 2, dlq: 0, stuck: 0 }, generatedAt: new Date().toISOString() } } },
}

const eventsMock = {
  request: { query: OUTBOX_EVENTS_QUERY, variables: { status: undefined, service: undefined, eventType: undefined, page: 1, limit: 50 } },
...
const healthMock = {
  ...
  result: {
    data: {
      systemHealth: [
        { service: 'auth', status: 'ok', latencyMs: 12, checks: { database: 'ok', redis: 'ok' }, uptime: 86400, lastChecked: '2026-06-07T10:00:00Z' },
        { service: 'finance', status: 'ok', latencyMs: 25, checks: { database: 'ok' }, uptime: 86400, lastChecked: '2026-06-07T10:00:00Z' },
      ],
    },
  },
}
...
const auditMock = {
  request: { query: AUDIT_LOG_QUERY, variables: { userId: undefined, companyId: undefined, tableName: undefined, action: undefined, recordId: undefined, fromDate: undefined, toDate: undefined, page: 1, limit: 50 } },
...
const dlqMock = {
  request: { query: OUTBOX_DLQ_QUERY, variables: { status: undefined, priority: undefined, eventType: undefined, page: 1, limit: 50 } },
...
describe('AdminLayout', () => {
  it('redirects non-admin to dashboard', () => {
    useAuthStore.setState({ user: null, isAuthenticated: false, accessToken: null, refreshToken: null, mfaPending: false, tempToken: null, themePreference: null })
    render(
  ...
  it('renders admin nav for system admin', () => {
    useAuthStore.setState({ user: adminUser as never, isAuthenticated: true, accessToken: 'tok', refreshToken: 'ref', mfaPending: false, tempToken: null, themePreference: null })
    render(
