...
export default function ReconcileWorkspace() {
  ...
  const canCreate =
    selectedBank.size === 1 &&
    selectedGl.size === 0 &&
    !stmt?.lines.find((l) => l.id === [...selectedBank][0])?.is_reconciled
  ...
  async function handleAutoMatch() {
    ...
    try {
      const r = await api.post<{ matched: number }>(
        `/finance/bank/statements/${statementId}/auto-match`,
      )
      await loadStatement()
  ...
  async function handleAddLines() {
    if (!statementId) return
    const valid = lineDrafts.filter(
      (l) => l.description && l.transaction_date && (Number(l.debit) > 0 || Number(l.credit) > 0),
    )
  ...
  async function handleFinalize() {
    if (!statementId || !confirm('Mark this statement as fully reconciled? This cannot be undone.'))
      return
    setFinalizing(true)
    try {
    ...
    return (
      <div style={{ padding: '24px' }}>
        ...
        {!account?.gl_account_id && (
          ...
          >
            ⚠ No GL account linked. Edit this account and link it to a chart of accounts entry to
            ...
            <Card padding="lg" style={{ width: '420px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <div>
                  ...
                  <input
                    style={inputStyle}
                    placeholder="2025-12"
                    value={newStmt.period}
                    onChange={(e) => {
                      setNewStmt((f) => ({ ...f, period: e.target.value }))
                    }}
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      setNewStmt((f) => ({ ...f, statement_date: e.target.value }))
                    }}
                  />
                </div>
                <div>
                  ...
                  <input
                    style={inputStyle}
                    type="number"
                    value={newStmt.opening_balance}
                    onChange={(e) => {
                      setNewStmt((f) => ({ ...f, opening_balance: e.target.value }))
                    }}
                  />
                </div>
                <div>
                  ...
                  <input
                    style={inputStyle}
                    type="number"
                    value={newStmt.closing_balance}
                    onChange={(e) => {
                      setNewStmt((f) => ({ ...f, closing_balance: e.target.value }))
                    }}
                  />
                </div>
                <div style={{ gridColumn: '1/-1' }}>
                  ...
                  <input
                    ...
                    value={newStmt.notes}
                    onChange={(e) => {
                      setNewStmt((f) => ({ ...f, notes: e.target.value }))
                    }}
                  />
                </div>
              </div>
              ...
              >
                ...
                <Button
                  ...
                  disabled={creatingStmt || !newStmt.period || !newStmt.statement_date}
                >
  ...
  if (loading)
    return (
      <div style={{ padding: '24px', color: theme.textMuted }}>
        Loading reconciliation workspace...
      </div>
    )
  if (!stmt)
    return <div style={{ padding: '24px', color: theme.textMuted }}>Statement not found.</div>
  ...
  return (
    <div
      style={{
        padding: '24px',
        paddingBottom: selectedBank.size > 0 || selectedGl.size > 0 ? '120px' : '24px',
      }}
    >
      {/* Header */}
      <PageHeader
        ...
        actions={
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            <Button
              ...
              size="sm"
              onClick={() => {
                navigate(`/finance/bank/${accountId}`)
              }}
            >
              ← Statements
            </Button>
        ...
        {[
          {
            ...
            value: (
              <AmountDisplay
                amount={stmt.opening_balance}
                currency={stmt.currency_code}
                size="sm"
              />
            ),
          },
          ...
          },
          {
            ...
            value: (
              <AmountDisplay
                amount={stmt.closing_balance - stmt.opening_balance}
                currency={stmt.currency_code}
                size="sm"
                colored
              />
        ...
        <div style={{ height: '6px', borderRadius: '3px', background: theme.border }}>
          <div
            style={{
              ...
              height: '100%',
              borderRadius: '3px',
              background: reconPct === 100 ? '#22c55e' : theme.accent,
                  ...
                  >
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      >
                        ...
                        >
                          {line.transaction_date.slice(0, 10)}
                        </span>
                        {line.reference && (
                      ...
                      {isMatched &&
                        line.matches?.map((m) => (
                          <p
                            key={m.id}
                            style={{ fontSize: '10px', color: theme.accent, margin: '2px 0 0' }}
                          >
                            {m.match_type === 'created' ? '⊕' : '⇌'}{' '}
                            {m.je_reference ?? m.je_description}
                          </p>
                        ))}
                    </div>
                  ...
                  >
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      >
                        ...
                        <span
                          style={{ fontSize: '10px', color: theme.accent, fontFamily: 'monospace' }}
                        >
                          {entry.reference}
                        </span>
                      </div>
                    ...
                    <div style={{ textAlign: 'right', marginLeft: '12px', flexShrink: 0 }}>
                      ...
                      <p style={{ fontSize: '10px', color: theme.textMuted, margin: '2px 0 0' }}>
                        {Number(entry.debit) > 0
                          ? `DR ${Number(entry.debit).toLocaleString()}`
                          : `CR ${Number(entry.credit).toLocaleString()}`}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              {selectedBank.size > 0 && (
                <span
                  style={{
                    marginLeft: '6px',
                    fontFamily: 'monospace',
                    color: selectedBankNet >= 0 ? '#22c55e' : '#ef4444',
                  }}
                >
                  ({selectedBankNet >= 0 ? '+' : ''}
                  {selectedBankNet.toLocaleString()})
                </span>
              )}
            </span>
              ...
              {selectedGl.size > 0 && (
                <span
                  style={{
                    marginLeft: '6px',
                    fontFamily: 'monospace',
                    color: selectedGlNet >= 0 ? '#22c55e' : '#ef4444',
                  }}
                >
                  ({selectedGlNet >= 0 ? '+' : ''}
                  {selectedGlNet.toLocaleString()})
                </span>
              )}
            </span>
            {selectedBank.size > 0 && selectedGl.size > 0 && (
              <>
                ...
                <span
                  style={{
                    fontSize: '12px',
                    fontWeight: 600,
                    color: matchDiff === 0 ? '#22c55e' : '#ef4444',
                  }}
                >
      ...
      {showAddLines && (
        ...
        >
          <Card padding="lg" style={{ width: '700px', maxHeight: '80vh', overflowY: 'auto' }}>
            ...
            >
              <h3 style={{ margin: 0, fontSize: '15px', color: theme.textPrimary }}>
                Add Statement Lines
              </h3>
              <Button
                variant="ghost"
      ...
      {showCreateEntry && createEntryLine && (
        ...
        >
          <Card padding="lg" style={{ width: '440px' }}>
            ...
            >
              <p style={{ fontSize: '11px', color: theme.textMuted, margin: '0 0 4px' }}>
                Bank line
              </p>
              <p
                style={{
              ...
              >
                {createEntryLine.transaction_date.slice(0, 10)} ·{' '}
            ...
            <div style={{ marginBottom: '16px' }}>
              ...
              <input
                style={inputStyle}
                value={createEntryDesc}
                onChange={(e) => {
                  setCreateEntryDesc(e.target.value)
                }}
              />
            </div>
            <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
              ...
              <Button
                ...
                disabled={creatingEntry || !offsetAccountId}
              >
