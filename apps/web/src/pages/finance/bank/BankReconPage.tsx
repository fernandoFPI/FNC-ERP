...
export default function BankReconPage() {
  ...
  const labelStyle = { fontSize: '11px', color: theme.textMuted, marginBottom: '4px', display: 'block' as const }

  const totalAccounts   = accounts.length
  const pendingTotal    = accounts.reduce((s, a) => s + Number(a.pending_statements), 0)
  const lastReconDate   = accounts.reduce((best, a) => {
  ...
  return (
    <div style={{ padding: '24px' }}>
      <PageHeader
        ...
        actions={<Button variant="primary" size="sm" onClick={openNew}>+ New Account</Button>}
      />

      {/* KPI strip */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(140px,1fr))', gap: '12px', marginBottom: '20px' }}>
        <Card padding="sm">
          <p style={{ fontSize: '10px', color: theme.textMuted, marginBottom: '4px' }}>Bank Accounts</p>
          <p style={{ fontSize: '22px', fontWeight: 700, color: theme.textPrimary }}>{totalAccounts}</p>
        </Card>
        <Card padding="sm">
          <p style={{ fontSize: '10px', color: theme.textMuted, marginBottom: '4px' }}>Pending Statements</p>
          <p style={{ fontSize: '22px', fontWeight: 700, color: pendingTotal > 0 ? theme.warning : theme.textPrimary }}>{pendingTotal}</p>
        </Card>
        <Card padding="sm">
          <p style={{ fontSize: '10px', color: theme.textMuted, marginBottom: '4px' }}>Last Reconciled</p>
          <p style={{ fontSize: '14px', fontWeight: 600, color: theme.textPrimary }}>{lastReconDate ? new Date(lastReconDate).toLocaleDateString() : '—'}</p>
        </Card>
      </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
          <thead>
            <tr style={{ borderBottom: `1px solid ${theme.border}`, background: theme.bgSurface }}>
              {['Account', 'Bank / Branch', 'Account No.', 'GL Account', 'Last Reconciled', 'Last Balance', 'Status', ''].map(h => (
          ...
          <tbody>
            ...
            {!loading && !accounts.length && (
              <tr><td colSpan={8} style={{ padding: '24px', textAlign: 'center', color: theme.textMuted }}>
                No bank accounts configured. <span style={{ color: theme.accent, cursor: 'pointer' }} onClick={openNew}>Add your first account →</span>
              </td></tr>
            )}
            {accounts.map(a => (
              <tr key={a.id} style={{ borderBottom: `1px solid ${theme.border}` }}>
                ...
                <td style={{ padding: '10px 14px', color: theme.textSecondary }}>{a.bank_name ?? '—'}</td>
                <td style={{ padding: '10px 14px', color: theme.textSecondary, fontFamily: 'monospace', fontSize: '12px' }}>{a.account_number ?? '—'}</td>
                <td style={{ padding: '10px 14px' }}>
                  {a.gl_account_code
                    ? <span style={{ fontSize: '12px', color: theme.textSecondary }}>{a.gl_account_code} · {a.gl_account_name}</span>
                    : <Badge variant="warning">No GL linked</Badge>
                  }
                </td>
                <td style={{ padding: '10px 14px', color: theme.textSecondary, fontSize: '12px' }}>
                  {a.last_reconciled_date ? new Date(a.last_reconciled_date).toLocaleDateString() : '—'}
                  {a.latest_period && <p style={{ fontSize: '11px', color: theme.textMuted }}>Latest: {a.latest_period}</p>}
                </td>
                <td style={{ padding: '10px 14px' }}>
                  {a.last_reconciled_balance != null
                    ? <AmountDisplay amount={a.last_reconciled_balance} currency={a.currency_code} size="sm" />
                ...
                <td style={{ padding: '10px 14px' }}>
                  {Number(a.pending_statements) > 0
                    ...
                    : a.last_reconciled_date ? <Badge variant="success">Up to date</Badge> : <Badge variant="neutral">Not started</Badge>
                  }
                </td>
                <td style={{ padding: '10px 14px' }}>
                  <div style={{ display: 'flex', gap: '6px' }}>
                    <Button variant="primary" size="sm" onClick={() => navigate(`/finance/bank/${a.id}`)}>Reconcile</Button>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <Card padding="lg" style={{ width: '500px', maxHeight: '85vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h3 style={{ margin: 0, color: theme.textPrimary, fontSize: '16px' }}>{editingId ? 'Edit Bank Account' : 'New Bank Account'}</h3>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
              <div style={{ gridColumn: '1/-1' }}>
                ...
                <input style={inputStyle} value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. Rafidain Bank — Main" />
              </div>
              <div>
                <label style={labelStyle}>Bank Name</label>
                <input style={inputStyle} value={form.bank_name} onChange={e => setForm(f => ({ ...f, bank_name: e.target.value }))} placeholder="Rafidain Bank" />
              </div>
              <div>
                <label style={labelStyle}>Branch</label>
                <input style={inputStyle} value={form.branch} onChange={e => setForm(f => ({ ...f, branch: e.target.value }))} placeholder="Main Branch" />
              </div>
              <div>
                <label style={labelStyle}>Account Number</label>
                <input style={inputStyle} value={form.account_number} onChange={e => setForm(f => ({ ...f, account_number: e.target.value }))} placeholder="XXXXXXXXXX" />
              </div>
              <div>
              ...
              <div style={{ gridColumn: '1/-1' }}>
                ...
                <select style={inputStyle} value={form.gl_account_id} onChange={e => setForm(f => ({ ...f, gl_account_id: e.target.value }))}>
              ...
              <div>
                ...
                <input style={inputStyle} type="number" value={form.opening_balance} onChange={e => setForm(f => ({ ...f, opening_balance: e.target.value }))} />
              </div>
              <div>
