            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              <thead>
                <tr style={{ background: theme.bgSurface }}>
                  {['Month', 'Payments Subject to WHT', 'WHT Amount', 'Effective Rate'].map(
                    (h, i) => (
                      <th
                        key={i}
                        style={{
              ...
              <tbody>
                {monthly.map((row, i) => {
                  const rate =
                    row.payments_subject_to_wht > 0
                      ? (row.wht_amount / row.payments_subject_to_wht) * 100
                      : 0
                  return (
                    <tr
                      key={i}
                    ...
                    >
                      <td
                        style={{ padding: '10px 14px', color: theme.textPrimary, fontWeight: 500 }}
                      >
                        {MONTHS[row.month - 1]} {data?.year}
                      </td>
                      <td style={{ padding: '10px 14px', textAlign: 'right' }}>
                        <AmountDisplay
                          amount={row.payments_subject_to_wht}
                          currency="IQD"
                          size="sm"
                        />
                      </td>
