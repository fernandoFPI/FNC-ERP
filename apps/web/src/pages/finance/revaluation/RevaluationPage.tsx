...
export default function RevaluationPage() {
  ...
  async function handleReverse() {
    ...
    try {
      await api.post(`/finance/revaluation/${reverseModal.runId}/reverse`, {
  ...
  const totalGainLoss = runs
    .filter((r) => r.status === 'posted')
    .reduce((s, r) => s + Number(r.total_gain_loss), 0)
  const gains = runs
    .filter((r) => r.status === 'posted' && Number(r.total_gain_loss) > 0)
    .reduce((s, r) => s + Number(r.total_gain_loss), 0)
  const losses = runs
    .filter((r) => r.status === 'posted' && Number(r.total_gain_loss) < 0)
    .reduce((s, r) => s + Number(r.total_gain_loss), 0)
        ...
        <Card padding="sm">
          ...
          <p
            style={{
              fontSize: '20px',
              fontWeight: 700,
              color: losses < 0 ? '#ef4444' : theme.textMuted,
              fontFamily: 'monospace',
            }}
          >
          ...
          <tbody>
            ...
            {runs.map((r) => (
              <tr key={r.id} style={{ borderBottom: `1px solid ${theme.border}` }}>
                ...
                <td
                  style={{ padding: '9px 12px', color: theme.textSecondary, textAlign: 'center' }}
                >
                  {r.line_count}
                </td>
                <td
                  style={{
                    ...
                    fontFamily: 'monospace',
                    fontWeight: 700,
                    color: Number(r.total_gain_loss) >= 0 ? '#22c55e' : '#ef4444',
                  }}
                >
                  {Number(r.total_gain_loss) >= 0 ? '+' : ''}
                  {Number(r.total_gain_loss).toLocaleString()}
                </td>
                <td
                  style={{
                    padding: '9px 12px',
                    color: r.journal_entry_id ? theme.accent : theme.textMuted,
                    fontSize: '11px',
                    fontFamily: 'monospace',
                  }}
                >
                ...
                <td style={{ padding: '9px 12px' }}>
                  {r.status === 'posted' && (
                    <Button
                      ...
                      size="sm"
                      onClick={() => {
                        setReverseModal({ runId: r.id, period: r.period })
                      }}
                    >
                      Reverse
                    </Button>
          ...
          <Card padding="lg" style={{ width: '680px', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            >
              <div>
                ...
                <input
                  style={inputStyle}
                  value={computeForm.period}
                  placeholder="2025-06"
                  onChange={(e) => {
                    setComputeForm((f) => ({ ...f, period: e.target.value }))
                  }}
                />
              </div>
              <div>
                ...
                <input
                  type="date"
                  style={inputStyle}
                  value={computeForm.run_date}
                  onChange={(e) => {
                    setComputeForm((f) => ({ ...f, run_date: e.target.value }))
                  }}
                />
              </div>
              <div>
                ...
                <select
                  style={inputStyle}
                  value={computeForm.functional_currency}
                  onChange={(e) => {
                    setComputeForm((f) => ({ ...f, functional_currency: e.target.value }))
                  }}
                >
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    setComputeForm((f) => ({ ...f, notes: e.target.value }))
                  }}
                />
              </div>
            </div>
            ...
            <Button
              ...
              disabled={computing || !computeForm.period}
            >
            ...
            {preview && (
              <>
                <div
                  style={{
                    marginTop: '16px',
                    background: preview.total_gain_loss >= 0 ? '#22c55e18' : '#ef444418',
                    border: `1px solid ${preview.total_gain_loss >= 0 ? '#22c55e' : '#ef4444'}`,
                    borderRadius: '8px',
                    padding: '10px 14px',
                    marginBottom: '12px',
                  }}
                >
                  <p
                    style={{
                      fontSize: '13px',
                      fontWeight: 700,
                      color: preview.total_gain_loss >= 0 ? '#22c55e' : '#ef4444',
                      margin: 0,
                    }}
                  >
                    Net Gain/(Loss): {preview.total_gain_loss >= 0 ? '+' : ''}
                    {preview.total_gain_loss.toLocaleString()} {computeForm.functional_currency}
                  </p>
                  <p style={{ fontSize: '11px', color: theme.textMuted, marginTop: '4px' }}>
                    {preview.lines.length} accounts to revalue
                  </p>
                </div>
                      ...
                      {preview.lines.map((l) => (
                        ...
                        >
                          <td style={{ padding: '5px 8px', textAlign: 'left' }}>
                            ...
                            <span style={{ color: theme.textPrimary, marginLeft: '6px' }}>
                              {l.account_name}
                            </span>
                          </td>
                          <td
                          ...
                          >
                            {l.original_balance.toLocaleString()}
                          </td>
                          <td
                            style={{
                              padding: '5px 8px',
                          ...
                          >
                            {l.revalued_balance.toLocaleString()}
                          </td>
                          <td
                            style={{
                              padding: '5px 8px',
                              ...
                              fontFamily: 'monospace',
                              fontWeight: 600,
                              color: l.gain_loss >= 0 ? '#22c55e' : '#ef4444',
                            }}
                          >
                            {l.gain_loss >= 0 ? '+' : ''}
                            {l.gain_loss.toLocaleString()}
                          </td>
                        </tr>
                      ))}
                    </tbody>
                ...
                >
                  <div>
                    ...
                    <select
                      style={inputStyle}
                      value={computeForm.gain_account_id}
                      onChange={(e) => {
                        setComputeForm((f) => ({ ...f, gain_account_id: e.target.value }))
                      }}
                    >
                  ...
                  <div>
                    ...
                    <select
                      style={inputStyle}
                      value={computeForm.loss_account_id}
                      onChange={(e) => {
                        setComputeForm((f) => ({ ...f, loss_account_id: e.target.value }))
                      }}
                    >
                      <option value="">— Select GL account —</option>
                      {glAccounts.map((g) => (
                ...
                <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
                  ...
                  <Button
                    ...
                    disabled={
                      posting ||
                      !computeForm.gain_account_id ||
                      !computeForm.loss_account_id ||
        ...
        >
          <Card padding="lg" style={{ width: '380px' }}>
            ...
            <p style={{ fontSize: '12px', color: theme.textSecondary, marginBottom: '16px' }}>
              This will create a reversal journal entry that cancels the FX revaluation for period{' '}
              <strong>{reverseModal.period}</strong>.
            </p>
