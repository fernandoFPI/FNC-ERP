...
export default function APDashboard() {
  ...
  const filtered = invoices
    .filter((inv) => {
      ...
      return (
        inv.invoice_number.toLowerCase().includes(q) || inv.vendor_name.toLowerCase().includes(q)
      )
    })
    .sort((a, b) => {
    ...
    <div style={{ ...pagePadding, margin: '0 auto', maxWidth: '1600px' }}>
        ...
        {[
          {
            ...
            color: totalOutstanding > 0 ? theme.danger : theme.textPrimary,
          },
          {
            label: 'Pending Approval',
            value: `${s?.pending_approval_count ?? 0}`,
            sub: 'invoices',
            color: (s?.pending_approval_count ?? 0) > 0 ? theme.warning : theme.textPrimary,
            onClick: () => {
          ...
          },
          {
            ...
            sub: `IQD ${fmtAmt(b31 + b61 + b90)} overdue`,
            color: (s?.overdue_count ?? 0) > 0 ? theme.danger : theme.textPrimary,
          },
          {
            label: 'Paid This Month',
            value: `IQD ${fmtAmt(s?.paid_this_month ?? 0)}`,
            color: theme.success,
          },
        ].map((kpi, i) => (
          <div
            key={i}
            style={{ cursor: kpi.onClick ? 'pointer' : undefined }}
            onClick={kpi.onClick}
          >
            <Card padding="md">
              {loading ? (
                ...
                <>
                  ...
                  <p style={{ fontSize: '22px', fontWeight: 700, color: kpi.color, margin: 0 }}>
                    {kpi.value}
                  </p>
                  {kpi.sub && (
      ...
      <Card padding="none">
        ...
        >
          ...
          />
          {(
            ['', 'draft', 'submitted', 'approved', 'partially_paid', 'paid', 'cancelled'] as const
          ).map((s) => (
            <button
              key={s}
              onClick={() => {
      ...
      <ConfirmDialog
        ...
        message={
          approvingInv
            ? `Approve invoice ${approvingInv.invoice_number} from ${approvingInv.vendor_name} for IQD ${fmtAmt(approvingInv.net_payable)}?`
            : ''
        }
