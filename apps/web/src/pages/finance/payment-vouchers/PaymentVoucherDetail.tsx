...
import { buildPaymentVoucherHTML, VoucherPrintLine, VoucherPrintJournal } from '../../../lib/voucherHtml'
...
const EMPTY_LINE = (): VoucherLine => ({
  statement: '', acct_1: '', acct_2: '', acct_3: '', acct_4: '', acct_5: '',
  amount_iqd: '', amount_usd: '', sequence: 0,
})

const STATUS_VARIANT: Record<string, 'neutral' | 'info' | 'success' | 'warning' | 'danger'> = {
  draft: 'warning', approved: 'info', paid: 'success', cancelled: 'danger',
}
...
export default function PaymentVoucherDetail() {
  ...
  // Queries
  const { data: voucherData, loading: loadingVoucher, refetch } = useQuery<{ paymentVoucher: VoucherDetail }>(
    PAYMENT_VOUCHER_QUERY,
    { variables: { id }, skip: isNew || !id, fetchPolicy: 'cache-and-network' },
  )
  const { data: journalsData } = useQuery<{ journalEntries: JournalSummary[] }>(JOURNAL_ENTRIES_QUERY, {
  ...
  useEffect(() => {
    ...
    setLines(voucher.lines.length > 0 ? voucher.lines.map((l) => ({
      ...l,
  ...
  const isReadOnly = !isNew && (voucher?.status === 'approved' || voucher?.status === 'paid' || voucher?.status === 'cancelled')
  ...
  function buildInput() {
    return {
      ...
      lines: lines.filter((l) => l.statement.trim() !== '').map((l, i) => ({
        statement: l.statement,
  ...
  async function handleSave() {
    ...
    try {
      if (isNew) {
        ...
        addToast({ type: 'success', message: `Voucher ${data.createPaymentVoucher.voucher_number} created` })
  ...
  async function handleApprove() {
    try {
      await approveVoucher({ variables: { id }, refetchQueries: [{ query: PAYMENT_VOUCHER_QUERY, variables: { id } }] })
  ...
  async function handleMarkPaid() {
    try {
      await markVoucherPaid({ variables: { id }, refetchQueries: [{ query: PAYMENT_VOUCHER_QUERY, variables: { id } }] })
      ...
      setLines(
        selected.map((j, idx) => {
          ...
          return {
            ...
            acct_1: '', acct_2: '', acct_3: '', acct_4: '', acct_5: '',
  ...
  const labelStyle: React.CSSProperties = {
    fontSize: '11px', color: theme.textMuted, fontWeight: 500,
  ...
  return (
    <div style={{ padding: '24px', margin: '0 auto', maxWidth: '1400px' }}>
      <PageHeader
        ...
        status={!isNew && voucher ? <Badge variant={STATUS_VARIANT[voucher.status] ?? 'neutral'}>{voucher.status}</Badge> : undefined}
        actions={
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            {!isReadOnly && (
              <Button variant="primary" size="sm" onClick={handleSave} loading={saving || creating || updating}>
                {isNew ? 'Create Voucher' : 'Save Changes'}
              </Button>
            )}
            {!isNew && voucher?.status === 'draft' && (
              <Button variant="secondary" size="sm" onClick={() => setShowApproveDialog(true)}>
                Approve
              </Button>
            )}
            {!isNew && voucher?.status === 'approved' && (
              <Button variant="secondary" size="sm" onClick={() => setShowPaidDialog(true)}>
                Mark Paid
              </Button>
            )}
            {!isNew && (
      ...
      {/* Tab bar */}
      {!isNew && (
        <div style={{ display: 'flex', gap: '2px', borderBottom: `1px solid ${theme.border}`, marginTop: '12px' }}>
      ...
      {voucher?.audited_at && tab === 'details' && (
        <div style={{ marginTop: '12px', padding: '8px 14px', background: theme.bgSurface, border: `1px solid ${theme.border}`, borderRadius: '8px', display: 'flex', gap: '20px', fontSize: '12px', color: theme.textSecondary }}>
          <span>Auditor: <strong>{voucher.auditor_email ?? '—'}</strong></span>
          <span>Audited at: <strong>{new Date(voucher.audited_at).toLocaleString()}</strong></span>
          {voucher.cashier_email && <span>Cashier: <strong>{voucher.cashier_email}</strong></span>}
        </div>
      )}
      ...
      {tab === 'preview' && (
        <div style={{ marginTop: '16px' }}>
          ...
          <div style={{ border: `1px solid ${theme.border}`, borderRadius: '8px', overflow: 'hidden', background: '#f5f5f5' }}>
            <iframe
              ref={iframeRef}
              srcDoc={previewHtml}
      ...
      {/* Details tab */}
      {(tab === 'details' || isNew) && (
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 400px', gap: '16px', marginTop: '16px', alignItems: 'start' }}>
        {/* Main column */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <Card>
            <div style={{ padding: '16px', borderBottom: `1px solid ${theme.border}` }}>
              <span style={{ fontWeight: 600, color: theme.textPrimary, fontSize: '14px' }}>Voucher Details</span>
            </div>
            <div style={{ padding: '16px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>

              <div>
                <label style={labelStyle}>Date *</label>
                <input type="date" style={inputStyle} value={form.voucher_date} readOnly={isReadOnly}
                  onChange={(e) => setForm((f) => ({ ...f, voucher_date: e.target.value }))} />
              </div>

              <div>
                <label style={labelStyle}>Received From (ادفعوا لأمر) *</label>
                <input type="text" style={inputStyle} value={form.received_from} readOnly={isReadOnly}
              ...
              <div>
                ...
                <input type="text" style={inputStyle} value={form.reference_to} readOnly={isReadOnly}
                  placeholder="Invoice / contract reference"
                  onChange={(e) => setForm((f) => ({ ...f, reference_to: e.target.value }))} />
              </div>

              <div>
                <label style={labelStyle}>Bank Account / Fund (حسابات البنك/الصندوق)</label>
                <input type="text" style={inputStyle} value={form.bank_account_fund} readOnly={isReadOnly}
                  placeholder="Account name or fund"
                  onChange={(e) => setForm((f) => ({ ...f, bank_account_fund: e.target.value }))} />
              </div>

              <div>
                <label style={labelStyle}>Receiver Name (المستلم)</label>
                <input type="text" style={inputStyle} value={form.receiver_name} readOnly={isReadOnly}
              ...
              <div>
                ...
                <input type="text" style={inputStyle} value={form.notes} readOnly={isReadOnly}
          ...
          <Card>
            <div style={{ padding: '14px 16px', borderBottom: `1px solid ${theme.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontWeight: 600, color: theme.textPrimary, fontSize: '14px' }}>Payment Lines</span>
              {!isReadOnly && (
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                <thead>
                  <tr style={{ background: theme.bgSurface }}>
                    {['Acct 1', 'Acct 2', 'Acct 3', 'Acct 4', 'Acct 5', 'Statement (البيان)', 'Amount IQD', 'Amount USD', ''].map((h) => (
                      <th key={h} style={{ padding: '8px 8px', textAlign: 'left', fontWeight: 600, fontSize: '11px', color: theme.textMuted, textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: `1px solid ${theme.border}`, whiteSpace: 'nowrap' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr key={idx} style={{ borderBottom: `1px solid ${theme.border}` }}>
                      {(['acct_1', 'acct_2', 'acct_3', 'acct_4', 'acct_5'] as const).map((f) => (
                        <td key={f} style={{ padding: '6px 6px' }}>
                          <input
                            ...
                            value={line[f]}
                            maxLength={12}
                            style={{ ...inputStyle, width: '64px', textAlign: 'center', fontFamily: 'monospace', fontSize: '11px' }}
                            onChange={(e) => updateLine(idx, f, e.target.value)}
                          />
                        </td>
                      ))}
                      <td style={{ padding: '6px 6px' }}>
                        <input
                          ...
                          onChange={(e) => updateLine(idx, 'statement', e.target.value)}
                        />
                      </td>
                      <td style={{ padding: '6px 6px' }}>
                        <input
                          ...
                          min="0"
                          step="0.01"
                          style={{ ...inputStyle, width: '110px', textAlign: 'right', fontFamily: 'monospace' }}
                          onChange={(e) => updateLine(idx, 'amount_iqd', e.target.value)}
                        />
                      </td>
                      <td style={{ padding: '6px 6px' }}>
                        <input
                          ...
                          min="0"
                          step="0.01"
                          style={{ ...inputStyle, width: '100px', textAlign: 'right', fontFamily: 'monospace' }}
                          onChange={(e) => updateLine(idx, 'amount_usd', e.target.value)}
                        />
                      </td>
                      <td style={{ padding: '6px 8px' }}>
                        {!isReadOnly && lines.length > 1 && (
                          <button onClick={() => removeLine(idx)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: theme.danger ?? '#e53e3e', fontSize: '16px', lineHeight: 1 }}>×</button>
                        )}
                      </td>
                    </tr>
                  ))}
                  ...
                  <tr style={{ background: theme.bgSurface, fontWeight: 600 }}>
                    <td colSpan={5}></td>
                    <td style={{ padding: '8px', fontSize: '12px', color: theme.textSecondary, textAlign: 'right', paddingRight: '16px' }}>Total</td>
                    <td style={{ padding: '8px', fontFamily: 'monospace', textAlign: 'right' }}>
                      <AmountDisplay amount={totalIqd} currency="IQD" />
                    </td>
                    <td style={{ padding: '8px', fontFamily: 'monospace', textAlign: 'right' }}>
                      {totalUsd > 0 ? <AmountDisplay amount={totalUsd} currency="USD" /> : <span style={{ color: theme.textMuted }}>—</span>}
                    </td>
                    <td></td>
                  </tr>
                </tbody>
          ...
          {activeJournals.length > 0 && (
            <Card>
              <div style={{ padding: '14px 16px', borderBottom: `1px solid ${theme.border}` }}>
                <span style={{ fontWeight: 600, color: theme.textPrimary, fontSize: '14px' }}>Linked Journal Entries</span>
              </div>
              <div style={{ padding: '12px' }}>
                {activeJournals.map((j) => (
                  <div key={j.id} style={{ marginBottom: '12px', padding: '12px', background: theme.bgSurface, borderRadius: '8px', border: `1px solid ${theme.border}` }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '6px' }}>
                      <div>
                        <span style={{ fontFamily: 'monospace', color: theme.accent, fontWeight: 600, marginRight: '10px' }}>{j.reference}</span>
                        <span style={{ fontSize: '12px', color: theme.textMuted }}>{j.entry_date}</span>
                        {j.audited_at && <span style={{ marginLeft: '8px' }}><Badge variant="success">Audited</Badge></span>}
                      </div>
                      <Badge variant={j.status === 'posted' ? 'success' : 'warning'}>{j.status}</Badge>
                    </div>
                    {j.description && (
                      <div style={{ fontSize: '12px', color: theme.textSecondary, marginBottom: '6px' }}>{j.description}</div>
                    )}
                    {j.linked_pos && j.linked_pos.length > 0 && (
                      <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                        {j.linked_pos.map((po) => (
                          <span key={po.po_id} style={{ fontSize: '11px', padding: '2px 8px', background: theme.bgCanvas, border: `1px solid ${theme.border}`, borderRadius: '4px', color: theme.textSecondary }}>
        ...
        <div style={{ position: 'sticky', top: '16px' }}>
          <Card>
            <div style={{ padding: '14px 16px', borderBottom: `1px solid ${theme.border}` }}>
              <span style={{ fontWeight: 600, color: theme.textPrimary, fontSize: '14px' }}>Link Journal Entries</span>
              <div style={{ fontSize: '11px', color: theme.textMuted, marginTop: '2px' }}>Posted journals to include in this voucher</div>
            </div>
            ...
            <div style={{ maxHeight: '460px', overflowY: 'auto' }}>
              {filteredJournals.length === 0 ? (
                <div style={{ padding: '20px', textAlign: 'center', color: theme.textMuted, fontSize: '13px' }}>No posted journals found</div>
              ) : (
                    ...
                    <label key={j.id} style={{
                      ...
                      <div style={{ minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
                          <span style={{ fontFamily: 'monospace', fontSize: '12px', fontWeight: 600, color: theme.accent }}>{j.reference}</span>
                          <span style={{ fontSize: '11px', color: theme.textMuted }}>{j.entry_date}</span>
                          {j.audited_at && <span style={{ fontSize: '10px', padding: '1px 5px', background: '#dcf5e7', color: '#166534', borderRadius: '4px' }}>audited</span>}
                        </div>
                        {j.description && (
                          <div style={{ fontSize: '11px', color: theme.textSecondary, marginTop: '2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '260px' }}>
                            {j.description}
                          </div>
                        )}
                        {j.linked_pos && j.linked_pos.length > 0 && (
            ...
            <div style={{ padding: '10px 14px', borderTop: `1px solid ${theme.border}`, fontSize: '12px', color: theme.textMuted }}>
