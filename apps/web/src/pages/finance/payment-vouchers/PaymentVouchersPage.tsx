  ...
  const columns: Column<PaymentVoucher>[] = [
    {
      key: 'voucher_number',
      header: 'Voucher No.',
      render: (v) => <span style={{ fontFamily: 'monospace', color: theme.accent, fontWeight: 600 }}>{v.voucher_number}</span>,
    },
    {
      key: 'voucher_date',
      header: 'Date',
      render: (v) => <span style={{ color: theme.textSecondary, fontSize: '13px' }}>{v.voucher_date}</span>,
    },
    {
      key: 'received_from',
      header: 'Received From',
      render: (v) => <span style={{ color: theme.textPrimary, fontSize: '13px' }}>{v.received_from}</span>,
    },
    {
      key: 'status',
      header: 'Status',
      render: (v) => <Badge variant={STATUS_VARIANT[v.status] ?? 'neutral'}>{v.status.charAt(0).toUpperCase() + v.status.slice(1)}</Badge>,
    },
    {
    ...
    },
    {
      ...
      header: 'Amount (USD)',
      render: (v) => parseFloat(v.total_amount_usd) > 0
    ...
    },
    {
      key: 'journal_count',
      header: 'Journals',
      render: (v) => <span style={{ color: theme.textMuted, fontSize: '13px' }}>{v.journal_count ?? '0'}</span>,
    },
    {
      key: 'auditor_email',
      header: 'Auditor',
      render: (v) => <span style={{ color: theme.textMuted, fontSize: '12px' }}>{v.auditor_email ?? '—'}</span>,
    },
  ]
  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1400px', margin: '0 auto' }}>
      <PageHeader
        ...
        actions={
          <Button variant="primary" size="sm" onClick={() => navigate('/finance/payment-vouchers/new')}>
            New Voucher
          </Button>
        }
      />
      ...
      <Card style={{ marginTop: '16px' }}>
        ...
        <Table
          ...
          loading={loading}
          rowKey="id"
          onRowClick={(v) => navigate(`/finance/payment-vouchers/${v.id}`)}
        />
      </Card>
    </div>
  )
