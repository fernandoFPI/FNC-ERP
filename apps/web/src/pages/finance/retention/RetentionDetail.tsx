...
export default function RetentionDetail() {
  ...
  const releasePct =
    Number(record.retention_amount) > 0
  ...
  return (
    <div style={{ padding: '24px' }}>
      ...
      <Card padding="md" style={{ marginBottom: '16px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '6px' }}>
          <span style={{ fontSize: '11px', color: theme.textMuted }}>
            Released: {Number(record.released_amount).toLocaleString()} IQD
          </span>
          <span style={{ fontSize: '11px', color: theme.textMuted }}>
            Remaining: {outstanding.toLocaleString()} IQD
          </span>
        </div>
      </Card>
      ...
      >
        ...
        <Card
          padding="sm"
          style={{ border: outstanding > 0 && isOverdue ? '1px solid #ef4444' : undefined }}
        >
          <p style={{ fontSize: '10px', color: theme.textMuted, marginBottom: '4px' }}>
            Outstanding
          </p>
        ...
        <div>
          <Card padding="none">
            ...
            {!releases.length ? (
              ...
              >
                No releases yet.
                {canRelease ? ' Click "Release Retention" to record the first release.' : ''}
              </div>
            ) : (
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                <thead>
                ...
                <tbody>
                  {releases.map((rel) => (
                    <tr key={rel.id} style={{ borderBottom: `1px solid ${theme.border}` }}>
                      <td style={{ padding: '8px 12px', color: theme.textPrimary }}>
                        {new Date(rel.release_date).toLocaleDateString()}
                      </td>
                      <td style={{ padding: '8px 12px' }}>
                        <AmountDisplay amount={rel.amount} currency="IQD" size="sm" colored />
                      </td>
                      <td
                        style={{
                          padding: '8px 12px',
                          color: rel.journal_entry_id ? theme.accent : theme.textMuted,
                          fontSize: '11px',
                          fontFamily: 'monospace',
                        }}
                      >
          ...
          <Card padding="lg" style={{ width: '460px' }}>
            ...
            >
              ...
              >
                ...
                <span
                  style={{ color: theme.textPrimary, fontWeight: 600, fontFamily: 'monospace' }}
                >
                  {Number(record.retention_amount).toLocaleString()} IQD
                </span>
              </div>
              <div
                style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}
              >
                ...
                <span style={{ color: theme.textSecondary, fontFamily: 'monospace' }}>
                  {Number(record.released_amount).toLocaleString()} IQD
                </span>
              </div>
              <div
                style={{
              ...
              >
                ...
                <span style={{ color: theme.accent, fontFamily: 'monospace' }}>
                  {outstanding.toLocaleString()} IQD
                </span>
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gap: '12px' }}>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    setReleaseForm((f) => ({ ...f, release_date: e.target.value }))
                  }}
                />
              </div>
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Project milestone completion"
                  onChange={(e) => {
                    setReleaseForm((f) => ({ ...f, notes: e.target.value }))
                  }}
                />
              </div>
              <div>
                ...
                >
                  <input
                    type="checkbox"
                    checked={releaseForm.post_journal_entry}
                    onChange={(e) => {
                      setReleaseForm((f) => ({ ...f, post_journal_entry: e.target.checked }))
                    }}
                  />
            ...
            >
              ...
              <Button
                ...
                disabled={
                  saving ||
                  !releaseForm.amount ||
                  Number(releaseForm.amount) > outstanding ||
            ...
            <div style={{ display: 'grid', gap: '12px' }}>
              <div>
                ...
                <input
                  type="date"
                  style={inputStyle}
                  value={editForm.expected_release_date}
                  onChange={(e) => {
                    setEditForm((f) => ({ ...f, expected_release_date: e.target.value }))
                  }}
                />
              </div>
              <div>
                <label style={labelStyle}>
                  {record.retention_type === 'ar'
                    ? 'Retention Receivable Account'
                    : 'Retention Payable Account'}
                </label>
                <select
                  style={inputStyle}
                  value={editForm.retention_account_id}
                  onChange={(e) => {
                    setEditForm((f) => ({ ...f, retention_account_id: e.target.value }))
                  }}
                >
              ...
              <div>
                <label style={labelStyle}>
                  {record.retention_type === 'ar' ? 'AR Account (offset)' : 'AP Account (offset)'}
                </label>
                <select
                  style={inputStyle}
                  value={editForm.offset_account_id}
                  onChange={(e) => {
                    setEditForm((f) => ({ ...f, offset_account_id: e.target.value }))
                  }}
                >
                  <option value="">— Select GL account —</option>
                  {glAccounts.map((g) => (
