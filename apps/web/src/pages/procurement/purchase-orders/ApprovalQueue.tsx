...
export default function ApprovalQueue() {
  ...
  const columns: Column<POItem>[] = [
    ...
    { key: 'vendor_name', header: 'Vendor', render: (o) => <span style={{ color: theme.textPrimary }}>{o.vendor_name ?? '—'}</span> },
    {
      key: 'status',
      header: 'Status',
      render: (o) => <Badge variant="warning">{o.status.replace(/_/g, ' ')}</Badge>,
    },
    { key: 'total_amount', header: 'Amount', render: (o) => <AmountDisplay amount={parseFloat(o.total_amount)} currency={o.currency_code} /> },
    {
      key: 'submitted_at',
      header: 'Submitted',
      render: (o) => <span style={{ color: theme.textMuted, fontSize: '13px' }}>{o.submitted_at ? o.submitted_at.slice(0, 10) : '—'}</span>,
    },
    {
      key: 'assigned_to_email',
      header: 'Assigned To',
      render: (o) => <span style={{ color: theme.textMuted, fontSize: '12px' }}>{o.assigned_to_email ?? '—'}</span>,
    },
  ]
    ...
    <div style={{ padding: '24px', margin: '0 auto', maxWidth: '1200px' }}>
      <PageHeader
        ...
        actions={
          <button
            onClick={() => refetch()}
            style={{ background: 'none', border: 'none', cursor: 'pointer', color: theme.accent, fontSize: '13px' }}
          >
            Refresh
          </button>
        }
