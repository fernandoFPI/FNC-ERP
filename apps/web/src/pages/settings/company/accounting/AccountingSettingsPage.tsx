...
const GL_ACCOUNTS = [
  ...
  { label: 'WHT Payable', code: '2310', description: 'WHT withheld from vendors, payable to tax authority' },
  { label: 'Payroll Expense', code: '5100', description: 'Default salaries and wages expense account' },
  { label: 'Social Security Payable', code: '2320', description: 'Employee and employer SS contributions payable' },
    ...
    <div style={{ padding: '24px' }}>
      ...
      <Card padding="none" style={{ marginBottom: '16px' }}>
        <div style={{ padding: '12px 20px', borderBottom: `1px solid ${theme.border}`, display: 'flex', alignItems: 'center', gap: '10px' }}>
          <h3 style={{ fontSize: '13px', fontWeight: 600, color: theme.textPrimary }}>Default Account Mapping</h3>
          <Badge variant="info" size="sm">Auto-configured</Badge>
        </div>
        {GL_ACCOUNTS.map((acct, i) => (
          ...
          >
            <div>
              <p style={{ fontSize: '13px', fontWeight: 500, color: theme.textPrimary }}>{acct.label}</p>
      ...
      <div style={{
        ...
        Account codes are assigned automatically per company during provisioning. To change default account mappings, update the chart of accounts in Finance → Accounts.
      </div>
    </div>
  )
}
