...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    api
      .get<PersonalProfile>('/users/me/profile')
      .then((r) => {
        setPersonal(r.data)
        setPersonalForm({
  ...
  async function savePersonal() {
    if (!personalForm.firstName.trim() || !personalForm.lastName.trim()) {
      addToast({ type: 'error', message: 'First and last name are required' })
      return
    }
  ...
  } = useQuery<ProfileData>(MY_PROFILE_QUERY)

  const {
    data: sessionsData,
    loading: sessionsLoading,
    refetch: refetchSessions,
  } = useQuery<SessionsData>(MY_SESSIONS_QUERY, {
            ...
            {personalLoading ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
                ...
                <Input
                  ...
                  onChange={(e) => {
                    setPersonalForm((f) => ({ ...f, jobTitle: e.target.value }))
                  }}
          ...
          <Card padding="md" style={{ maxWidth: '440px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              <Input
                ...
                error={
                  confirmPwd && newPassword !== confirmPwd ? 'Passwords do not match' : undefined
                }
              />
              <Button
                variant="primary"
            ...
            {profile?.mfaEnabled ? (
              <div>
                ...
                <p style={{ fontSize: '13px', color: theme.textSecondary, marginBottom: '16px' }}>
                  Your account is protected with an authenticator app. To disable, enter your
                  password below.
                </p>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
                  <thead>
                    <tr style={{ background: theme.bgSurface }}>
                      {['Device', 'Platform', 'IP Address', 'Created', 'Last Active', ''].map(
                        (h) => (
                          <th
                            key={h}
                            style={{
