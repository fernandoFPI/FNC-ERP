...
const LIFECYCLE_STAGES = [
  'Client Enquiry', 'Scope Review', 'Bidding', 'Client Approval', 'Execution', 'Closeout',
]
...
export default function ProjectForm() {
  ...
  const field = (key: keyof FormState, label: string, type = 'text', placeholder?: string) => (
    <div>
      ...
      <input type={type} style={inputStyle(!!errors[key])} value={form[key]} onChange={set(key)} placeholder={placeholder} />
    </div>
  )

  const dateTimeRow = (dateKey: keyof FormState, dateLabel: string, timeKey: keyof FormState, required?: boolean) => (
    <div style={grid2}>
      <div>
        <label style={labelStyle}>
          {dateLabel}{required ? ' *' : ''}
          {errors[dateKey] && <span style={{ color: '#ef4444', fontWeight: 400, marginLeft: '6px', textTransform: 'none' }}>{errors[dateKey]}</span>}
        </label>
        <input type="date" style={inputStyle(!!errors[dateKey])} value={form[dateKey]} onChange={set(dateKey)} />
      </div>
      <div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px', paddingBottom: '80px' }}>
              ...
              <React.Fragment key={s}>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minWidth: '60px' }}>
                  <div style={{
                    ...
                    background: i < stageIdx ? theme.accent : i === stageIdx ? theme.accent : theme.bgCanvas,
                  ...
                  <div style={{
                    fontSize: '9px', marginTop: '5px', textAlign: 'center', lineHeight: 1.3,
                ...
                {i < LIFECYCLE_STAGES.length - 1 && (
                  <div style={{ flex: 1, height: '2px', marginTop: '12px', background: i < stageIdx ? theme.accent : theme.border }} />
                )}
              </React.Fragment>
            ))}
          </div>
        ...
        <div style={card}>
          ...
          {!isEdit && (
            <div style={{ marginBottom: '14px', padding: '8px 12px', background: theme.accentBg, border: `1px solid ${theme.accentBorder}`, borderRadius: '7px', fontSize: '12px', color: theme.textMuted }}>
        ...
        <div style={card}>
          ...
          <p style={{ margin: '10px 0 0', fontSize: '12px', color: theme.textMuted }}>
            Additional team members (technical, commercial, site) can be added from the Team tab after creation.
          </p>
        </div>
        ...
        <div style={card}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <div>
              <label style={labelStyle}>
                Receiving Date *
                {errors.receivingDate && <span style={{ color: '#ef4444', fontWeight: 400, marginLeft: '6px', textTransform: 'none' }}>{errors.receivingDate}</span>}
              </label>
              <input type="date" style={inputStyle(!!errors.receivingDate)} value={form.receivingDate} onChange={set('receivingDate')} />
            </div>
      ...
      {/* ── Sticky footer ─────────────────────────────────────────────────── */}
      <div style={{
        position: 'sticky', bottom: 0, display: 'flex', justifyContent: 'flex-end',
        gap: '8px', padding: '12px 24px', borderTop: `1px solid ${theme.border}`,
        background: theme.bgSurface, zIndex: 10,
      }}>
