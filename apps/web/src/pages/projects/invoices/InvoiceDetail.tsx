...
import { buildInvoiceHTML, type BankAccount as InvoiceBankAccount, type InvoiceLine } from '../../../lib/invoiceHtml'
...
export default function InvoiceDetail() {
  ...
  const [editLines,       setEditLines]       = useState<Array<{ _key: string; id: string; description: string; qty: string; unitCost: string }>>([])
  ...
  const bankAccounts: BankAccount[] = bankData?.bankAccounts?.filter((a: BankAccount) => a.isActive) ?? []
  ...
  useEffect(() => {
    ...
    void QRCode.toDataURL(url, { width: 120, margin: 1, color: { dark: '#1a3c5e', light: '#ffffff' } })
      .then(setQrDataUrl)
  ...
  async function handleSetBank(bankAccountId: string | null) {
    try {
      ...
      addToast({ type: 'success', message: bankAccountId ? 'Bank account set' : 'Bank account removed' })
      void refetch()
    } catch (e: unknown) {
  ...
  async function handleTransition(action: 'submit-for-review' | 'approve' | 'issue', successMsg: string) {
  ...
  if (loading || !inv) return <div style={{ padding: '24px', color: 'var(--text-muted)' }}>Loading…</div>
  ...
  return (
    <div style={{ padding: '24px', margin: '0 auto', maxWidth: '1160px' }}>
      <PageHeader
        ...
        actions={
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            ...
            {editMode ? (
              <>
                <Button variant="primary" size="sm" loading={saving} onClick={() => void saveEdit()}>Save Changes</Button>
            ...
            {inv.status === 'draft' && (
              <>
                <Button variant="primary" size="sm" loading={transitioning}
                  onClick={() => handleTransition('submit-for-review', 'Invoice submitted for review')}>
                  Submit for Review
                </Button>
                <Button variant="ghost" size="sm" onClick={() => setShowVoid(true)}>Void</Button>
              </>
            )}
            {inv.status === 'review' && (
              <Button variant="primary" size="sm" loading={transitioning}
            ...
            {canPay && (
              <Button variant="primary" size="sm" onClick={() => setPaymentFormOpen(true)}>
                Record Payment
              </Button>
            )}
          </div>
      ...
      {!['issued', 'paid', 'void', 'cancelled'].includes(inv.status) ? (
        <Card style={{ padding: '14px 16px', marginTop: '14px', display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '12px' }}>
          <span style={{ fontSize: '12px', color: theme.textMuted, whiteSpace: 'nowrap' }}>Payment type:</span>
          <SearchableSelect
          ...
          {(inv.paymentType ?? 'wire_transfer') === 'wire_transfer' && (
            <>
              <span style={{ fontSize: '12px', color: theme.textMuted, whiteSpace: 'nowrap' }}>Bank account:</span>
              <SearchableSelect
        ...
        <Card style={{ padding: '14px 16px', marginTop: '14px', display: 'flex', alignItems: 'center', gap: '10px' }}>
          <Badge variant="neutral">{inv.paymentType === 'cash' ? 'Cash' : 'Wire Transfer'}</Badge>
          {selectedBank && (
            <>
              <span style={{ fontSize: '13px', fontWeight: 500, color: theme.textPrimary }}>{selectedBank.accountName}</span>
              <span style={{ fontSize: '12px', color: theme.textMuted }}>{selectedBank.bankName}</span>
              {selectedBank.iban && <span style={{ fontSize: '12px', color: theme.textMuted, fontFamily: 'monospace' }}>{selectedBank.iban}</span>}
            </>
          )}
        </Card>
      )}
      ...
      <div style={{ marginTop: '16px' }}>
        ...
        {tab === 'lines' && (
          <Card style={{ padding: '20px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
              <div style={{ fontWeight: 600, color: theme.textPrimary, fontSize: '14px' }}>Invoice Lines</div>
              {editMode && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <input
                    ...
                    style={{ padding: '4px 8px', borderRadius: '6px', border: `1px solid ${theme.border}`, background: theme.bgCanvas, color: theme.textPrimary, fontSize: '12px' }}
                  />
                </div>
              )}
            </div>
              ...
              <thead>
                <tr style={{ background: theme.bgSurfaceHover }}>
                  {(editMode
                    ...
                    : ['#', 'Description', 'Source', 'Qty', 'Unit Cost', 'Subtotal', 'Margin', 'Total']
                  ).map((h, i) => (
                    <th key={i} style={{ padding: '8px 12px', textAlign: 'left', fontSize: '11px', color: theme.textMuted, fontWeight: 600, borderBottom: `1px solid ${theme.border}` }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {editMode
                  ? editLines.map((el, idx) => {
                      const original = (inv.lines ?? [] as InvoiceLine[]).find((l: InvoiceLine) => l.id === el.id)
                      const qty = parseFloat(el.qty) || 0
                      const unit = parseFloat(el.unitCost) || 0
                      const preview = qty * unit
                      const inpStyle: React.CSSProperties = { width: '100%', padding: '4px 8px', borderRadius: '5px', border: `1px solid ${theme.border}`, background: theme.bgCanvas, color: theme.textPrimary, fontSize: '12px', boxSizing: 'border-box' }
                      return (
                        <tr key={el._key} style={{ borderBottom: `1px solid ${theme.border}` }}>
                          ...
                          <td style={{ padding: '6px 8px' }}>
                            <input
                              ...
                              onChange={(e) => setEditLines((p) => p.map((r) => r._key === el._key ? { ...r, description: e.target.value } : r))}
                            />
                          </td>
                          <td style={{ padding: '6px 12px' }}><Badge variant="neutral">{original?.sourceType ?? 'manual'}</Badge></td>
                          <td style={{ padding: '6px 8px', width: '80px' }}>
                            <input
                              ...
                              onChange={(e) => setEditLines((p) => p.map((r) => r._key === el._key ? { ...r, qty: e.target.value } : r))}
                            />
                          </td>
                          <td style={{ padding: '6px 8px', width: '130px' }}>
                            <input
                              ...
                              value={el.unitCost}
                              onFocus={(e) => e.target.select()}
                              onChange={(e) => setEditLines((p) => p.map((r) => r._key === el._key ? { ...r, unitCost: e.target.value } : r))}
                            />
                          </td>
                          ...
                          <td style={{ padding: '6px 8px', textAlign: 'center' }}>
                            <button
                              onClick={() => removeEditLine(el._key)}
                              style={{ background: 'none', border: 'none', cursor: 'pointer', color: theme.danger ?? '#ef4444', fontSize: '16px', lineHeight: 1, padding: '2px 6px', borderRadius: '4px' }}
                              title="Remove line"
                            >×</button>
                          </td>
                        </tr>
                  ...
                  : (inv.lines ?? []).map((l: InvoiceLine) => (
                      <tr key={l.id} style={{ borderBottom: `1px solid ${theme.border}` }}>
                        <td style={{ padding: '8px 12px', color: theme.textMuted, fontSize: '12px' }}>{l.lineNumber}</td>
                        <td style={{ padding: '8px 12px', color: theme.textPrimary, fontSize: '13px' }}>{l.description}</td>
                        <td style={{ padding: '8px 12px' }}><Badge variant="neutral">{l.sourceType}</Badge></td>
                        <td style={{ padding: '8px 12px', fontFamily: 'monospace', fontSize: '13px', color: theme.textSecondary }}>{l.qty}</td>
                        <td style={{ padding: '8px 12px' }}><AmountDisplay amount={l.unitCost} currency={cur} size="sm" /></td>
                        <td style={{ padding: '8px 12px' }}><AmountDisplay amount={l.subtotal} currency={cur} size="sm" /></td>
                        <td style={{ padding: '8px 12px', fontSize: '12px', color: theme.success }}>{l.marginPct}%</td>
                        <td style={{ padding: '8px 12px', fontWeight: 600 }}><AmountDisplay amount={l.lineTotal} currency={cur} size="sm" /></td>
                      </tr>
                    ))
                }
              </tbody>
            ...
            {editMode && (
              <>
                <div style={{ marginTop: '10px' }}>
                  <button
                    onClick={addEditLine}
                    style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', padding: '5px 12px', border: `1px dashed ${theme.border}`, borderRadius: '6px', background: 'none', cursor: 'pointer', fontSize: '12px', color: theme.accent }}
                  >
                    + Add Line
                  </button>
                </div>
                <div style={{ marginTop: '10px', padding: '10px 12px', background: theme.bgSurfaceHover, borderRadius: '8px', fontSize: '12px', color: theme.textMuted, display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <span>⚠</span>
                  <span>Editing recalculates totals from qty × unit cost. Margin percentage is preserved; net payable is recalculated automatically.</span>
                </div>
              </>
            )}
          </Card>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            <Card style={{ padding: '16px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                ...
                <div style={{ fontSize: '13px', fontWeight: 600, color: outstanding > 0 ? theme.accent : theme.success }}>
                  {outstanding > 0
                    ? `${formatCurrency(outstanding, cur)} outstanding`
                    : 'Fully paid'}
                </div>
              </div>
              <div style={{ height: '8px', background: theme.border, borderRadius: '999px', overflow: 'hidden' }}>
                <div style={{
                  height: '100%',
                  width: `${paidPct}%`,
            ...
            {payments.length > 0 ? (
              <Card style={{ padding: '20px' }}>
                <div style={{ fontWeight: 600, color: theme.textPrimary, marginBottom: '12px', fontSize: '14px' }}>Payment History</div>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr style={{ background: theme.bgSurfaceHover }}>
                  ...
                  <tbody>
                    {payments.map((p) => (
                      <tr key={p.id} style={{ borderBottom: `1px solid ${theme.border}` }}>
                        <td style={{ padding: '8px 12px', color: theme.textSecondary, fontSize: '13px' }}>{p.paymentDate}</td>
                        <td style={{ padding: '8px 12px', fontWeight: 600 }}><AmountDisplay amount={p.amount} currency={cur} /></td>
                        <td style={{ padding: '8px 12px' }}>
                          {p.paymentMethod
                            ? <Badge variant="neutral">{p.paymentMethod.replace('_', ' ')}</Badge>
                            : <span style={{ color: theme.textMuted }}>—</span>}
                        </td>
                        <td style={{ padding: '8px 12px', color: theme.textMuted, fontSize: '12px', fontFamily: 'monospace' }}>
                          {p.paymentReference ?? '—'}
                        </td>
                      </tr>
                    ))}
        ...
        {tab === 'pdf' && (
          <div>
            ...
            <div style={{ border: `1px solid ${theme.border}`, borderRadius: '8px', overflow: 'hidden', background: '#f5f5f5' }}>
              <iframe
                ref={iframeRef}
                srcDoc={buildInvoiceHTML(inv, selectedBank, inv.paymentType ?? 'wire_transfer', inv.companyStampImage, qrDataUrl)}
        ...
        {/* Activity tab */}
        {tab === 'activity' && id && (
          <ActivityLog recordId={id} tableName="project_invoices" />
        )}
      </div>
      ...
      <InvoicePaymentForm
        ...
        onSuccess={() => { setPaymentFormOpen(false); refetch() }}
      />
    </div>
  )
}
